...
# ======================================================================================
    ...
    def _uq__default_zorders(self) -> dict:
        """
        Canonical zorder map for all Cartopy map plots in SHAKEuq.
        Always merge user overrides via _uq__merge_zorders().
        """
        return {
            "ocean": 0,
            "land": 1,
    ...
    def _uq__merge_zorders(self, zorders: dict = None) -> dict:
        """
        Merge user zorders into canonical defaults (casts to float).
        Safe even if user passes weird types.
        """
        z = dict(self._uq__default_zorders())
        if isinstance(zorders, dict):
            for k, v in zorders.items():
                try:
                    z[str(k)] = float(v)
                except Exception:
                    # ignore non-castable overrides
                    pass
        return z
    
    
    def uq_save_state_pickle(self, path: str) -> str:
        """Save current uq_state to a pickle file (compute once, plot later)."""
        import os
        import pickle
    
        if not isinstance(getattr(self, "uq_state", None), dict) or not self.uq_state:
            raise RuntimeError("uq_state is empty. Build dataset first.")
        if not path:
            raise ValueError("path is required")
    
        os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
        with open(path, "wb") as f:
            pickle.dump(self.uq_state, f)
        return path
    
    
    def uq_load_state_pickle(self, path: str) -> dict:
        """Load uq_state from a pickle file into this instance."""
        import pickle
        ...
        if not isinstance(state, dict):
            raise RuntimeError("Loaded object is not a dict uq_state.")
        self.uq_state = state
        return self.uq_state
    
    
    def uq_prepare_method_outputs(
        self,
        *,
        versions,
        imts,
        ...
        method_kwargs_by_method: dict = None,
        save_state_pickle_path: str = None,
        debug: bool = False,
    ) -> list:
        """
        Wrapper: ensure method outputs exist BEFORE plotting.
    
        This does NOT invent new uq_state keys.
        It triggers your existing compute/store pathway by calling _uq__get_field_for_plot()
        with compute_if_missing=True, which relies on your method runners and storage.
    
        Returns: list of records with (version, imt, method, what, status).
        """
        if not getattr(self, "uq_state", None):
            raise RuntimeError("UQ dataset not built yet. Run uq_build_dataset(...) first.")
        ...
        versions = _norm_list(versions)
        imts = _norm_list(imts)
        methods = _norm_list(methods)
        whats = _norm_list(whats)
    
        out = []
        method_kwargs_by_method = method_kwargs_by_method or {}
    
        for v in versions:
            vkey = self._uq__norm_version_safe(v)
            for imt in imts:
                imt_u = str(imt).upper().strip()
                for m in methods:
                    method_u = str(m).strip()
                    mk = method_kwargs_by_method.get(str(m).lower().strip().replace(" ", ""), None)
                    for w in whats:
                        what_u = str(w).lower().strip()
    
                        try:
                            Z, meta, status, mu2d, sig2d = self._uq__get_field_for_plot(
                                version=vkey,
                                imt=imt_u,
                                method=method_u,
                                what=what_u,
                            ...
                            status = "compute_fail"
                            meta = {"error": repr(e), "version": vkey, "imt": imt_u, "method": method_u, "what": what_u}
                            if debug:
                                print(f"[uq_prepare_method_outputs] FAIL v={vkey} imt={imt_u} m={method_u} w={what_u}: {repr(e)}")
        ...
        if save_state_pickle_path:
            self.uq_save_state_pickle(save_state_pickle_path)
    ...
    def uq_plot_method_map(
        ...
        *,
        ...
        fail_mode: str = "raise",   # "raise" | "annotate"
        ...
        # canonical zorders
        zdef = self._uq__merge_zorders(zorders)
        ...
        fail_mode_s = str(fail_mode).lower().strip()
    
        # decide whether we will render a normal map or an annotated “missing/fail” frame
        can_plot_field = (Z is not None) and (status_s == "ok")
    
        if (not can_plot_field) and fail_mode_s == "raise":
            raise RuntimeError(
                f"Requested field missing: method={method_u}, what={what_u}, imt={imt_u}, v={vkey}; status={status}"
            )
    
        # resolve colorbar spec (only meaningful when we have Z)
        cmap_r, norm_r, ticks_r, label_r, _bounds_r = self._uq__resolve_colorbar_spec(
            imt=imt_u,
            what=what_u,
            mean_scale_type=mean_scale_type,
        ...
        fig, ax, used_cartopy, ccrs, zax = self._uq__cartopy_axes(
            figsize=figsize,
            extent=extent,
            use_utm=bool(use_utm),
            zorders=zdef,
        ...
        # plot field if available
        pm = None
        if can_plot_field:
            if used_cartopy:
                pm = ax.pcolormesh(lon2d, lat2d, Z, transform=ccrs.PlateCarree(), cmap=cmap_r, norm=norm_r, zorder=zax["field"])
            else:
                pm = ax.pcolormesh(lon2d, lat2d, Z, cmap=cmap_r, norm=norm_r)
    
            # contours (optional if you want them for mean)
            try:
                if what_u == "mean" and ticks_r is not None and len(ticks_r) > 2:
                    ax.contour(lon2d, lat2d, Z, levels=ticks_r, linewidths=0.5,
                               transform=(ccrs.PlateCarree() if used_cartopy else None),
                               zorder=zax["contours"])
            except Exception:
                pass
    
            # colorbar
            if colorbar and pm is not None:
                cb = self._uq__apply_colorbar(
                    fig, ax, pm,
                    ticks=ticks_r,
                    label=(colorbar_label or label_r),
                    shrink=float(colorbar_shrink),
                    label_fs=float(label_fs),
                    tick_fs=float(tick_fs),
                    colorbar_kwargs=(colorbar_kwargs or {}),
                    zorder=zax["colorbar"],
                )
    
        # if missing/fail, annotate explicitly (no NaN fabrication)
        if not can_plot_field:
            reason = None
            if isinstance(meta, dict):
                reason = meta.get("reason") or meta.get("error")
            msg = f"{status_s.upper()}"
            if reason:
                msg += f"\n{reason}"
            try:
                ax.text(
        ...
        # overlays: rupture + obs (still useful even if field missing)
        if show_rupture:
            try:
                self._uq__overlay_rupture(ax, version=vkey, used_cartopy=used_cartopy, ccrs=ccrs,
                                         zorder=zax["rupture"], rupture_kwargs=rupture_kwargs)
            except Exception:
                pass
    
        if show_obs:
            try:
                self._uq__overlay_observations(ax, version=vkey, imt=imt_u, used_cartopy=used_cartopy, ccrs=ccrs,
                                               zorder=zax["obs"], obs_size=float(obs_size), obs_kwargs=obs_kwargs)
            except Exception:
                pass
    
        # counts box
        if counts_box:
            self._uq__plot_counts_box(ax, version=vkey, zorder=zax["text"], fontsize=float(counts_box_fs))
    
        # title
        if show_title:
            ttl = title or f"{method_u} v{vkey} {imt_u} {what_u}"
            try:
                ax.set_title(ttl, fontsize=float(title_fs))
            except Exception:
                pass
        ...
        key = None
        try:
            # your existing key resolution helper (if present)
            key = (meta or {}).get("key", None) if isinstance(meta, dict) else None
        except Exception:
            key = None
    
        fname_stem = self._uq__fname_stem(method_u, what_u, imt_u, vkey, key=key, tag=None)
        ...
        if export_grid_csv and can_plot_field:
            grid_csv_path = self._uq__export_grid_csv(
                lon2d=lon2d, lat2d=lat2d, Z2d=Z,
                fname_stem=fname_stem,
                output_path=output_path,
    ...
    def uq_plot_method_map(
        ...
        *,
        ...
        # zorder normalization (uses your new helpers if you added them; otherwise fallback)
        if hasattr(self, "_uq__merge_zorders"):
            zdef = self._uq__merge_zorders(zorders)
        else:
            zdef = zorders or {}
    
        # retrieve field (may compute/store if requested)
        Z, meta, status, _mu2d, _sig2d = self._uq__get_field_for_plot(
            version=vkey,
            imt=imt_u,
            method=method_u,
        ...
        # ✅ IMPORTANT: match your real _uq__resolve_colorbar_spec signature
        # (no vmin/vmax arguments here; those are applied in pcolormesh)
        cmap_r, norm_r, ticks_r, label_r, _bounds_r = self._uq__resolve_colorbar_spec(
            imt=imt_u,
            what=what_u,
            mean_scale_type=mean_scale_type,
        ...
        if can_plot_field:
            pm = ax.pcolormesh(
                ...
                cmap=cmap_r,
                norm=norm_r,
                vmin=vmin,           # ✅ apply here
                vmax=vmax,           # ✅ apply here
                shading="auto",
                zorder=zax.get("field", 2) if isinstance(zax, dict) else 2,
            )
            ...
            if station_radius_km and float(station_radius_km) > 0:
                ...
                try:
                    lats, lons, _w, _df = self._uq__collect_version_obs_xy(vkey, imt_u)
                    if np.asarray(lats).size > 0:
                        m = self._uq_mask_within_km_of_points(lat2d, lon2d, lats, lons, float(station_radius_km))
                        ax.contour(
                            lon2d, lat2d, np.asarray(m, float),
                            levels=[0.5],
                            transform=tr,
                            zorder=zax.get("contours", 25) if isinstance(zax, dict) else 25,
                            **station_radius_kwargs,
                        )
                except Exception:
                    pass
    
        # if missing/fail, annotate explicitly (no NaNs invented)
        if not can_plot_field:
            reason = None
            if isinstance(meta, dict):
                reason = meta.get("reason") or meta.get("error")
            ...
            ax.text(
                ...
                fontsize=float(title_fs),
                bbox=dict(boxstyle="round,pad=0.5", alpha=0.85),
                zorder=zax.get("text", 40) if isinstance(zax, dict) else 40,
            )
    
        # overlays (works even if field missing)
        self._uq__overlay_obs_and_rupture(
            ax,
            version=vkey,
            imt=imt_u,
            used_cartopy=used_cartopy,
            ccrs=ccrs,
            zdef=zax if isinstance(zax, dict) else zdef,
            show_obs=bool(show_obs),
            obs_size=float(obs_size),
            obs_kwargs=obs_kwargs,
            show_rupture=bool(show_rupture),
        ...
        if counts_box:
            self._uq__plot_counts_box(ax, version=vkey, zorder=(zax.get("text", 40) if isinstance(zax, dict) else 40),
                                      fontsize=counts_box_fs)
    
        # colorbar only if we have a plotted mappable
        if colorbar and (pm is not None):
            self._uq__apply_colorbar(
                fig, ax, pm,
                ticks=ticks_r,
                ...
                tick_fs=tick_fs,
                colorbar_kwargs=colorbar_kwargs,
                zorder=(zax.get("colorbar", None) if isinstance(zax, dict) else None),
            )
        ...
        saved_paths = {}
        if save and output_path:
            # uses your existing bundler if present; otherwise your simple saver
            try:
                self._uq__save_figure_bundle(
                    fig=fig,
                    output_path=output_path,
                    base_name=f"map__v{vkey}__{imt_u}__{method_u}__{what_u}".replace(" ", ""),
                    save_formats=save_formats,
                    dpi=dpi,
                )
                saved_paths = {"bundle": True}
            except Exception:
                # fallback: save png only
                try:
                    from pathlib import Path
                    outdir = Path(output_path) / "figures"
                    outdir.mkdir(parents=True, exist_ok=True)
                    p = outdir / f"map__v{vkey}__{imt_u}__{method_u}__{what_u}.png"
                    fig.savefig(p, dpi=dpi, bbox_inches="tight")
                    saved_paths = {"png": str(p)}
                except Exception:
                    saved_paths = {}
    ...
    def uq_plot_version_differences(
        ...
        **diff_kwargs,
        ...
        SAFETY:
        - Avoid kwarg collisions if caller passes save/show/save_formats inside diff_kwargs.
        """
        # ---- prevent duplicate kwargs coming from **diff_kwargs ----
        diff_kwargs = dict(diff_kwargs or {})
        diff_kwargs.pop("save", None)
        diff_kwargs.pop("show", None)
        diff_kwargs.pop("save_formats", None)
        diff_kwargs.pop("output_path", None)
    ...
    def uq_plot_version_differences(
        ...
        **diff_kwargs,
        ...
        IMPORTANT:
          Does NOT require uq_state['version_list'].
          Uses uq_state['unified']['version_keys'] or falls back to uq_state['versions'] keys.
        """
    
        # avoid collisions from **diff_kwargs
        diff_kwargs = dict(diff_kwargs or {})
        for k in ("save", "show", "save_formats", "output_path", "fail_mode"):
            diff_kwargs.pop(k, None)
    
        if not getattr(self, "uq_state", None):
            raise RuntimeError("uq_state missing. Run uq_build_dataset() first.")
    
        # canonical version list
        u = (self.uq_state.get("unified") or {})
        vlist = u.get("version_keys", None)
        if not vlist:
            vmap = (self.uq_state.get("versions") or {})
            if isinstance(vmap, dict) and vmap:
                try:
                    vlist = sorted(vmap.keys(), key=lambda x: int(str(x)))
                except Exception:
                    vlist = sorted(vmap.keys())
            else:
                vlist = []
    
        if len(vlist) < 2:
            raise RuntimeError(f"Need at least 2 versions for differences; found {len(vlist)}.")
    ...
    def uq_plot_difference_map(
        ...
        *,
        ...
        # NEW: same failure behavior as your map audit
        fail_mode: str = "raise",   # "raise" | "annotate"
        ...
        # NEW: accept what DIFF_PLOT_KWARGS sends
        colorbar_shrink: float = 0.85,
        colorbar_kwargs: dict = None,
        colorbar_label: str = "Δ",
        ...
        # -------------------------------
        # canonical version list (FIX: no uq_state['version_list'])
        # -------------------------------
        u = (self.uq_state.get("unified") or {})
        vlist = u.get("version_keys", None)
        if not vlist:
        ...
        ok_from = (Zfrom is not None) and (str(status_f).lower().strip() == "ok")
        ok_to = (Zto is not None) and (str(status_t).lower().strip() == "ok")
    
        if not (ok_from and ok_to):
            msg = f"Difference fields missing (from/to). status_from={status_f}, status_to={status_t}"
            if fail_mode_s == "raise":
                raise RuntimeError(msg)
    
            # annotate-only: create a figure with message (no silent NaNs)
            lon2d, lat2d = self._uq__get_unified_grid_safe()
            extent = self._get_grid_extent(v_to_k, grid_mode="unified", margin_deg=0.0)
            fig, ax, used_cartopy, ccrs, zdef = self._uq__cartopy_axes(
                figsize=figsize,
            ...
            rec = dict(
                event_id=getattr(self, "event_id", "event"),
                imt=imt_u, what=what_u, method=method_u,
                v_from=v_from_k, v_to=v_to_k,
                status="missing",
                status_from=str(status_f),
                status_to=str(status_t),
            )
            return fig, ax, rec
        ...
        if vmin is None or vmax is None:
            try:
                vmin_, vmax_ = self._uq__autosym_limits(d, pct=99.0)
            except Exception:
                vmin_, vmax_ = (-1.0, 1.0)
            if vmin is None:
                vmin = vmin_
        ...
        tr = ccrs.PlateCarree() if (used_cartopy and ccrs is not None) else None
        pm_kw = dict(cmap=cmap, shading="auto", vmin=float(vmin), vmax=float(vmax), zorder=zdef.get("field", 2))
        if tr is not None:
            pm_kw["transform"] = tr
        pm = ax.pcolormesh(lon2d, lat2d, np.asarray(d, float), **pm_kw)
        ...
        if title is None:
            title = f"Δ({method_u} {what_u}) — {imt_u} — v{v_to_k} − v{v_from_k}"
        ax.set_title(str(title), zorder=zdef.get("text", 40))
    
        # colorbar (NOW uses colorbar_shrink / colorbar_kwargs)
        try:
            cbk = dict(colorbar_kwargs or {})
            cb = plt.colorbar(pm, ax=ax, shrink=float(colorbar_shrink), **cbk)
        ...
        if save and output_path:
            try:
                self._uq__save_figure_simple(
                    fig,
                    fname_stem=f"diff__{method_u}__{what_u}__{imt_u}__v{v_from_k}_to_v{v_to_k}".replace(" ", ""),
                    subdir="uq_plots/uq_diffs",
                    output_path=output_path,
                    save_formats=save_formats,
                    dpi=dpi,
                )
    ...
    def uq_plot_version_differences(
        ...
        **diff_kwargs,
        ...
        NOTE: Uses uq_state['unified']['version_keys'] (preferred) or uq_state['versions'] keys.
        """
    
        # Prevent collisions from caller passing save/show/etc inside **diff_kwargs
        diff_kwargs = dict(diff_kwargs or {})
        for k in ("save", "save_formats", "output_path", "show", "fail_mode"):
            diff_kwargs.pop(k, None)
...
#########################################
    ...
    def cdi_filtering_diagnostic_5sets(
        ...
        profiles_5sets=None,
        debug=False,
        keep_obs_provenance=False,
        drop_reason_priority=None,
    ):
        """
        Extended CDI diagnostic.
    
        New:
        - keep_obs_provenance: retain all points + filter provenance
        - obs_table per set with kept/dropped + reasons
        """
    
        diag = {}
    
        # ---- existing logic (unchanged) ----
        # This part assumes you already compute:
        #   obs_used[set_name]
        #   residuals[set_name]
        #   metrics table
        #
        # DO NOT change your internal filtering logic here.
        # Just capture provenance at each filter stage.
    
        # ------------------------------------------------
        # NEW: provenance collection scaffold
        # ------------------------------------------------
        obs_table = {}
    
        for set_name, profile in profiles_5sets.items():
    
            # full candidate observations BEFORE filtering
            df_all = profile["obs_all"].copy()  # <-- you already have this internally
    
            df = df_all.copy()
            df["kept"] = True
            df["drop_reasons"] = [[] for _ in range(len(df))]
    
            # ---- example filter hooks (adapt names to your code) ----
            def _mark_dropped(mask, reason):
                idx = df.index[mask]
                df.loc[idx, "kept"] = False
                for i in idx:
                    df.at[i, "drop_reasons"].append(reason)
    
            # Examples (replace masks with your real masks)
            if "mask_trust_only" in profile:
                _mark_dropped(profile["mask_trust_only"], "trust_only")
    
            if "mask_outlier" in profile:
                _mark_dropped(profile["mask_outlier"], "outlier")
    
            if "mask_min_neighbors" in profile:
                _mark_dropped(profile["mask_min_neighbors"], "min_neighbors")
    
            if "mask_grid_bin" in profile:
                _mark_dropped(profile["mask_grid_bin"], "grid_bin")
    
            if "mask_quantile" in profile:
                _mark_dropped(profile["mask_quantile"], "quantile")
    
            # ---- choose primary reason (priority rule) ----
            if drop_reason_priority is None:
            ...
            def _primary_reason(reasons):
                for r in drop_reason_priority:
                    if r in reasons:
                        return r
                return None
    
            df["drop_reason_primary"] = df["drop_reasons"].apply(_primary_reason)
            df["drop_reason_primary"] = df["drop_reason_primary"].fillna("kept")
    
            # residuals (NaN for dropped)
            if set_name in profile.get("residuals", {}):
                df["residual"] = profile["residuals"].get(set_name)
            else:
                df["residual"] = None
    
            obs_table[set_name] = df
    
        # ---- attach to diagnostic output ----
        if keep_obs_provenance:
            diag["obs_table"] = obs_table
    
        # ---- existing outputs (unchanged) ----
        diag["results_df"] = profile["results_df"]
        diag["best_set"] = profile.get("best_set")
        diag["obs_used"] = profile.get("obs_used")
        diag["residuals"] = profile.get("residuals")
    ...
    def _cdi_condition(
        ...
        *,
        ...
        cdi_grid_agg="median",
        cdi_quantile=(0.05, 0.95),
        cdi_quantile_field="residual",
        cdi_use_prior_residual=True,
        # sponheuer knobs (already supported in your file)
        cdi_enable_sponheuer=False,
        cdi_spon_thr_sigma=2.5,
        cdi_spon_min_r_km=1.0,
        cdi_spon_robust=True,
        ...
        provenance_reason_priority: tuple = ("extent", "local_outlier", "grid_thin", "quantile", "sponheuer", "clustering"),
        debug=False,
        # IMPORTANT: your existing code already expects `version` sometimes.
        version=None,
    ):
        """
        Backward compatible:
          - default return is DataFrame (same as before)
          - if return_provenance=True -> returns (df_kept, provenance_dict)

        Provenance output includes:
          - obs_all: all input rows with _obs_id
          - obs_kept: final kept rows
          - obs_dropped: dropped rows
          - drop_reason: dict obs_id -> reason (first reason applied)
        """
        import numpy as np
        import pandas as pd
        ...
        if not isinstance(obs_df, pd.DataFrame) or obs_df.empty:
            ...
            if return_provenance:
                return obs_df, {"obs_all": obs_df, "obs_kept": obs_df, "obs_dropped": obs_df.iloc[0:0], "drop_reason": {}}
            return obs_df

        df0 = obs_df.copy().reset_index(drop=True)

        # Ensure stable unique ids
        if "_obs_id" not in df0.columns:
            df0["_obs_id"] = np.arange(len(df0), dtype=int)

        # --- start provenance bookkeeping ---
        drop_reason = {}  # obs_id -> first reason
        def _mark_dropped(ids, reason):
            if ids is None:
                return
            for _id in ids:
                if _id not in drop_reason:
                    drop_reason[_id] = reason

        # Ensure columns exist
        if "nresp" not in df0.columns:
            df0["nresp"] = np.nan

        # trusted flag
        nresp = pd.to_numeric(df0["nresp"], errors="coerce")
        df0["_trusted"] = (nresp >= float(trust_nresp_ge)).fillna(False)

        # optional residual to prior
        if bool(cdi_use_prior_residual):
            vkey = _norm_version(version) if version is not None else None
            try:
                lon2d, lat2d, prior = self._get_prior_mean_unified(vkey, "MMI")
            ...
            if prior is not None:
                lon_vals = lon2d[0, :]
                lat_vals = lat2d[:, 0]
                lon = df0["lon"].to_numpy(dtype=float)
                lat = df0["lat"].to_numpy(dtype=float)
                ix = np.clip(np.searchsorted(lon_vals, lon) - 1, 0, lon_vals.size - 1)
                iy = np.clip(np.searchsorted(lat_vals, lat) - 1, 0, lat_vals.size - 1)
                prior_at = prior[iy, ix]
                df0["residual"] = df0["value"].to_numpy(dtype=float) - prior_at
            else:
                df0["residual"] = np.nan
        else:
            df0["residual"] = np.nan
        ...
        if "local_outlier" in strat and enable_local_outlier and len(df) >= (int(min_neighbors) + 1):
            ...
            dropped_ids = df.loc[~keep, "_obs_id"].to_numpy(dtype=int)
            _mark_dropped(dropped_ids, "local_outlier")
        ...
        # --- grid thinning ---
        if "grid_thin" in strat and len(df) > 0:
            before_ids = set(df["_obs_id"].to_numpy(dtype=int).tolist())
            df_thin = self._cdi_thin_grid(df, bin_km=float(cdi_grid_bin_km), agg=str(cdi_grid_agg), min_pts=1, debug=debug)
            after_ids = set(df_thin["_obs_id"].to_numpy(dtype=int).tolist()) if (df_thin is not None and len(df_thin) > 0 and "_obs_id" in df_thin.columns) else set()
            dropped = sorted(list(before_ids - after_ids))
            _mark_dropped(dropped, "grid_thin")
            df = df_thin

        # --- quantile trims ---
        def _apply_quantile(field_name: str, reason_name: str):
            nonlocal df
            if len(df) <= 0:
                return
            before_ids = set(df["_obs_id"].to_numpy(dtype=int).tolist())
            ql, qh = cdi_quantile
            df2 = self._cdi_quantile_trim(df, q_low=float(ql), q_high=float(qh), field=field_name, debug=debug)
            after_ids = set(df2["_obs_id"].to_numpy(dtype=int).tolist()) if (df2 is not None and len(df2) > 0 and "_obs_id" in df2.columns) else set()
            dropped = sorted(list(before_ids - after_ids))
            _mark_dropped(dropped, reason_name)
            df = df2

        if "quantile_value" in strat and len(df) > 0:
            _apply_quantile("value", "quantile")

        if "quantile_residual" in strat and len(df) > 0:
            arr = df["residual"].to_numpy(dtype=float) if "residual" in df.columns else np.array([])
            field = "residual" if (arr.size > 0 and np.isfinite(arr).any()) else "value"
            _apply_quantile(field, "quantile")
        ...
        if (("sponheuer" in strat) or bool(cdi_enable_sponheuer)) and len(df) > 0:
            ...
            df2 = self._cdi_sponheuer_filter(
                df,
                event_lat=event_lat,
                event_lon=event_lon,
                thr_sigma=float(cdi_spon_thr_sigma),
            ...
            after_ids = set(df2["_obs_id"].to_numpy(dtype=int).tolist()) if (df2 is not None and len(df2) > 0 and "_obs_id" in df2.columns) else set()
            dropped = sorted(list(before_ids - after_ids))
            _mark_dropped(dropped, "sponheuer")
            df = df2

        # --- clustering (merges dense bins; mark non-representatives as dropped) ---
        if enable_clustering and ("cluster" in strat or "clustering" in strat or ("grid_thin" not in strat)) and len(df) >= int(cluster_min_pts):
            import pandas as pd
            ...
            for _, g in df.groupby("_bin"):
                ...
                # representative: the first row (existing behavior)
                rep = g.iloc[[0]].copy()
                out_rows.append(rep)

                # everything else is "merged away"
                dropped_ids = g.iloc[1:]["_obs_id"].to_numpy(dtype=int)
                _mark_dropped(dropped_ids, "clustering")
        ...
        # provenance tables
        kept_ids = set(df["_obs_id"].to_numpy(dtype=int).tolist()) if ("_obs_id" in df.columns) else set()
        all_ids = df0["_obs_id"].to_numpy(dtype=int).tolist()
        dropped_ids = [i for i in all_ids if i not in kept_ids]

        prov = {
            "obs_all": df0,
            "obs_kept": df.copy(),
            "obs_dropped": df0.loc[df0["_obs_id"].isin(dropped_ids)].copy(),
            "drop_reason": drop_reason,
            "reason_priority": tuple(provenance_reason_priority),
        }
        return df, prov
    ...
    def cdi_filtering_diagnostic_5sets(
        ...
        *,
        ...
        debug: bool = False,
        # NEW:
        keep_obs_provenance: bool = True,
        drop_reason_priority: tuple = ("extent", "local_outlier", "grid_thin", "quantile", "sponheuer", "clustering"),
    ):
        """
        Robust diagnostic:
          - builds raw CDI obs internally (no dependency on profiles["obs_all"])
          - optionally stores provenance per set for mapping
        """
        import numpy as np
        import pandas as pd
        ...
        # prior version = first available (same philosophy you use elsewhere)
        try:
            v0 = sorted(list((self.uq_state.get("versions") or {}).keys()))[0]
        except Exception:
            v0 = vkey

        # ---- build RAW candidate CDI ----
        obs_raw = self.build_observations(version=vkey, imt="MMI", dyfi_source="cdi", sigma_override=None)
        n_raw = int(len(obs_raw)) if hasattr(obs_raw, "__len__") else 0

        # ---- extent filter once (shared) ----
        obs_ext, obs_drop = self.filter_observations_to_extent(
            obs_raw, version=vkey, grid_mode="unified", margin_deg=float(margin_deg), return_dropped=True
        )
        n_ext = int(len(obs_ext)) if hasattr(obs_ext, "__len__") else 0

        # tag extent-dropped with ids for provenance
        if keep_obs_provenance:
            import numpy as np
            if "_obs_id" not in obs_ext.columns:
                obs_ext = obs_ext.copy().reset_index(drop=True)
                obs_ext["_obs_id"] = np.arange(len(obs_ext), dtype=int)
            if obs_drop is not None and (not getattr(obs_drop, "empty", True)):
                obs_drop = obs_drop.copy().reset_index(drop=True)
                if "_obs_id" not in obs_drop.columns:
                    # keep separate id-space for extent drop (won't collide with kept ids)
                    obs_drop["_obs_id"] = -1 - np.arange(len(obs_drop), dtype=int)

        # reference grid (later shakemap) for scoring
        lon2d, lat2d, ref_mmi = self._get_prior_mean_unified(ref_v, "MMI")
        lon_vals = lon2d[0, :]
        lat_vals = lat2d[:, 0]
        ...
        for set_name, profile in profiles_5sets.items():
            # condition CDI
            if keep_obs_provenance:
                df_used, prov = self._cdi_condition(
                    obs_ext,
                    version=v0,  # compare residual against fixed prior
                    return_provenance=True,
                    provenance_reason_priority=drop_reason_priority,
                    debug=debug,
                    **profile
                )

                # add extent dropped as separate bucket
                if obs_drop is not None and (not getattr(obs_drop, "empty", True)):
                    prov["extent_dropped"] = obs_drop.copy()
            else:
                df_used = self._cdi_condition(obs_ext, version=v0, debug=debug, **profile)
                prov = None

            ok = isinstance(df_used, pd.DataFrame) and (not df_used.empty)
            obs_used[set_name] = df_used.copy()
            ...
            if not ok:
                rows.append(dict(
                    ...
                    bias_ref=np.nan,
                    std_ref=np.nan,
                    note="empty after conditioning",
                ))
                residuals[set_name] = pd.DataFrame()
                continue

            ref_at = sample_ref(df_used)
            res = df_used["value"].to_numpy(dtype=float) - ref_at
            ...
            df_res = df_used.copy()
            df_res["ref_mmi"] = ref_at
            df_res["residual"] = res
            if keep_obs_provenance and prov is not None:
                df_res.attrs["provenance"] = prov

            residuals[set_name] = df_res

            rows.append(dict(
                set_name=set_name,
                ok=True,
                n_kept=int(len(df_used)),
                retained_frac=float(len(df_used) / max(n_ext, 1)),
                rmse_ref=rmse,
                mae_ref=mae,
                bias_ref=bias,
                std_ref=std,
        ...
        return {
            ...
            "meta": {
                ...
                "n_extent": n_ext,
                "n_dropped_extent": int(len(obs_drop)) if obs_drop is not None else None,
                "keep_obs_provenance": bool(keep_obs_provenance),
            },
        }
    ...
    def uq_plot_cdi_rmse_vs_nkept(
        ...
        *,
        ...
        label_mode: str = "legend",   # "legend" | "annotate" | "both" | "none"
        legend: bool = None,          # if None -> inferred from label_mode
        legend_loc: str = "best",
        legend_outside: bool = False,
        legend_bbox_to_anchor=(1.02, 1.0),
        legend_ncol: int = 1,

        annotate: bool = None,        # if None -> inferred from label_mode
        annotate_kwargs: dict = None,
        ...
        if font_sizes is None:
            font_sizes = {"title": 12, "label": 12, "tick": 10, "legend": 10, "annot": 10}
        if grid_kwargs is None:
            grid_kwargs = {"alpha": 0.25, "linewidth": 0.7}

        mode = str(label_mode).lower().strip()
        if legend is None:
            legend = (mode in ("legend", "both"))
        if annotate is None:
            annotate = (mode in ("annotate", "both"))

        df = diag.get("results_df", None)
        if df is None or df.empty:
            raise ValueError("diag['results_df'] is empty; nothing to plot.")

        dfp = df.copy()
        dfp = dfp[dfp["ok"] == True].copy()
        if dfp.empty:
            raise ValueError("No ok=True CDI sets to plot.")

        fig, ax = plt.subplots(figsize=figsize, dpi=dpi)

        # scatter per set to make legend meaningful
        for _, r in dfp.iterrows():
            name = str(r["set_name"])
            x = float(r[x_key])
            y = float(r[y_key])
            ax.scatter([x], [y], s=120, label=name, zorder=3)

            if annotate:
                akw = dict(ha="left", va="bottom", xytext=(3, 3), textcoords="offset points")
                if annotate_kwargs:
                    akw.update(annotate_kwargs)
                ax.annotate(name, (x, y), fontsize=font_sizes.get("annot", 10), **akw)

        ax.set_xlabel(x_key, fontsize=font_sizes.get("label", 12))
        ax.set_ylabel(y_key, fontsize=font_sizes.get("label", 12))
        ax.tick_params(axis="both", labelsize=font_sizes.get("tick", 10))
        ...
        if legend:
            if legend_outside:
                ...
                ax.legend(loc=legend_loc, frameon=True, fontsize=font_sizes.get("legend", 10), ncol=int(legend_ncol))
    ...
    def uq_plot_cdi_residual_hist(
        ...
        *,
        sets=None,
        mode: str = "overlay",   # "overlay" | "small_multiples" | "kde_overlay" | "kde_small_multiples"
        bins: int = 40,
        density: bool = False,
        figsize=(11, 4.5),
        font_sizes: dict = None,
        ...
        dpi: int = 300,
        # NEW: KDE knobs
        kde_bw: float = None,        # None -> scipy default
        kde_points: int = 250,
        kde_alpha: float = 0.95,
    ):
        import numpy as np
        import matplotlib.pyplot as plt
        from pathlib import Path
        ...
        if font_sizes is None:
            font_sizes = {"title": 12, "label": 12, "tick": 10, "legend": 10}
        if grid_kwargs is None:
            grid_kwargs = {"alpha": 0.25, "linewidth": 0.7}
        if legend_kwargs is None:
            legend_kwargs = {"frameon": True, "fontsize": font_sizes.get("legend", 10)}

        residuals = diag.get("residuals", {}) or {}
        if not residuals:
            raise ValueError("diag['residuals'] is empty; run cdi_filtering_diagnostic_5sets first.")

        if sets is None:
            sets = list(residuals.keys())

        mode_s = str(mode).lower().strip()
        use_kde = mode_s.startswith("kde")

        # collect data
        data = {}
        for sname in sets:
            df = residuals.get(sname, None)
            if df is None or getattr(df, "empty", True):
                continue
            if "residual" not in df.columns:
                continue
            arr = np.asarray(df["residual"], float)
            arr = arr[np.isfinite(arr)]
            if arr.size:
                data[sname] = arr

        if not data:
            raise ValueError("No residual arrays to plot.")

        def _plot_kde(ax, arr, label):
            try:
                from scipy.stats import gaussian_kde
            except Exception:
                # fallback: step histogram
                ax.hist(arr, bins=bins, density=True, histtype="step", linewidth=1.5, label=label, alpha=kde_alpha)
                return
            kde = gaussian_kde(arr, bw_method=kde_bw)
            xs = np.linspace(np.min(arr), np.max(arr), int(kde_points))
            ys = kde(xs)
            ax.plot(xs, ys, linewidth=2.0, label=label, alpha=kde_alpha)

        if mode_s in ("overlay", "kde_overlay"):
            fig, ax = plt.subplots(figsize=figsize, dpi=dpi)

            for sname, arr in data.items():
                if use_kde:
                    _plot_kde(ax, arr, sname)
                else:
                    ax.hist(arr, bins=bins, density=density, alpha=0.45, label=sname)

            ax.set_xlabel("CDI − Reference MMI", fontsize=font_sizes.get("label", 12))
            ax.set_ylabel("Density" if (density or use_kde) else "Count", fontsize=font_sizes.get("label", 12))
            ax.tick_params(axis="both", labelsize=font_sizes.get("tick", 10))
            ...
            if show_title:
                ax.set_title(title or "Residuals distribution", fontsize=font_sizes.get("title", 12))

            fig.tight_layout()

        elif mode_s in ("small_multiples", "kde_small_multiples"):
            # grid layout
            ...
            fig, axes = plt.subplots(nrow, ncol, figsize=figsize, dpi=dpi, squeeze=False)
            for i, sname in enumerate(names):
                ...
                if use_kde:
                    ...
                    ax.hist(arr, bins=bins, density=density, alpha=0.70)

                ax.set_title(sname, fontsize=font_sizes.get("title", 11))
                ax.tick_params(axis="both", labelsize=font_sizes.get("tick", 9))
                if grid:
                    ax.grid(True, **grid_kwargs)
                if xlim is not None:
                    ax.set_xlim(xlim)
            ...
            if show_title:
                fig.suptitle(title or "Residuals distribution (small multiples)", fontsize=font_sizes.get("title", 12))
    ...
    def uq_plot_cdi_residual_map(
        ...
        *,
        ...
        # colorbar
        show_colorbar: bool = True,
        cbar_label: str = "Residual (CDI − Reference)",
        cbar_label_kwargs: dict = None,
        cbar_tick_params: dict = None,
        ...
        # NEW: zorder control
        zorder_land: int = 1,
        zorder_shakemap: int = 2,
        zorder_ocean: int = 3,
        zorder_points: int = 10,
        zorder_kept: int = 11,
        zorder_dropped: int = 12,

        # NEW: show filtered points
        show_all_obs: bool = False,
        show_dropped: bool = True,
        dropped_edge_by_reason: dict = None,
        dropped_edge_default: str = "red",
        kept_edge: str = "black",
        kept_linewidth: float = 0.6,
        dropped_linewidth: float = 0.8,
        reason_legend: bool = True,
        reason_legend_outside: bool = True,
        reason_legend_bbox=(1.02, 0.02),
        reason_legend_loc="lower left",
        ...
        # pick set
        if set_name is None:
            set_name = diag.get("best_set", None)
        if set_name is None:
            raise ValueError("set_name not provided and diag['best_set'] is None.")

        res = (diag.get("residuals") or {}).get(set_name, None)
        if res is None or getattr(res, "empty", True):
            raise ValueError(f"No residual table for set_name={set_name!r}")

        # provenance (optional)
        prov = getattr(res, "attrs", {}).get("provenance", None)

        # color scaling
        vals = np.asarray(res["residual"], float)
        vals = vals[np.isfinite(vals)]
        if vals.size == 0:
            raise ValueError("Residual array is empty or non-finite.")

        if robust:
            lo, hi = np.nanpercentile(vals, robust_q)
            vmin0, vmax0 = float(lo), float(hi)
        else:
            vmin0, vmax0 = float(np.nanmin(vals)), float(np.nanmax(vals))

        if symmetric:
            V = max(abs(vmin0 - center), abs(vmax0 - center))
            vmin_use, vmax_use = center - V, center + V
        else:
            vmin_use, vmax_use = vmin0, vmax0
        ...
        if use_cartopy:
            import cartopy.crs as ccrs
            proj = ccrs.PlateCarree() if str(projection).lower().strip() == "platecarree" else ccrs.PlateCarree()
            fig = plt.figure(figsize=figsize, dpi=dpi)
        ...
        # extent
        lon = np.asarray(res["lon"], float)
        lat = np.asarray(res["lat"], float)
        lon0, lon1 = float(np.nanmin(lon)), float(np.nanmax(lon))
        lat0, lat1 = float(np.nanmin(lat)), float(np.nanmax(lat))
        ext = (lon0 - extent_pad_deg, lon1 + extent_pad_deg, lat0 - extent_pad_deg, lat1 + extent_pad_deg)

        if use_cartopy:
            ax.set_extent(ext, crs=proj)

        # basemap layers
        if coastlines and use_cartopy:
            coastlines_kwargs = coastlines_kwargs or dict(linewidth=0.6, alpha=0.7, zorder=5)
            coastlines_kwargs.setdefault("zorder", 5)
            ax.coastlines(**coastlines_kwargs)

        if borders and use_cartopy:
            import cartopy.feature as cfeature
            borders_kwargs = borders_kwargs or dict(linewidth=0.5, alpha=0.6, zorder=6)
            borders_kwargs.setdefault("zorder", 6)
            ax.add_feature(cfeature.BORDERS, **borders_kwargs)

        if land and use_cartopy:
            import cartopy.feature as cfeature
            land_kwargs = land_kwargs or dict(alpha=0.15)
            ax.add_feature(cfeature.LAND, zorder=zorder_land, **land_kwargs)

        # ShakeMap background (optional)
        if show_shakemap:
            # relies on your existing helpers inside SHAKEuq (already in your pipeline)
            try:
                Z, lon2d, lat2d = self.extract_raw_shakemap(
                    imt=str(shakemap_imt).upper(),
                    version=(diag.get("meta", {}) or {}).get("reference_version") if shakemap_source == "reference" else (diag.get("meta", {}) or {}).get("version"),
                )
                # fallback: if user passed explicit version string
                if isinstance(shakemap_source, str) and shakemap_source not in ("reference", "target"):
                    Z, lon2d, lat2d = self.extract_raw_shakemap(imt=str(shakemap_imt).upper(), version=str(shakemap_source))
                ax.pcolormesh(
                    lon2d, lat2d, Z,
                    transform=proj if use_cartopy else None,
                    shading=shakemap_shading,
                    alpha=float(shakemap_alpha),
                    rasterized=bool(shakemap_rasterized),
                    zorder=zorder_shakemap,
                )
            except Exception:
                pass

        # ocean ON TOP of shakemap (your request)
        if ocean and use_cartopy:
            import cartopy.feature as cfeature
            ocean_kwargs = ocean_kwargs or dict(alpha=0.25)
            ax.add_feature(cfeature.OCEAN, zorder=zorder_ocean, **ocean_kwargs)

        # gridlines
        if gridlines and use_cartopy:
            gridlines_kwargs = gridlines_kwargs or dict(draw_labels=True, linewidth=0.5, alpha=0.3, zorder=999)
            gl = ax.gridlines(crs=proj, **gridlines_kwargs)
            gl.top_labels = False
            gl.right_labels = False
        ...
        # --- overlay: kept vs dropped by reason ---
        reason_handles = []
        if show_all_obs and prov is not None:
            obs_all = prov.get("obs_all", None)
            drop_reason = prov.get("drop_reason", {}) or {}
            extent_dropped = prov.get("extent_dropped", None)

            if dropped_edge_by_reason is None:
                dropped_edge_by_reason = {
                    "local_outlier": "red",
                    "grid_thin": "orange",
                    "quantile": "purple",
                    "sponheuer": "brown",
            ...
            # build kept/dropped masks from ids
            kept_ids = set(prov.get("obs_kept", pd.DataFrame()).get("_obs_id", pd.Series([], dtype=int)).to_list())
            if obs_all is not None and (not getattr(obs_all, "empty", True)) and "_obs_id" in obs_all.columns:
                all_ids = obs_all["_obs_id"].to_numpy(dtype=int)
                dropped_ids = [i for i in all_ids if i not in kept_ids]

                # plot kept outlines
                df_kept = obs_all.loc[obs_all["_obs_id"].isin(list(kept_ids))].copy()
                if not df_kept.empty:
                    ax.scatter(
                        df_kept["lon"], df_kept["lat"],
                        s=float(s),
                        facecolors="none",
                        edgecolors=kept_edge,
                        linewidths=float(kept_linewidth),
                        transform=proj if use_cartopy else None,
                        zorder=zorder_kept,
                    )

                # plot dropped by reason (edge colors)
                if show_dropped and dropped_ids:
                    df_drop = obs_all.loc[obs_all["_obs_id"].isin(dropped_ids)].copy()
                    if not df_drop.empty:
                        # assign reason col
                        df_drop["_drop_reason"] = [drop_reason.get(int(i), "filtered") for i in df_drop["_obs_id"].to_numpy(dtype=int)]

                        for reason, g in df_drop.groupby("_drop_reason"):
                            ec = dropped_edge_by_reason.get(reason, dropped_edge_default)
                            ax.scatter(
                                g["lon"], g["lat"],
                                s=float(s),
                                facecolors="none",
                                edgecolors=ec,
                                linewidths=float(dropped_linewidth),
                                transform=proj if use_cartopy else None,
                                zorder=zorder_dropped,
                                label=f"dropped:{reason}",
                            )

                        if reason_legend:
                            # build a small legend from labeled groups
                            if reason_legend_outside:
                                ax.legend(loc=reason_legend_loc, bbox_to_anchor=reason_legend_bbox, frameon=True, fontsize=9)
                            else:
                                ax.legend(loc="best", frameon=True, fontsize=9)

            # extent dropped separately (if present)
            if show_dropped and extent_dropped is not None and (not getattr(extent_dropped, "empty", True)):
                ax.scatter(
                    extent_dropped["lon"], extent_dropped["lat"],
                    s=float(s),
                    facecolors="none",
                    edgecolors=dropped_edge_by_reason.get("extent", "gray"),
                    linewidths=float(dropped_linewidth),
                    transform=proj if use_cartopy else None,
                    zorder=zorder_dropped,
                    label="dropped:extent",
                )
        ...
        # titles
        if show_title:
            ax.set_title(title or f"CDI residual map — {set_name}", fontsize=11)
    ...
    def uq_plot_delta_to_shakemap(
        self,
        *,
        version,
        imt="MMI",
        what="sigma",                    # mean | sigma
        method="bayes",                  # bayes | rk | ens
        compute_if_missing=False,
        method_kwargs=None,
        # diff styling (mirrors uq_plot_difference_map)
        cmap="seismic",
        vmin=None,
        vmax=None,
        zorders=None,
        ...
        # fetch ShakeMap field
        Zs, _, st_s, *_ = self._uq__get_field_for_plot(version=vkey, imt=imt_u, method="ShakeMap", what=what_u)
        # fetch method field
        Zm, _, st_m, *_ = self._uq__get_field_for_plot(
            version=vkey, imt=imt_u, method=method_u, what=what_u,
            compute_if_missing=bool(compute_if_missing), method_kwargs=method_kwargs
        )
    
        if Zs is None or str(st_s).lower() != "ok":
            raise RuntimeError(f"ShakeMap baseline missing: status={st_s}")
        if Zm is None or str(st_m).lower() != "ok":
            raise RuntimeError(f"{method_u} field missing: status={st_m}")
    
        d = np.asarray(Zm, float) - np.asarray(Zs, float)
        ...
        pm = ax.pcolormesh(lon2d, lat2d, d, transform=(ccrs.PlateCarree() if used_cartopy else None),
                           cmap=cmap, vmin=vmin, vmax=vmax, zorder=zax.get("field", 2))
    
        if show_rupture:
            self._uq__plot_rupture(ax, version=vkey, zorder=zax.get("rupture", 20), rupture_kwargs=rupture_kwargs)
        if show_obs:
            self._uq__plot_obs(ax, version=vkey, imt=imt_u, zorder=zax.get("obs", 30), s=obs_size, obs_kwargs=obs_kwargs)
    
        # autosym if needed
        if vmin is None or vmax is None:
            vmin2, vmax2 = self._uq__autosym_limits(d, pct=99.0)
            if vmin is None: vmin = vmin2
            if vmax is None: vmax = vmax2
            pm.set_clim(vmin, vmax)
    
        self._uq__apply_colorbar(fig, ax, pm, label=f"Δ({method_u} − ShakeMap) {what_u}",
                                 shrink=0.85, pad=0.02, label_fs=11, tick_fs=10)
    
        if title is None:
            title = f"Δ({method_u} − ShakeMap) {what_u} — {imt_u} — v{vkey}"
        ax.set_title(title)
        ...
        if save and output_path:
            self._uq__save_figure_bundle(
                fig=fig,
                output_path=output_path,
                base_name=f"delta_to_shakemap__v{vkey}__{imt_u}__{method_u}__{what_u}",
                save_formats=save_formats,
                dpi=dpi,
            )
        ...
        rec = dict(version=vkey, imt=imt_u, method=method_u, what=what_u, status="ok")
        return fig, ax, rec
    ...
    # ==================================================================================================
    # SHAKEuq.py — DROP-IN PATCH (paste INSIDE class SHAKEuq)
    #
    # Goal:
    #   1) Prevent audits from dying when a requested field is missing (default fail_mode -> "annotate")
    #   2) Provide a robust "safe" wrapper that enforces fail_mode and filters kwargs
    #   3) Provide a state sanity checker you can call after uq_build_dataset()
    #
    # What you must ALSO change (one-line edit):
    #   - In uq_plot_method_map(...) signature: change default fail_mode from "raise" to "annotate"
    #
    # No external notebook helpers needed after this.
    # ==================================================================================================
    
    
    # --------------------------------------------------------------------------------------------------
    # (A) ONE-LINE EDIT — change default fail_mode in uq_plot_method_map signature
    # --------------------------------------------------------------------------------------------------
    # Find your method definition:
    #   def uq_plot_method_map(..., fail_mode="raise", ...):
    # Change it to:
    #   def uq_plot_method_map(..., fail_mode="annotate", ...):
    #
    ...
    # --------------------------------------------------------------------------------------------------
    # (B) DROP-IN METHODS — paste these inside class SHAKEuq (anywhere among public helpers)
    # --------------------------------------------------------------------------------------------------
    
    def uq_state_summary(self):
        """
        Small state summary for debugging/audits.
        Safe to call anytime.
        """
        st = getattr(self, "uq_state", None) or {}
        versions = st.get("versions") or {}
        unified = st.get("unified") or {}
        vlist = st.get("version_list") or unified.get("version_keys") or []
        return dict(
            has_state=bool(st),
            n_versions_dict=len(versions),
            n_version_list=len(vlist),
            unified_keys=list(unified.keys()),
            state_keys=list(st.keys()),
        )
    
    
    def uq_assert_state_ready(self, require_versions=True, require_unified=True):
        """
        Call this right after uq_build_dataset().
        It throws a clear RuntimeError if the build produced an unusable state.
        """
        s = self.uq_state_summary()
        if not s["has_state"]:
            raise RuntimeError("uq_state is missing/empty. Did uq_build_dataset() run successfully?")
    
        if require_versions and s["n_versions_dict"] == 0:
            raise RuntimeError(
                "uq_build_dataset() produced uq_state['versions']=0. Nothing can be computed/plotted.\n"
                "This typically means version products were not ingested.\n"
                "Check: version_list correctness; shakemap_folder contents; parsing of event products."
            )
    
        if require_unified and (not s["unified_keys"]):
            raise RuntimeError(
                "uq_build_dataset() produced uq_state['unified']=missing/empty. Unified stacks are required."
            )
        return True
    
    
    def _uq__filter_kwargs_for_callable(self, fn, kwargs: dict):
        """
        Internal signature-safe filter (keeps notebooks robust across refactors).
        If fn accepts **kwargs, returns kwargs unchanged.
        """
        if kwargs is None:
            return {}
        try:
    ...
    def uq_plot_method_map_safe(
        ...
        **plot_kwargs,
        ...
        """
        Safe wrapper around uq_plot_method_map:
          - enforces fail_mode (default annotate)
          - filters plot_kwargs to uq_plot_method_map signature
          - returns (fig, ax, rec)
        """
        payload = dict(
            version=version,
        ...
        payload = self._uq__filter_kwargs_for_callable(self.uq_plot_method_map, payload)
        return self.uq_plot_method_map(**payload)
    
    
    def uq_plot_method_map_panel_safe(
        self,
        *,
        version,
        imt,
        ...
        **plot_kwargs,
        ...
        """
        Safe wrapper around uq_plot_method_map_panel:
          - enforces fail_mode (default annotate)
          - filters plot_kwargs to panel signature
        """
        if not hasattr(self, "uq_plot_method_map_panel"):
            raise AttributeError("uq_plot_method_map_panel not found in this SHAKEuq version.")
        ...
        payload = self._uq__filter_kwargs_for_callable(self.uq_plot_method_map_panel, payload)
        return self.uq_plot_method_map_panel(**payload)
    
    
    def uq_plot_version_differences_safe(
        self,
        *,
        imt,
        what="sigma",
        ...
        **diff_kwargs,
        ...
        """
        Safe wrapper around uq_plot_version_differences:
          - keeps audit alive (fail_mode annotate where supported)
          - filters kwargs to signature
        """
        if not hasattr(self, "uq_plot_version_differences"):
            raise AttributeError("uq_plot_version_differences not found in this SHAKEuq version.")
        ...
        payload = self._uq__filter_kwargs_for_callable(self.uq_plot_version_differences, payload)
        return self.uq_plot_version_differences(**payload)
    
    
    # --------------------------------------------------------------------------------------------------
    # (C) OPTIONAL BUT RECOMMENDED: add a hard sanity check at the END of uq_build_dataset()
    # --------------------------------------------------------------------------------------------------
    # If you want the module to fail early (instead of printing "versions: 0" and moving on),
    # add this ONE line at the very end of uq_build_dataset() (before return):
    #
    #   self.uq_assert_state_ready(require_versions=True, require_unified=True)
    #
    # This will stop the pipeline immediately if the dataset build didn't ingest versions.
    # --------------------------------------------------------------------------------------------------
    
    
    # ==================================================================================================
    # HOW TO USE AFTER PATCH (in notebook)
    # ==================================================================================================
    # After uq.uq_build_dataset():
    #   uq.uq_assert_state_ready()
    #
    # For mapping plots (instead of uq.uq_plot_method_map):
    #   fig, ax, rec = uq.uq_plot_method_map_safe(..., fail_mode="annotate", **MAP_PLOT_KWARGS)
    #
    # For diffs:
    #   recs = uq.uq_plot_version_differences_safe(..., **DIFF_PLOT_KWARGS)
    # ==================================================================================================

...
############################
    ...
    # override with update v26.5 below 
    ...
    # override for orientation fix v26.5
    ...
    def uq_build_dataset(
        ...
        Path policy enforced:
          - Dataset base is ALWAYS: <base_folder>/SHAKEtime/<event_id>/uq
            unless base_folder already includes SHAKEtime or already points to <event_id>/uq.
          - If user passes "./export" -> "./export/SHAKEtime/<event_id>/uq"
          - If user passes "./export/SHAKEtime" -> "./export/SHAKEtime/<event_id>/uq"
          - If user passes ".../<event_id>/uq" (already final) -> keep as is
        ...
        # Case A: user already passed ".../<event_id>/uq" (final dataset folder)
        # Accept it as-is to avoid duplicating event_id/uq.
        if bf.name == "uq" and bf.parent.name == str(event_id):
            base = bf
    
        # Case B: user passed something that already ends with ".../SHAKEtime"
        # -> append <event_id>/uq
        elif bf.name.lower() == "shaketime":
            base = bf / str(event_id) / "uq"
    
        else:
            # Case C: user passed a folder that already contains a "SHAKEtime" segment somewhere
            # (e.g., "/data/project/export/SHAKEtime/runs")
            parts_lower = [p.lower() for p in bf.parts]
            if "shaketime" in parts_lower:
                base = bf / str(event_id) / "uq"
            else:
                # Case D: default - inject SHAKETIME
                base = bf / "SHAKEtime" / str(event_id) / "uq"
        ...
        self.uq_state = {
            ...
            # NOTE: base is the FULL dataset folder (ends with .../<event_id>/uq)
    ...
    # ======================================================================
    # Patch 4.2-PFIX1: Posterior loader must respect caller output_path
    # ======================================================================
    def _uq_posterior_sigma_fields(self, version, imt, method, output_path=None):
        """
        Return (sigma_ep_post_2d, sigma_total_post_2d, meta_dict).
    
        Load order:
          1) standardized uq_posteriors NPZ (any method) in output_path if provided
          2) legacy bayes loader (signature-flex) for bayesupdate
        """
        import numpy as np
        ...
        # 1) standardized store (USE output_path if given)
        dat = self._uq_load_posterior_npz(method=m, imt=imtU, version=v, output_path=output_path)
        ...
        # 2) legacy bayes loader support (only for bayesupdate)
        if m == "bayesupdate":
    ...
    # ---------------------------
    def _uq_cartopy_axes(self, figsize=(10, 8)): #legacy function
        import matplotlib.pyplot as plt
        try:
            import cartopy.crs as ccrs
            fig = plt.figure(figsize=figsize)
    ...
    def _uq_set_map_extent_from_axes(self, ax, pad_deg: float = 0.1): #legacy function 
    ...
    # ======================================================================
    # Patch 4.2-HFIX2: Broadcast sigma_a (and sigma_ep0) to grid shape
    # Fixes: IndexError: invalid index to scalar variable (like_var scalar)
    ...
    def _uq_bayes_local_posterior_at_mask(
        ...
        # ------------------------------------------------------------
        # Allow sigma_ep0 and/or sigma_a to be provided as scalars/constants.
        # They must be indexable by boolean masks on the unified grid.
        if sigma_ep0.ndim == 0:
    ...
    #override orientation fix v26..5
    ...
    def uq_extract_target_series(
        ...
        # decompose prior sigma_total (for bookkeeping + updates)
    ...
    # override for orientation v26.5 
    ...
    # ----------------------------------------------------------------------
    #override orientation update 26.5 
    def _uq_raw_sample_from_xml(self, grid_xml_path, uncertainty_xml_path, lat, lon, imt="MMI", sample="nearest"):
        ...
        Compatible with _uq_parse_gridxml_to_arrays() that returns:
          - lat_vec, lon_vec (1D)
          - fields: dict {NAME: idx0} where idx0 is 0-based index interpreted from XML
          - data: 2D array with nlat*nlon rows, columns are the raw grid_data columns
    
        Key fix:
          ShakeMap grid_field indices are sometimes ABSOLUTE (including LON/LAT in grid_field list),
          and sometimes RELATIVE (fields only, with LON/LAT implicit in grid_data columns 0,1).
          This function auto-detects which case applies and selects the correct column.
        """
        import numpy as np
        ...
        def _pick_field_col(D, name_candidates):
            """
            Return the correct column index in D["data"] for the requested field, or None.
            name_candidates: list of possible field names (already uppercased)
            """
            ...
            # Find the field index from mapping
            found = None
            ...
            # Decide whether mapping is absolute or field-block (needs +2)
            if _has_lonlat(fields):
                # Absolute indexing: use as-is
                col = found
                if 0 <= col < ncols:
                    return col
                return None
    
            # Otherwise assume field-block indexing (lon/lat are implicit as first 2 cols)
            col = found + 2
            if 0 <= col < ncols:
                return col
    
            # Fallback: if +2 fails but raw found works, use found
            if 0 <= found < ncols:
                return found
    
            return None
    
        # Candidate names (robustness)
        g_candidates = [imtU]
    ...
    # override orinetation update 26.5
    def _uq_area_raw_values_from_xml(
        ...
        raw_mu_vals : (n,) float
        raw_sg_vals : (n,) float
        meta : dict diagnostics about subsampling and missing fields
        """
        import numpy as np
    ...
    def uq_compare_raw_vs_unified_curves(
        ...
        # -----------------------------
        # Export CSV to export/SHAKEtime/<event_id>/uq/
        # -----------------------------
        if export_table and output_path is not None:
            ...
            outp = out_root / "SHAKEtime" / str(eid) / "uq"
            outp.mkdir(parents=True, exist_ok=True)
    ...
    # Paste this at the END of class SHAKEtime (later defs override earlier ones).
    #
    ...
    def _uq_posterior_outdir(self, output_path: str = None) -> str:
        ...
        if output_path is None:
            output_path = "./export"
        p = Path(output_path) / "SHAKEtime" / str(self.event_id) / "uq" / "uq_posteriors"
        p.mkdir(parents=True, exist_ok=True)
        return str(p)
    ...
    def _uq_load_posterior_npz(self, *, method: str, imt: str, version: int, output_path: str = None):
        ...
        f = self._uq_find_latest_posterior_file(method=method, imt=imt, version=version, output_path=output_path)
        if f is None:
            return None
        dat = dict(np.load(f, allow_pickle=True))
        meta = {}
        if "meta_json" in dat:
            try:
        ...
        *,
        ...
        if not bool(panel_all):
            ...
            if save and output_path:
                out_dir = Path(output_path) / "SHAKEtime" / str(self.event_id) / "uq" / "uq_maps"
                out_dir.mkdir(parents=True, exist_ok=True)
                if k == "change":
    ...
    def uq_extract_target_series(
        ...
        interp_method="nearest",
        interp_kwargs=None,
        # NEW (v26.6): compute only requested methods (Option B safety)
        ...
        Build tidy DataFrame of per-target, per-version:
          - ShakeMap published prediction (mean_published) and RAW sigma_total_published (from uncertainty.xml)
          - Method predicted prediction (mean_predicted) and updated sigma_total_predicted
          - Deltas vs ShakeMap published for explicit comparison.
        ...
        v26.6 Step 2:
          - Adds method "assimilation_chain": posterior(v-1) -> prior(v) (working space),
            propagated per-target-mask. Aleatory component remains fixed.
          - Adds methods_to_compute to avoid computing unused methods unless explicitly requested.
        """
        import numpy as np
        import pandas as pd
        from pathlib import Path
        ...
        # ----------------------------
        # Normalize + plan which methods to compute
        # ----------------------------
        def _norm_method(m):
            s = str(m).strip()
        ...
        legacy = ("bayes", "hierarchical", "kriging", "montecarlo")
        if methods_to_compute is None:
            compute = set(["ShakeMap"] + list(legacy))
        else:
            compute = set([_norm_method(m) for m in methods_to_compute])
            compute.add("ShakeMap")

        # Dependencies: MonteCarlo uses Bayes reference stats
        need_bayes_ref = ("montecarlo" in compute)
        ...
        # aggregation effective mode (support your global_stat variants)
        ...
        for t in targets:
            ...
            # ----------------------------
            # v26.6 Step 2: chain priors (per target)
            # ----------------------------
            if "assimilation_chain" in compute:
                mu_chain_prior_ws = np.array(mu0_ws, copy=True)
                s_ep_chain_prior = np.array(s_ep0, copy=True)
            else:
                mu_chain_prior_ws = None
                s_ep_chain_prior = None
            ...
            for v in versions:
                ...
                # ---------------------------------------------------------
                # Helper: append a method row (keeps schema consistent)
                # ---------------------------------------------------------
                def _append_method_row(method_name, mean_pred, sig_tot_pred, sig_ep_pred, sig_a_used, nobs_for_row):
                    rows.append({
                        "target_id": t.get("id", "GLOBAL"),
                        "target_type": ttype,
                        "version": int(v),
                        ...
                        "sigma_published_min": float(sig_pub_min),
                        "sigma_published_max": float(sig_pub_max),
                        "mean_predicted": float(mean_pred) if np.isfinite(mean_pred) else np.nan,
                        "sigma_total_predicted": float(sig_tot_pred) if np.isfinite(sig_tot_pred) else np.nan,
                        "delta_mean_vs_published": (float(mean_pred) - float(mean_pub)) if (np.isfinite(mean_pred) and np.isfinite(mean_pub)) else np.nan,
                        "delta_sigma_vs_published": (float(sig_tot_pred) - float(sig_pub_raw)) if (np.isfinite(sig_tot_pred) and np.isfinite(sig_pub_raw)) else np.nan,
                        "sigma_epistemic_predicted": float(sig_ep_pred) if np.isfinite(sig_ep_pred) else np.nan,
                        "sigma_aleatoric_used": float(sig_a_used) if np.isfinite(sig_a_used) else np.nan,
                        "n_obs_total": int(nobs_for_row),
                        "n_obs_seismic": int(counts.get("seismic", 0)),
                        "n_obs_intensity": int(counts.get("intensity", 0)),
                        "n_obs_unknown": int(counts.get("unknown", 0)),
                        "n_cells": int(n_cells),
                ...
                # ---------------------------------------------------------
                # NO-OBS behavior (per method)
                # ---------------------------------------------------------
                if n_obs == 0:
                    # bayes/hierarchical/kriging/montecarlo revert to v0 target scalars
                    if "bayes" in compute:
                        _append_method_row("bayes", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, 0)
                    if "hierarchical" in compute:
                        _append_method_row("hierarchical", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, 0)
                    if "kriging" in compute:
                        _append_method_row("kriging", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, 0)
                    if "montecarlo" in compute:
                        _append_method_row("montecarlo", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, 0)

                    # assimilation_chain: posterior=prior, but prior may already be updated from earlier versions
                    if "assimilation_chain" in compute:
                        mu_c_lin = self._uq_mu_from_ws(imtU, mu_chain_prior_ws)
                        mean_c = self._uq_agg(mu_c_lin[mask], agg=agg_effective)
                        s_ep_c_t = self._uq_agg(s_ep_chain_prior[mask], agg=agg_effective)
                        sig_c = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_c_t)**2)))
                        _append_method_row("assimilation_chain", mean_c, sig_c, s_ep_c_t, s_a0_t, 0)
                    ...
                    if audit:
                        audit_rows.append({
                            ...
                            "n_obs_intensity": int(counts.get("intensity", 0)),
                            "n_obs_unknown": int(counts.get("unknown", 0)),
                            "note": "No obs => bayes/hier/krig/mc revert to v0 prior scalars. assimilation_chain keeps last prior.",
                        })
                    continue

                # ---------------------------------------------------------
                # BAYES local update (working space) — compute if needed
                # ---------------------------------------------------------
                mu_b_ws = None
                s_ep_b = None
                mean_b = np.nan
                ...
                if ("bayes" in compute) or need_bayes_ref:
                    mu_b_ws, s_ep_b = self._uq_bayes_local_posterior_at_mask(
                        mu0_ws, s_ep0, s_a0,
                        lat2d, lon2d, mask, obs,
                        update_radius_km=update_radius_km,
                    ...
                    if "bayes" in compute:
                        _append_method_row("bayes", mean_b, sig_b, s_ep_b_t, s_a0_t, n_obs)

                # ---------------------------------------------------------
                # Assimilation chain (v26.6 Step 2)
                # ---------------------------------------------------------
                if "assimilation_chain" in compute:
                    mu_c_ws, s_ep_c = self._uq_bayes_local_posterior_at_mask(
                        mu_chain_prior_ws, s_ep_chain_prior, s_a0,
                        lat2d, lon2d, mask, obs,
                        update_radius_km=update_radius_km,
                        kernel=kernel,
                        kernel_scale_km=kernel_scale_km,
                    ...
                    # Merge updated mask back into full-grid priors (avoid NaNs outside mask)
                    mu_chain_next = np.array(mu_chain_prior_ws, copy=True)
                    s_ep_chain_next = np.array(s_ep_chain_prior, copy=True)

                    mu_chain_next[mask] = mu_c_ws[mask]
                    s_ep_chain_next[mask] = s_ep_c[mask]

                    # report chain aggregate at target
                    mu_c_lin = self._uq_mu_from_ws(imtU, mu_chain_next)
                    mean_c = self._uq_agg(mu_c_lin[mask], agg=agg_effective)
                    s_ep_c_t = self._uq_agg(s_ep_chain_next[mask], agg=agg_effective)
                    sig_c = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_c_t)**2)))

                    _append_method_row("assimilation_chain", mean_c, sig_c, s_ep_c_t, s_a0_t, n_obs)

                    # propagate
                    mu_chain_prior_ws = mu_chain_next
                    s_ep_chain_prior = s_ep_chain_next

                # ---------------------------------------------------------
                # Hierarchical-like update (independent per version; v0 prior)
                # ---------------------------------------------------------
                if "hierarchical" in compute:
                    mu_h_ws, s_ep_h = self._uq_hierarchical_posterior_at_mask(
                        mu0_ws, s_ep0, s_a0,
                    ...
                    mean_h = self._uq_agg(mu_h_lin[mask], agg=agg_effective)
                    s_ep_h_t = self._uq_agg(s_ep_h[mask], agg=agg_effective)
                    sig_h = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_h_t)**2)))
                    _append_method_row("hierarchical", mean_h, sig_h, s_ep_h_t, s_a0_t, n_obs)

                # ---------------------------------------------------------
                # Kriging residual posterior (independent per version; v0 prior)
                # ---------------------------------------------------------
                if "kriging" in compute:
                    mu_k_ws, s_ep_k = self._uq_ok_residual_posterior_at_mask(
                        mu0_ws, s_ep0, s_a0,
                    ...
                    mean_k = self._uq_agg(mu_k_lin[mask], agg=agg_effective)
                    s_ep_k_t = self._uq_agg(s_ep_k[mask], agg=agg_effective)
                    sig_k = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_k_t)**2)))
                    _append_method_row("kriging", mean_k, sig_k, s_ep_k_t, s_a0_t, n_obs)

                # ---------------------------------------------------------
                # Monte Carlo (target-level) around Bayes posterior stats
                # ---------------------------------------------------------
                if "montecarlo" in compute:
                    # If Bayes wasn't computed for some reason, fall back to v0
                    mu_ref = float(mean_b) if np.isfinite(mean_b) else float(mu0_lin_t)
                    sig_ref = float(sig_b) if (np.isfinite(sig_b) and mc_include_aleatory) else float(s_ep_b_t if np.isfinite(s_ep_b_t) else s_ep0_t)
                    ...
                    if mc_include_aleatory:
                        sig_tot_mc = sig_mc
                        sig_ep_mc = float(np.sqrt(max(0.0, sig_tot_mc**2 - float(s_a0_t)**2)))
                    else:
                        sig_ep_mc = sig_mc
                        sig_tot_mc = float(np.sqrt(max(0.0, float(s_a0_t)**2 + sig_ep_mc**2)))

                    _append_method_row("montecarlo", mean_mc, sig_tot_mc, sig_ep_mc, s_a0_t, n_obs)
                ...
                if audit:
                    audit_rows.append({
                        ...
                        "mask_kind": meta.get("kind", "global" if t.get("type") == "global" else "mask"),
                        "mask_meta": str(meta)[:800],
                        "note": "Published sigma_total is RAW uncertainty.xml. Methods predict mean+sigma via v0 prior + obs. assimilation_chain propagates posterior(v-1)->prior(v).",
                    })

        df = pd.DataFrame(rows)

        # ----------------------------
        # Audit export (same behavior as before)
        # ----------------------------
        if audit:
            try:
                out_dir = Path(audit_output_path) if audit_output_path is not None else None
                if out_dir is not None:
                    out_dir.mkdir(parents=True, exist_ok=True)
                # keep your existing writer if present
                if hasattr(self, "_uq_write_audit"):
                    self._uq_write_audit(
                        df=df,
                ...
                # audit should never break the pipeline
                pass
    ...
    def uq_plot_targets_decay(
        ...
        kernel_scale_km=20.0,
        measurement_sigma=0.30,
        # NEW (v26.6 Step 4): two-likelihood controls (optional)
        ...
        mark_first_reach=False,
        reach_mode="first_cross",
        truth_tolerance=None,
        truth_method_for_reach="ShakeMap",
        warn_on_truth_miss=True,
        # audit
        audit=True,
        audit_output_path=None,
        audit_prefix=None,
        ...
        """
        v26.6-compatible target-decay plotter.
    
        Key behavior:
          - Option-B compute: passes methods_to_compute=methods into uq_extract_target_series
          - Step 4 plumbing: accepts measurement_sigma_instr / measurement_sigma_dyfi and forwards them
            (they only matter if uq_extract_target_series uses them; otherwise harmless)
          - Truth overlay:
              * preferred: truth stored in target dict (truth_<imt>, truth_value, truth)
              * fallback: truth_value kwarg (scalar or dict keyed by target_id)
          - Axis control: ymin/ymax/xmin/xmax supported
        """
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        # -----------------------------
        # 1) Extract series (Option B: compute only requested methods)
        # -----------------------------
        df = self.uq_extract_target_series(
            version_list=version_list,
            imt=imt,
            ...
            kernel_scale_km=kernel_scale_km,
            measurement_sigma=measurement_sigma,
            # Step 4 forwarding (safe even if unused internally)
            ...
            interp_method=interp_method,
            interp_kwargs=interp_kwargs,
            methods_to_compute=methods,  # <<< Option B
            audit=audit,
            audit_output_path=audit_output_path or output_path,
            audit_prefix=audit_prefix,
        )
        ...
        # delta support: delta_<col> relative to first shown version (per target/method)
        if w.lower().startswith("delta_"):
            base_col = w[len("delta_") :].strip()
            if base_col not in df.columns:
                raise ValueError(
                    f'delta requested but base column "{base_col}" not found. '
                    f"Choose from: {sorted(df.columns.tolist())}"
                )
            df2 = df.copy()
            df2[w] = np.nan
        ...
        # -----------------------------
        # 3) Unified axes for band shading masks
        # -----------------------------
        versions = [int(v) for v in (version_list or [])]
        _, lat2d, lon2d = self._uq_get_unified_for_versions(
            versions,
        ...
        # map id -> target dict for truth lookup
        targets_by_id = {}
        try:
        ...
        # Truth helpers
        # -----------------------------
        def _get_truth_for_target(_tid):
            if not plot_truth:
                return None
    
            tv = None
            tdict = targets_by_id.get(str(_tid), None)
    
            # 1) look inside target dict
            if isinstance(tdict, dict):
                if truth_key is not None:
                    tv = tdict.get(truth_key, None)
                else:
                    imtl = str(imt).lower().strip()
                    for k in (f"truth_{imtl}", "truth_value", "truth"):
                        if k in tdict:
                            tv = tdict.get(k, None)
                            break
    
            # 2) fallback to truth_value kwarg
            if tv is None and truth_value is not None:
                if isinstance(truth_value, dict):
                    tv = truth_value.get(str(_tid), None)
                else:
                    tv = truth_value
    
            if tv is None:
                if warn_on_truth_miss and plot_truth:
                    # keep silent by default; user can inspect via audit logs
                    pass
                return None
        ...
        def _first_reach_marker(x, y, truth, mode="first_cross", tol=None):
            if truth is None:
                return None, None, None
            x = np.asarray(x, dtype=float)
            ...
            if mode == "closest":
                d = np.abs(y - truth)
                j = int(np.argmin(d))
                if tol is not None and float(d[j]) > float(tol):
                    return None, None, None
                return x[j], y[j], j
    
            # default: first_cross (>= truth)
            hit = (y >= truth)
            if not np.any(hit):
                if tol is not None:
                    near = (np.abs(y - truth) <= float(tol))
                    if np.any(near):
        ...
        _truth_line_defaults = dict(
            color="red",
            linestyle="--",
            linewidth=max(1.0, float(linewidth) * 0.9),
            alpha=0.95,
        )
        _truth_marker_defaults = dict(
            color="red",
            marker="*",
            markersize=max(10, int(markersize * 1.2)),
            linewidth=0,
        )
        ...
        for tid in targets:
            ...
            # published band for area/global
            ...
            if published_band and is_area_like and ("ShakeMap" in methods):
                ...
                for v in sorted(sub["version"].unique().tolist()):
                    ...
                    xs.append(int(v))
                    if w_is_mean_like:
                        lo.append(band["mu_min"])
                        hi.append(band["mu_max"])
                    else:
                        lo.append(band["sig_min"])
                        hi.append(band["sig_max"])
                ...
                if len(xs) > 0:
                    ax.fill_between(
                        ...
                        lo,
                        hi,
                        alpha=float(published_band_alpha),
                        label="ShakeMap min/max (area/global)",
                    )
            ...
            # truth overlay (mean-like only)
            tv = _get_truth_for_target(tid)
            if w_is_mean_like and (tv is not None):
                lk = dict(_truth_line_defaults)
                lk.update(truth_line_kwargs or {})
                ax.axhline(float(tv), **lk, label=str(truth_label))
    
                if mark_first_reach and (truth_method_for_reach in methods):
                    s0 = sub[sub["method"] == truth_method_for_reach].sort_values("version")
                    xm, ym, _ = _first_reach_marker(
                        s0["version"].values,
                        s0[w].values,
                        float(tv),
                        mode=reach_mode,
                    ...
                    if xm is not None:
                        mk = dict(_truth_marker_defaults)
                        mk.update(truth_marker_kwargs or {})
                        ax.plot([xm], [ym], label=f"first reach ({truth_method_for_reach})", **mk)
    
            ax.set_xlabel(str(xlabel))
            if ylabel is None:
                ax.set_ylabel(f"{str(imt).upper()} mean" if w_is_mean_like else f"{w}")
            else:
                ax.set_ylabel(str(ylabel))
            ...
            if show_title:
                if title is not None:
                    ax.set_title(
                        str(title).replace("{target}", str(tid)).replace("{imt}", str(imt).upper()),
                        fontsize=title_size,
                    )
                else:
        ...
        if plot_combined:
            ...
            # only support a single global truth line in combined plot when truth_value is scalar
            tv_combined = None
            if plot_truth and (truth_value is not None) and (not isinstance(truth_value, dict)):
                try:
                    tv_combined = float(truth_value)
                except Exception:
                    tv_combined = None
            ...
            if w_is_mean_like and (tv_combined is not None):
                lk = dict(_truth_line_defaults)
                lk.update(truth_line_kwargs or {})
                ax.axhline(float(tv_combined), **lk, label=str(truth_label))
            ...
            ax.set_xlabel(str(xlabel))
            ax.set_ylabel(str(ylabel) if ylabel is not None else f"{w}")
    ...
    def _uq_collect_obs_for_version(
        ...
        prefer_domain=True,
        allow_fallback=True,
        # NEW (v26.6 Step 4): two-likelihood controls
        measurement_sigma_instr=None,
        measurement_sigma_dyfi=None,
        ...
        v26.6 Step 4:
          - Optionally attaches per-observation measurement variance:
              * instrumented/seismic -> measurement_sigma_instr
              * DYFI/intensity      -> measurement_sigma_dyfi
            (fallback to measurement_sigma if the specific one is None)
          - When prefer_domain=True, retains legacy domain filtering behavior.
          - When prefer_domain=False, returns ALL obs (mixed streams).

        Returns:
          obs_list: list of dicts with fields:
            lat, lon, y_ws, w, type, domain,
            (optional) meas_var, sigma_obs
          obs_counts: dict with counts by inferred type
        """
        imtU = str(imt).upper().strip()
        domain_pref = self._uq_infer_obs_domain(imtU)
        ...
        # set per-stream measurement sigmas (working-space assumed consistent with your existing knobs)
        sig_instr = measurement_sigma if (measurement_sigma_instr is None) else float(measurement_sigma_instr)
        sig_dyfi  = measurement_sigma if (measurement_sigma_dyfi  is None) else float(measurement_sigma_dyfi)

        # normalize and infer domains
        tmp = []
        for r in raw:
            if r is None or (not isinstance(r, dict)):
                continue
            ...
            typ = str(r.get("type", r.get("domain", r.get("source", "unknown")))).lower()
            if ("dyfi" in typ) or ("mmi" in typ) or ("intensity" in typ) or ("cdi" in typ):
                dom = "intensity"
            elif ("station" in typ) or ("seismic" in typ) or ("pga" in typ) or ("pgv" in typ) or ("instrument" in typ):
                dom = "seismic"
            else:
                dom = "unknown"
        ...
        # legacy domain preference (keeps your old behavior)
        if prefer_domain:
            preferred = [r for (r, dom) in tmp if dom == domain_pref]
            if preferred:
                use = preferred
        ...
        for (r, dom) in tmp:
            ...
            if dom == "intensity":
                ...
                counts["unknown"] += 1
                sig_obs = float(measurement_sigma)
            ...
            if attach_per_obs_sigma:
                ob["sigma_obs"] = float(sig_obs)
                ob["meas_var"] = float(sig_obs) ** 2
    ...
    def _uq_bayes_local_posterior_at_mask(
        ...
        v26.6 Step 4:
          - If obs contains `meas_var` (or `sigma_obs`), use per-observation likelihood variance.
          - Else fall back to scalar measurement_sigma.
        ...
        for ob in obs_list:
            ...
            # per-observation measurement variance (preferred), else scalar fallback
            if "meas_var" in ob:
                meas_var = float(ob["meas_var"])
            elif "sigma_obs" in ob:
                meas_var = float(ob["sigma_obs"]) ** 2
            else:
                meas_var = float(measurement_sigma) ** 2

            # like variance per cell (sigma_a is cellwise)
            like_var = np.clip(sigma_a**2 + meas_var, 1e-16, np.inf)
    ...
    def uq_extract_target_series(
        ...
        measurement_sigma_instr=None,
        measurement_sigma_dyfi=None,
        # kriging (kept for signature compatibility, even if not used by your current method set)
        ok_range_km=60.0,
        ok_variogram="exponential",
        ok_nugget=1e-6,
        ok_sill=None,
        ...
        """
        v26.6 Step 4:
          - Adds method "bayes_2lik" using mixed obs with per-observation likelihood variance:
              * seismic/instrumented -> measurement_sigma_instr
              * DYFI/intensity      -> measurement_sigma_dyfi
          - Uses prefer_domain=False for bayes_2lik so both streams can contribute.
        """
        import numpy as np
        import pandas as pd
        from pathlib import Path
        ...
        legacy = ("bayes", "bayes_2lik", "assimilation_chain", "hierarchical", "kriging", "montecarlo")
        if methods_to_compute is None:
            compute = set(["ShakeMap"] + list(legacy))
        else:
            compute = set([_norm_method(m) for m in methods_to_compute])
        ...
        # NOTE: your _uq_decompose_sigma_safe returns 3 values (we discard first)
        _sig_tot0, s_a0, s_ep0 = self._uq_decompose_sigma_safe(
            imtU,
            sig0_raw,
            sigma_aleatory=sigma_aleatory,
        ...
        for t in targets:
            ...
            for v in versions:
                ...
                rows.append({
                    ...
                    "sigma_total_predicted": float(sig_pub_raw),
                    "delta_mean_vs_published": 0.0,
                    "delta_sigma_vs_published": 0.0,
                    "sigma_epistemic_predicted": np.nan,
                    "sigma_aleatoric_used": np.nan,
                ...
                # LEGACY obs: prefer domain (old behavior)
                obs1, counts1 = self._uq_collect_obs_for_version(
                    int(v), imtU,
                    measurement_sigma=measurement_sigma,
                    include_weights=True,
                    prefer_domain=True,
                ...
                # 2LIK obs: mixed streams + per-obs sigma (new behavior)
                obs2, counts2 = self._uq_collect_obs_for_version(
                    int(v), imtU,
                    measurement_sigma=measurement_sigma,
                    include_weights=True,
                    prefer_domain=False,   # key change
                    allow_fallback=True,
                    measurement_sigma_instr=measurement_sigma_instr,
                    measurement_sigma_dyfi=measurement_sigma_dyfi,
                    attach_per_obs_sigma=True,
                ...
                def _append(method_name, mean_pred, sig_tot_pred, sig_ep_pred, sig_a_used, counts):
                    rows.append({
                        "target_id": t.get("id", "GLOBAL"),
                        "target_type": ttype,
                        "version": int(v),
                        ...
                        "sigma_epistemic_predicted": float(sig_ep_pred) if np.isfinite(sig_ep_pred) else np.nan,
                        "sigma_aleatoric_used": float(sig_a_used) if np.isfinite(sig_a_used) else np.nan,
                        "n_obs_total": int(counts.get("total", 0)),
                        "n_obs_seismic": int(counts.get("seismic", 0)),
                        "n_obs_intensity": int(counts.get("intensity", 0)),
                        "n_obs_unknown": int(counts.get("unknown", 0)),
                        "n_cells": int(n_cells),
                ...
                # -------- bayes (1-likelihood, legacy) --------
                mu_b_ws = s_ep_b = None
                mean_b = sig_b = s_ep_b_t = np.nan

                if ("bayes" in compute) or need_bayes_ref:
                    if counts1.get("total", 0) == 0:
                        if "bayes" in compute:
                            _append("bayes", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, counts1)
                    else:
                        mu_b_ws, s_ep_b = self._uq_bayes_local_posterior_at_mask(
                            mu0_ws, s_ep0, s_a0,
                            lat2d, lon2d, mask, obs1,
                            update_radius_km=update_radius_km,
                            kernel=kernel,
                            kernel_scale_km=kernel_scale_km,
                            measurement_sigma=measurement_sigma,
                        ...
                        sig_b = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_b_t)**2)))
                        if "bayes" in compute:
                            _append("bayes", mean_b, sig_b, s_ep_b_t, s_a0_t, counts1)

                # -------- bayes_2lik (two-likelihood) --------
                if "bayes_2lik" in compute:
                    if counts2.get("total", 0) == 0:
                        _append("bayes_2lik", mu0_lin_t, sig0_raw_t, s_ep0_t, s_a0_t, counts2)
                    else:
                        mu2_ws, s_ep2 = self._uq_bayes_local_posterior_at_mask(
                            mu0_ws, s_ep0, s_a0,
                            lat2d, lon2d, mask, obs2,
                            update_radius_km=update_radius_km,
                            kernel=kernel,
                            kernel_scale_km=kernel_scale_km,
                        ...
                        _append("bayes_2lik", mean2, sig2, s_ep2_t, s_a0_t, counts2)

                # -------- assimilation_chain (still uses legacy obs stream) --------
                if "assimilation_chain" in compute:
                    if counts1.get("total", 0) == 0:
                        mu_c_lin = self._uq_mu_from_ws(imtU, mu_chain_prior_ws)
                        mean_c = self._uq_agg(mu_c_lin[mask], agg=agg_effective)
                        s_ep_c_t = self._uq_agg(s_ep_chain_prior[mask], agg=agg_effective)
                        sig_c = float(np.sqrt(max(0.0, float(s_a0_t)**2 + float(s_ep_c_t)**2)))
                        _append("assimilation_chain", mean_c, sig_c, s_ep_c_t, s_a0_t, counts1)
                    else:
                        mu_c_ws, s_ep_c = self._uq_bayes_local_posterior_at_mask(
                            mu_chain_prior_ws, s_ep_chain_prior, s_a0,
                            lat2d, lon2d, mask, obs1,
                ...
                if audit:
                    audit_rows.append({
                        ...
                        "agg": str(agg_effective),
                        "n_cells": int(n_cells),
                        "obs_legacy_total": int(counts1.get("total", 0)),
                        "obs_2lik_total": int(counts2.get("total", 0)),
                        "obs_2lik_seismic": int(counts2.get("seismic", 0)),
                        "obs_2lik_intensity": int(counts2.get("intensity", 0)),
                        "mask_kind": meta.get("kind", "global" if t.get("type") == "global" else "mask"),
                        "note": "bayes uses prefer_domain=True (legacy). bayes_2lik uses prefer_domain=False + per-obs meas_var.",
                    })

        df = pd.DataFrame(rows)

        # optional audit writer (do not break pipeline)
        if audit:
            try:
                if hasattr(self, "_uq_write_audit"):
                    self._uq_write_audit(

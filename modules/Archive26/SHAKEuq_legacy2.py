...
Date:
    February, 2026
Version:
    26.6 (draft split from SHAKEuq v26.5) LEGACY 
...
# ##################################################
    ...
    def _uq__get_unified_grid_safe(self):
        ...
        # Prefer existing class helper if present
        try:
        ...
        if lon2d is None or lat2d is None:
            # some builds store x/y or lons/lats; try common alternates
            lon2d = u.get("lon", u.get("lons", None))
            lat2d = u.get("lat", u.get("lats", None))
        if lon2d is None or lat2d is None:
            raise RuntimeError("Unified grid not found. Expected _get_unified_grid() or uq_state['unified']['lon2d'/'lat2d'].")
    ...
    def _uq__axis_versions_or_time(self, version_list, *, x_axis="auto"):
        ...
        # try TAE_hours from sanity table
        tae = []
        try:
    ...
    def _uq__circle_stats(self, lon2d, lat2d, field2d, *, lon, lat, radius_km=30.0):
        """Return (mean, min, max, n) of field2d within radius_km around (lon, lat)."""
        import numpy as np
        if field2d is None:
        ...
        try:
            ...
            if vals.size == 0:
                return np.nan, np.nan, np.nan, 0
            return float(np.mean(vals)), float(np.min(vals)), float(np.max(vals)), int(vals.size)
        except Exception:
            return np.nan, np.nan, np.nan, 0
    ...
    def _uq__get_published_unified_grids(self, vkey, imt_u):
        ...
        try:
            # Prefer existing helper that maps IMT->sigma field key
    ...
    def _uq__get_method_grids(
        ...
        *,
        ...
        """Return (mean2d, sigma2d, meta_dict) for a method at a version on unified grid."""
        imt_u = str(imt).upper().strip()
        m = str(method).lower().strip()
        mk = {"raw": "ShakeMap", "shakemap": "ShakeMap", "bayes": "bayes",
        ...
        vpack = ((self.uq_state or {}).get("versions") or {}).get(vkey)
        if vpack is None:
        ...
        if str(mk).lower().strip() == "bayes":
            ...
            if k is None and run_if_missing and hasattr(self, "run_bayes_update"):
                try:
                    self.run_bayes_update(
                        version=vkey,
                        mode=(mode or method_kwargs.get("mode_fallback", "mmi_dyfi")),
                        **{kk: vv for kk, vv in method_kwargs.items() if kk not in ("mode_key",)}
                    )
                except Exception as e:
                    if debug:
                        print(f"[uq_plot_targets_decay] bayes run_if_missing failed: {repr(e)}")
                k = self._uq__resolve_bayes_key(vkey, mode=mode, mode_key=mode_key, imt_u=imt_u)
            ...
            post = pack.get("posterior") or {}
            meta.update({"key": k, "meta_pack": pack.get("meta") or {}})
            return post.get("mean_grid"), post.get("sigma_grid"), meta
        ...
        if str(mk).lower().strip() == "rk":
            ...
            if k is None and run_if_missing and hasattr(self, "run_residual_kriging_update"):
                try:
                    self.run_residual_kriging_update(version=vkey, key=key, **method_kwargs)
                except Exception as e:
                    if debug:
                        print(f"[uq_plot_targets_decay] rk run_if_missing failed: {repr(e)}")
                k = self._uq__resolve_update_key(vkey, prefix="rk__", key=key)
            ...
            meta.update({"key": k, "meta_pack": pack.get("meta") or {}})
            return pack.get("mean_post"), pack.get("sigma_post"), meta
        ...
        if str(mk).lower().strip() == "ens":
            ...
            if k is None and run_if_missing and hasattr(self, "run_ensemble_update"):
                try:
                    self.run_ensemble_update(version=vkey, update_key=update_key, **method_kwargs)
                except Exception as e:
                    if debug:
                        print(f"[uq_plot_targets_decay] ens run_if_missing failed: {repr(e)}")
                k = self._uq__resolve_update_key(vkey, prefix="ens__", key=update_key)
            ...
            meta.update({"key": k, "meta_pack": pack.get("meta") or {}})
            post = pack.get("posterior") or {}
            mu = post.get("mean_grid") if isinstance(post, dict) else pack.get("mean_post")
            sig = post.get("sigma_grid") if isinstance(post, dict) else pack.get("sigma_post")
            return mu, sig, meta
    ...
    def uq_plot_targets_decay(
        ...
        *,
        ...
        area_radius_km=30.0,      # used only when point_mode="area"
        # plot style
        figsize=(10, 5),
        dpi=150,
        markers=None,
        ...
        raw_show_summary=True,        # applies to GLOBAL and POINT-AREA raw curve
        ...
        """Plot uncertainty / prediction decay over versions or time-after-event.

        Targets:
          - points: list of dicts: {id, lat, lon, true_mmi?, true_pga?, ...}
          - global_targets: list of strings (optional). If empty and no points, defaults to GLOBAL.

        point_mode:
          - "point": nearest grid sample at (lon, lat)
          - "area": statistics within radius (km) around (lon, lat):
                * all methods plotted as AREA MEAN
                * raw ShakeMap additionally shows min–max shaded envelope (like global decay)

        what:
          - "sigma": uses sigma grids
          - "mean":  uses mean grids
        """
        import numpy as np
        import pandas as pd
        import matplotlib.pyplot as plt
        ...
        if what_s not in ("sigma", "mean"):
            raise ValueError("what must be 'sigma' or 'mean' for this plotting utility.")
        ...
        truth_kwargs = truth_kwargs or {"color": "r", "linewidth": 1.8, "linestyle": "-", "alpha": 0.9, "label": "truth"}

        lon2d, lat2d = self._uq__get_unified_grid_safe()

        # compute x location for 24h line even on version axis
        x24 = None
        if show_24h_line:
            try:
                x_axis_s = str(x_axis).lower().strip()
                if x_axis_s == "time" or (x_axis_s == "auto" and all(np.isfinite(t) for t in tae_h)):
                    ...
                    # map 24h to nearest version position using tae_h
        ...
        for (ttype, tid, tlat, tlon, tmeta) in targets:
            ...
            # raw summary caches (min/max shading) for GLOBAL and POINT-AREA
            raw_mean_cache = []
            raw_med_cache = []
            raw_min_cache = []
            raw_max_cache = []
            ...
            for meth in methods:
                ...
                for j, vk in enumerate(vkeys):
                    mu2d, sig2d, meta = self._uq__get_method_grids(
                        vkey=vk,
                        method=meth,
                        imt=imt_u,
                        run_if_missing=run_if_missing,
                        method_kwargs=(method_kwargs.get(meth) or method_kwargs.get(str(meth).lower()) or {}),
                        debug=debug,
                    )

                    grid2d = sig2d if what_s == "sigma" else mu2d

                    # --- value extraction per target/mode
                    if ttype == "point":
                        if point_mode_s == "point":
                            val = self._uq__nearest_grid_sample(lon2d, lat2d, grid2d, lon=tlon, lat=tlat)

                        else:
                            # area mode: mean within radius
                            vmean, vmin, vmax, n = self._uq__circle_stats(
                                lon2d, lat2d, grid2d, lon=tlon, lat=tlat, radius_km=area_radius_km
                            )
                            val = vmean

                            # raw ShakeMap additionally caches min/max envelope like global
                            if str(meth).lower().strip() in ("shakemap", "raw") and raw_show_summary:
                                if grid2d is not None and np.isfinite(vmean):
                                    raw_mean_cache.append(float(vmean))
                                    # median/min/max within the circle for better stability
                                    # (median computed from vals if needed, but we approximate by recomputing once)
                                    # recompute robustly:
                                    try:
                                        import numpy as _np
                                        d = self._uq__haversine_km(lon2d, lat2d, float(tlon), float(tlat))
                                        msk = (d <= float(area_radius_km))
                                        vals = _np.asarray(grid2d, dtype=float)[msk]
                                        vals = vals[_np.isfinite(vals)]
                                        raw_med_cache.append(float(_np.median(vals)) if vals.size else _np.nan)
                                        raw_min_cache.append(float(_np.min(vals)) if vals.size else _np.nan)
                                        raw_max_cache.append(float(_np.max(vals)) if vals.size else _np.nan)
                                    except Exception:
                                        raw_med_cache.append(np.nan)
                                        raw_min_cache.append(np.nan)
                                        raw_max_cache.append(np.nan)
                        ...
                        if grid2d is None:
                            ...
                            if str(meth).lower().strip() in ("shakemap", "raw") and raw_show_summary:
                                raw_mean_cache.append(float(np.nanmean(vv)))
            ...
            # raw summary overlays (GLOBAL or POINT-AREA)
            raw_needed = (ttype == "global") or (ttype == "point" and point_mode_s == "area")
            if raw_needed and raw_show_summary and raw_mean_cache:
                try:
                    ymed = np.array(raw_med_cache, dtype=float) if raw_med_cache else None
                    if ymed is not None and ymed.size == len(x):
            ...
            # truth line (only for POINT and only if provided)
            if ttype == "point" and show_truth_line and isinstance(tmeta, dict):
                tv = None
                if imt_u == "MMI":
            ...
            # y-label (prefer existing helper if present)
            if what_s == "sigma":
                try:

...
# ##################################################
    ...
    # ==========================================================================
    # PATCH v26.10 — CDI routing + version/time gate + 2-likelihood + DYFI weights
    # Paste at END of SHAKEuq.py (inside class indentation; "last definition wins")
    # ==========================================================================

    def _uq_is_cdi_available_for_version(self, version, stations_folder=None, rupture_folder=None):
        """
        CDI availability gate:
          - Requires dyfi_cdi_file exists and has rows.
          - Optional version gate via self.dyfi_cdi_use_after_version (default=4).
          - Optional time gate via self.dyfi_use_after_hours (default=24) IF dyfi_source="auto".
        """
        import os
        import numpy as np
        ...
        # file must exist
        cdi_file = getattr(self, "dyfi_cdi_file", None)
        if not cdi_file or (not os.path.exists(str(cdi_file))):
            return False

        # version gate (default: after v4)
        v_gate = int(getattr(self, "dyfi_cdi_use_after_version", 4))
        if v < v_gate:
            return False
        ...
        try:
            ...
            if "lat" in df.columns and "lon" in df.columns:
                if int(np.isfinite(df["lat"].values).sum()) == 0:
                    return False
        except Exception:
            return False
    ...
    def _uq_bayes_local_posterior_2lik_at_mask(
        ...
        """
        2-likelihood wrapper: relies on _uq_bayes_local_posterior_at_mask which already:
          - uses ob["meas_var"] if present,
          - else ob["sigma_obs"]**2,
          - else measurement_sigma**2,
          - uses ob["w"] as an observation weight.
        """
        return self._uq_bayes_local_posterior_at_mask(
            mu0_ws,
            sigma_ep0,
    ...
    def _uq_dyfi_weighted_posterior_at_mask(
        ...
        """
        DYFI-weighted wrapper: weights must be pre-applied into obs_list as ob["w"].
        """
        return self._uq_bayes_local_posterior_at_mask(
            mu0_ws,
            sigma_ep0,
    ...
    def _uq_collect_obs_for_version(
        ...
        """
        Unified obs collector with DYFI source switching:
          stationlist → per-version stationlist (stations + DYFI) via _uq_collect_observations()
          cdi         → CDI for MMI only (version-gated via dyfi_cdi_use_after_version, default=4)
          auto        → stationlist early; CDI after BOTH:
                          - version gate (>= dyfi_cdi_use_after_version)
                          - time gate (TaE_hours >= dyfi_use_after_hours)
        """
        import numpy as np
        ...
        # infer preferred domain by IMT
        domain_pref = "intensity" if imtU == "MMI" else "seismic"
        if hasattr(self, "_uq_infer_obs_domain"):
            try:
                domain_pref = self._uq_infer_obs_domain(imtU)
            except Exception:
                pass
        ...
        if mode == "auto":
            ...
            tae_h = None
            try:
                tae_h = float(self._uq_hours_since_origin(v))
            except Exception:
                tae_h = None
            h_gate = float(getattr(self, "dyfi_use_after_hours", 24.0))
            pass_time = (tae_h is not None) and np.isfinite(tae_h) and (tae_h >= h_gate)

            # version+file gate
            pass_ver = self._uq_is_cdi_available_for_version(v)

            mode_eff = "cdi" if (pass_time and pass_ver) else "stationlist"
        ...
        if mode_eff == "cdi":
            if imtU != "MMI":
                ...
                # must pass version/file gate for CDI
                if not self._uq_is_cdi_available_for_version(v):
                    if not allow_fallback:
                        return [], {"total": 0, "seismic": 0, "intensity": 0, "unknown": 0}
                    mode_eff = "stationlist"
                else:
        ...
        # stationlist branch
        raw = self._uq_collect_observations(v, imtU)
        if raw is None:
            raw = []
        ...
        for ob in raw:
            ...
            if dom not in ("seismic", "intensity"):
                dom = "unknown"
            o["domain"] = dom

            # source normalize
            if "source" not in o or o["source"] is None:
                o["source"] = "stationlist"

            # attach per-obs sigma/variance (if requested)
            if attach_per_obs_sigma:
                if o.get("meas_var", None) is None:
                    so = o.get("sigma_obs", None)
                    if so is None:
                        # default per-domain sigma
                        if dom == "seismic" and measurement_sigma_instr is not None:
                            so = float(measurement_sigma_instr)
                        elif dom == "intensity" and measurement_sigma_dyfi is not None:
                            so = float(measurement_sigma_dyfi)
                        else:
                            so = float(measurement_sigma)
                        o["sigma_obs"] = float(so)
                        o["meas_var"] = float(so) ** 2
                    else:
                        try:
                            o["meas_var"] = float(so) ** 2
                        except Exception:
        ...
        # prefer_domain filtering
        if prefer_domain and domain_pref in ("seismic", "intensity"):
            obs_pref = [o for o in obs_all if str(o.get("domain", "unknown")).lower().strip() == domain_pref]
            if len(obs_pref) > 0:
                c = {"total": 0, "seismic": 0, "intensity": 0, "unknown": 0}
                for oo in obs_pref:
                    c["total"] += 1
                    dd = oo.get("domain", "unknown")
                    c[dd] = c.get(dd, 0) + 1
                return obs_pref, c
            if not allow_fallback:
                return [], {"total": 0, "seismic": 0, "intensity": 0, "unknown": 0}
    ...
    def uq_extract_target_series(
        ...
        def _apply_dyfi_weights(obs_list, mode, w_max):
            ...
            for o in (obs_list or []):
                ...
                # fallback threshold rule (if exists)
                if hasattr(self, "_uq_dyfi_weight_from_row"):
                    try:
                        o["w"] = float(self._uq_dyfi_weight_from_row(n))
                    except Exception:
        ...
        for t in targets:
            ...
            for v in versions:
                ...
                # priors in WS-space
                mu0_ws = self._uq_mu_to_ws(imtU, mu_v_lin)
                mu0_ws_t = self._uq_mu_to_ws(imtU, np.array([mu0_lin_t], dtype=float))[0]
                s_ep0 = sig0_raw_t * 0.0 + float(s_ep0_t)
                s_a0 = sig0_raw_t * 0.0 + float(s_a0_t)

                # collect obs streams (dyfi_source pass-through)
                obs_pref, c_pref = self._uq_collect_obs_for_version(
                    int(v),
                    imtU,
                    measurement_sigma=measurement_sigma,
                ...
                # bayes_2lik (mixed obs + per-observation meas_var)
                if "bayes_2lik" in compute:
                    if c_mix.get("total", 0) == 0:
                        mean2, sig2, s_ep2_t = float(mu0_lin_t), float(sig0_raw_t), float(s_ep0_t)
                    else:
                ...
                # dyfi_weighted (bayes_2lik + DYFI weights)
                if "dyfi_weighted" in compute:
                    if c_mix.get("total", 0) == 0:
                        meanw, sigw, s_epw_t = float(mu0_lin_t), float(sig0_raw_t), float(s_ep0_t)
                    else:

...
class SHAKEtime:
    ...
    def uq_compute_local_metrics(
        ...
        """
        LOCAL posterior metrics over time (versions):
    
        - Mean σ (prior/post) within X km of observations
        - Mean σ (prior/post) where PoE(mu, σ, threshold) >= poe_cut
        - Mean Δσ = σ_prior - σ_post (global + local subsets)
    
        Exports into:
          export/<event_id>/uq/uq_results/
        """
        import numpy as np
        import json

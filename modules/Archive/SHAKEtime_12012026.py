...
class SHAKEtime:
    ...
    # UPDATED ONLY: EXPORT/IMPORT ROOT = export/<event_id>/uq
    # 
    # 
    #
    #
    ...
    # ---------------------------
    # XML parsing (ShakeMap grid/uncertainty)
    # ---------------------------
    ...
    def _uq_parse_grid_xml(self, xml_path):
        ...
        dx = self._uq_sm_xml_get_attrib_float(
            spec_elem,
            "nominal_lon_spacing",
            self._uq_sm_xml_get_attrib_float(spec_elem, "lon_spacing"),
        )
        dy = self._uq_sm_xml_get_attrib_float(
            spec_elem,
            "nominal_lat_spacing",
            self._uq_sm_xml_get_attrib_float(spec_elem, "lat_spacing"),
        )
        dx = float(dx)
        dy = float(dy)
    ...
    def _uq_parse_uncertainty_xml(self, xml_path):
        ...
        return spec, sigma_fields
    
    # ---------------------------
    # File resolution (matches your conventions; kwargs supported)
    # ---------------------------
    ...
    # ---------------------------
    # IMT helpers (PSA token expansion handled)
    # ---------------------------
    def uq_list_available_imts(self, version_list, stations_folder=None, rupture_folder=None):
        ...
        for v in list(version_list):
            ...
            if not grid_path.exists():
                ...
                continue
            try:
                _, fields, _ = self._uq_parse_grid_xml(grid_path)
    ...
    # ---------------------------
    # Stations/DYFI parsing (your USGSParser pattern; strong cleanup + debug)
    # ---------------------------
    def _uq_parse_stationlist_with_usgsparser(self, stationlist_json):
        """
        Uses your exact pattern:
          USGSParser("instrumented_data", json_file=...).get_dataframe(value_type="pga"/"mmi")
    
        Returns:
          inst_list, dyfi_list, debug_dict
        """
        import numpy as np
        import pandas as pd
        from pathlib import Path
        ...
        # USGSParser is assumed globally available in your class/module
        ...
        if df_pga is not None:
            ...
            if len(df_pga) > 0:
                ...
                if "longitude" not in df.columns or "latitude" not in df.columns:
                    ...
                    # Prefer a real PGA value column if present; otherwise keep location-only
                    pga_col = None
                    for c in ["pga", "PGA", "value", "amplitude", "amp"]:
                        if c in df.columns:
                            pga_col = c
                            break
                    ...
                    if pga_col is not None:
                        ...
                        for _, r in df_val.iterrows():
                            inst.append({"lon": float(r["longitude"]), "lat": float(r["latitude"]), "pga": float(r[pga_col]), "w": 1.0})
                    else:
                        for _, r in df.iterrows():
                            inst.append({"lon": float(r["longitude"]), "lat": float(r["latitude"]), "pga": None, "w": 1.0})
    
        # --- DYFI MMI ---
        try:
            df_mmi = p.get_dataframe(value_type="mmi")
        except Exception as e:
            dbg["note"] += f"|get_dataframe_mmi_failed: {e}"
            df_mmi = None
                ...
                if "longitude" not in df.columns or "latitude" not in df.columns:
                    ...
                    if "intensity" in df.columns:
                        ...
                        for _, r in df.iterrows():
                            ...
                            dyfi.append({"lon": float(r["longitude"]), "lat": float(r["latitude"]), "intensity": float(r["intensity"]), "nresp": nresp, "w": w})
    
        # final finite cleanup
        inst2 = [o for o in inst if np.isfinite(o["lon"]) and np.isfinite(o["lat"])]
        dyfi2 = [o for o in dyfi if np.isfinite(o["lon"]) and np.isfinite(o["lat"]) and np.isfinite(o["intensity"])]
    
        if dbg["pga_df_rows"] == 0 and dbg["mmi_df_rows"] == 0:
            dbg["note"] += "|both_dataframes_empty_or_none"
    ...
    # ---------------------------
    # DATASET BUILDER (stores uq_state; exports spec/axes/per-version arrays + traces + sanity)
    # ---------------------------
    def uq_build_dataset(
        ...
        for v in version_list:
            ...
            raw = {
                ...
                "grid_spec": None,
                "mean_fields": {},
                "sigma_fields": {},  # ShakeMap sigma prior (often total)
                "vs30": None,
                "lats_1d": None,
                "lons_1d": None,
            ...
            if Path(grid_path).exists():
                spec, mean_fields, vs30 = self._uq_parse_grid_xml(grid_path)
                raw["grid_spec"] = spec
            ...
            if raw["uncertainty_xml_exists"]:
                try:
                    _, sig_fields = self._uq_parse_uncertainty_xml(unc_path)
                    raw["sigma_fields"] = sig_fields
                except Exception as e:
            ...
            sanity_rows.append(
                {
                    ...
                    # station debug (audit for "empty vs filtered")
        ...
        for v in version_list:
            ...
            uv = {
                "version": v,
                "unified_mean": {},
                "unified_sigma_prior_total": {},  # name it explicitly
                "unified_vs30": None,
                "aligned_to_unified": True,
            }
    
            if raw["vs30"] is not None:
                uv["unified_vs30"] = self._uq_bilinear_interp_regular_grid(slats, slons, raw["vs30"], ULAT2, ULON2)
            else:
                uv["unified_vs30"] = np.full((unified_spec["nlat"], unified_spec["nlon"]), np.nan, dtype=float)
    
            for imt in requested:
                if imt in raw["mean_fields"]:
                    uv["unified_mean"][imt] = self._uq_bilinear_interp_regular_grid(slats, slons, raw["mean_fields"][imt], ULAT2, ULON2)
                else:
                ...
                if sig is None:
                    ...
                    if sig.shape == (raw["grid_spec"]["nlat"], raw["grid_spec"]["nlon"]):
                        uv["unified_sigma_prior_total"][imt] = self._uq_bilinear_interp_regular_grid(slats, slons, sig, ULAT2, ULON2)
                    elif sig.shape == (unified_spec["nlat"], unified_spec["nlon"]):
                        uv["unified_sigma_prior_total"][imt] = sig
                    else:
                        uv["unified_sigma_prior_total"][imt] = np.full((unified_spec["nlat"], unified_spec["nlon"]), float(np.nanmedian(sig)), dtype=float)
        ...
        self.uq_state = {
            ...
            # UPDATED ONLY: uq_state base_folder now points to export/<event_id>/uq
    ...
    # ---------------------------
    # Uncertainty decomposition: ShakeMap sigma prior vs aleatory vs epistemic
    # ---------------------------
    def _uq_decompose_sigma(
        ...
        """
        Returns: sigma_total_prior, sigma_aleatory, sigma_epistemic_prior
        """
        import numpy as np
        sig_total = np.asarray(sigma_prior_total, dtype=float)
        if sigma_aleatory is None:
            sigma_a = float(self._uq_sigma_aleatory_default(imt))
        else:
            sigma_a = float(sigma_aleatory)
    
        if sigma_total_from_shakemap:
            # sigma_ep = sqrt(max(0, total^2 - aleatory^2))
            sig_ep2 = np.maximum(0.0, sig_total * sig_total - sigma_a * sigma_a)
            sig_ep = np.sqrt(sig_ep2)
        else:
            # ShakeMap sigma is already epistemic
    ...
    # ---------------------------
    # Observation extraction for a given IMT
    # ---------------------------
    def _uq_collect_observations(self, version: int, imt: str):
        """
        Returns list of obs dicts:
          {"lat","lon","value","w","type"}
        MMI uses DYFI intensity.
        PGA uses instrumented PGA if present (and finite); otherwise none.
        """
        import numpy as np
        if not hasattr(self, "uq_state") or self.uq_state is None:
        ...
        if imt_u == "MMI":
            for o in raw["stations"]["dyfi"]:
                ...
                if val is None or (not np.isfinite(val)):
                    continue
                obs.append({"lat": float(o["lat"]), "lon": float(o["lon"]), "value": float(val), "w": float(o.get("w", 1.0)), "type": "DYFI"})
            return obs
    
        if imt_u == "PGA":
            for o in raw["stations"]["instrumented"]:
                val = o.get("pga", None)
                if val is None or (not np.isfinite(val)):
                    continue
                obs.append({"lat": float(o["lat"]), "lon": float(o["lon"]), "value": float(val), "w": float(o.get("w", 1.0)), "type": "instrumented"})
            return obs
    
        # For PGV / PSA* you can extend later if your stationlist provides those columns.
        return obs
    
    # ---------------------------
    # Bayes impact audit (sigma reduction stats)
    # ---------------------------
    def _uq_bayes_impact_audit(self, sigma_ep_prior, sigma_ep_post, obs_used: int):
        ...
        changed = d > 0
        frac = float(changed.mean()) if d.size else 0.0
        return {
            "n_obs_used": int(obs_used),
            "frac_changed": float(frac),
            "mean_drop": float(np.mean(d)),
            "median_drop": float(np.median(d)),
            "p90_drop": float(np.percentile(d, 90)),
            "max_drop": float(np.max(d)),
            "note": "" if obs_used > 0 else "no_observations_used",
        }
    
    # ---------------------------
    # Bayes update (epistemic-only, spatial influence radius + kernel)
    # ---------------------------
    ...
    def uq_bayes_update(
        ...
        sigma_aleatory=None,
        sigma_total_from_shakemap: bool = True,
        measurement_sigma=None,  # optional extra obs noise term
        ...
        results = {"method": "bayes_update", "imt": imt, "per_version": {}, "audit": {}}
        ...
        for v in versions:
            ...
            # --- prior mean (linear as stored) + prior sigma (ShakeMap uncertainty) ---
            ...
            # Decompose sigma in ShakeMap convention (sigma fields are in the sigma-space)
            ...
            # --- Convert mu to working space for correct math ---
            mu_prior = self._uq_mu_to_working_space(mu_prior_linear, imt)
    
            # Start posteriors in working space
            mu_post = np.asarray(mu_prior, dtype=float).copy()
            ...
            for o in obs:
                ...
                # Convert obs into working space
                val_o = self._uq_obs_to_working_space(val_o_lin, imt)
                if val_o is None or (not np.isfinite(val_o)):
                    continue
                ...
                # Observation variance: aleatory + measurement noise (both in working space)
                ...
                mu_sel = mu_post[mask]
                mu_post_sel = (mu_sel * prec_prior + val_o * prec_like) / np.maximum(1e-12, prec_post)
                sig_ep_post_sel = np.sqrt(1.0 / np.maximum(1e-12, prec_post))
    
                mu_post[mask] = mu_post_sel
                sig_ep_post[mask] = sig_ep_post_sel
                obs_used += 1
    
            sig_total_post = np.sqrt(np.maximum(0.0, sig_a * sig_a + sig_ep_post * sig_ep_post))
    
            audit = self._uq_bayes_impact_audit(sig_ep_prior, sig_ep_post, obs_used=obs_used)
            if obs_used > 0 and audit["max_drop"] <= 0.0:
                audit["note"] = (audit.get("note", "") + "|obs_exist_but_no_sigma_change_check_sigma_aleatory_or_radius_or_weights").strip("|")
    
            # Convert posterior mean back to linear output units
            mu_post_linear = self._uq_mu_from_working_space(mu_post, imt)
            mu_prior_linear_out = self._uq_mu_from_working_space(mu_prior, imt)
            ...
            if export:
                ...
                np.savez_compressed(
                    ...
                    sigma_total_post=sig_total_post,
                    mu_prior=np.asarray(mu_prior_linear_out, dtype=float),
                    sigma_ep_prior=sig_ep_prior,
                    sigma_total_prior=sig_total_prior,
                    sigma_aleatory=np.array([sig_a], dtype=float),
                    # Optional: include working-space mu for debugging
                    mu_post_working=np.asarray(mu_post, dtype=float),
                    mu_prior_working=np.asarray(mu_prior, dtype=float),
    ...
    def uq_bayes_update(
        ...
        # UPDATED ONLY: base already points to .../uq (because uq_state["base_folder"] is uq)
        base = Path(self.uq_state["base_folder"])
        ...
        for v in versions:
            ...
            mu_prior = np.asarray(uv["unified_mean"].get(imt, None), dtype=float)
            sig_total_prior_grid = np.asarray(uv["unified_sigma_prior_total"].get(imt, self._uq_default_sigma_for_imt(imt, mu_prior)), dtype=float)
            ...
            # For each obs: update cells in radius (distance-based weights)
            for o in obs:
                lat_o = float(o["lat"])
                lon_o = float(o["lon"])
                val_o = float(o["value"])
                ...
                if kernel == "gaussian":
                    # weight decays with distance; std chosen so that ~3σ ~= radius
                    sigma_k = max(1e-6, radius / 3.0)
                    wdist = np.exp(-0.5 * (dsel / sigma_k) ** 2)
                else:
                    wdist = np.ones_like(dsel, dtype=float)
                ...
                # observation variance: aleatory + meas (epistemic is what we update)
                sig_obs2 = (sig_a * sig_a) + (sig_meas * sig_meas)
                if sig_obs2 <= 0:
                    sig_obs2 = 1e-6
    
                # Bayesian fusion on epistemic variance (precision additivity)
                # posterior precision = prior_precision + w_eff/sig_obs2
                sig_ep2 = sig_ep_post[mask] ** 2
                prec_prior = 1.0 / np.maximum(1e-12, sig_ep2)
                prec_like = w_eff / sig_obs2
                prec_post = prec_prior + prec_like
    
                # mean update using epistemic variance only (consistent with updated epistemic precision)
                mu_sel = mu_post[mask]
                mu_post_sel = (mu_sel * prec_prior + val_o * prec_like) / np.maximum(1e-12, prec_post)
            ...
            # enforce audit non-zero if obs exist but no change -> explain why
            if obs_used > 0 and audit["max_drop"] <= 0.0:
                audit["note"] = (audit.get("note", "") + "|obs_exist_but_no_sigma_change_check_sigma_aleatory_or_radius_or_weights").strip("|")
            ...
            results["per_version"][int(v)] = {
                "sigma_aleatory": float(sig_a),
                "n_obs_used": int(obs_used),
                "mu_prior_mean": float(np.nanmean(mu_prior)),
                "mu_post_mean": float(np.nanmean(mu_post)),
                "sigma_ep_prior_mean": float(np.nanmean(sig_ep_prior)),
                "sigma_ep_post_mean": float(np.nanmean(sig_ep_post)),
                "sigma_total_prior_mean": float(np.nanmean(sig_total_prior)),
                "sigma_total_post_mean": float(np.nanmean(sig_total_post)),
    ...
    # ---------------------------
    # PoE and metrics on unified grid
    # ---------------------------
    ...
    """
    def _uq_poe_exceed(self, mu, sigma, threshold):
        import numpy as np
        mu = np.asarray(mu, dtype=float)
        sigma = np.asarray(sigma, dtype=float)
        z = (threshold - mu) / np.maximum(1e-12, sigma)
    ...
    def _uq_poe_exceed(self, mu, sigma, threshold):
        ...
        imt = None
        # Try to infer IMT from caller context if passed as arrays only: not possible reliably.
        # So: this function assumes caller passes mu in OUTPUT space and sigma in ShakeMap sigma-space,
        # and we convert mu/threshold internally if lognormal using self.uq_last_poe_imt if set.
        #
        # To keep signature unchanged, we use an attribute set by callers (below).
        if hasattr(self, "_uq_poe_imt_context"):
            imt = str(getattr(self, "_uq_poe_imt_context"))
        else:
            imt = "MMI"
        ...
        if self._uq_is_lognormal_imt(imt):
            ...
            # handle non-positive thresholds (thr_work = -inf) => PoE = 1 everywhere finite
        ...
        # normal-space IMTs (MMI)
        z = (thr - mu) / np.maximum(1e-12, sigma)
        return 1.0 - self._uq_norm_cdf(z)

        
    
    def _uq_cell_area_km2(self):
        """
        Approx cell area on lat/lon grid using mean latitude.
        """
        import numpy as np
        if not hasattr(self, "uq_state") or self.uq_state is None:
            raise RuntimeError("UQ dataset not built yet.")
        ...
        dy = float(spec["dy"])
        dx = float(spec["dx"])
        lat0 = float(spec["lat_min"])
        nlat = int(spec["nlat"])
        latc = lat0 + dy * (nlat - 1) / 2.0
        # km per degree
        km_per_deg_lat = 111.32
        km_per_deg_lon = 111.32 * np.cos(np.radians(latc))
        return abs(dx) * km_per_deg_lon * abs(dy) * km_per_deg_lat
    
    # ---------------------------
    # Monte Carlo (supports optional spatial correlation via FFT Gaussian filter)
    # ---------------------------
    def _uq_fft_gaussian_filter(self, field2d, sigma_cells):
        """
        Pure numpy FFT gaussian smoothing (no SciPy).
        sigma_cells: std dev in grid cells
        """
        import numpy as np
        Z = np.asarray(field2d, dtype=float)
        if sigma_cells <= 0:
            return Z
        ...
        # Gaussian in frequency domain: exp(-2*pi^2*sigma^2*k^2)
        H = np.exp(-2.0 * (np.pi ** 2) * (sigma_cells ** 2) * (KX ** 2 + KY ** 2))
        F = np.fft.fft2(Z)
    ...
    def uq_monte_carlo(
        ...
        sigma_aleatory=None,
        sigma_total_from_shakemap: bool = True,
        correlation_length_km: float = 0.0,  # 0 => iid
        export: bool = True,
        seed: int = 1234,
    ):
        import numpy as np
        ...
        spec = self.uq_state["unified_spec"]
        dx_deg = abs(float(spec["dx"]))
        dy_deg = abs(float(spec["dy"]))
        lat0 = float(spec["lat_min"])
        latc = lat0 + float(spec["dy"]) * (int(spec["nlat"]) - 1) / 2.0
        km_per_deg_lon = 111.32 * np.cos(np.radians(latc))
        km_per_deg_lat = 111.32
        dx_km = dx_deg * km_per_deg_lon
        dy_km = dy_deg * km_per_deg_lat
        cell_km = float(max(1e-6, 0.5 * (dx_km + dy_km)))
        sigma_cells = float(correlation_length_km / cell_km) if correlation_length_km and correlation_length_km > 0 else 0.0
        ...
        out = {
            ...
            "nsim": int(nsim),
            "which_sigma": which,
            "correlation_length_km": float(correlation_length_km),
            "per_version": {},
            "mu_unit": str(self._uq_get_imt_unit(imt)),
            "sigma_base_unit": str(self._uq_base_unit_for_sigma(imt)),
        }
    
        is_logn = bool(self._uq_is_lognormal_imt(imt))
        ...
        for v in versions:
            ...
            mu_linear = np.asarray(uv["unified_mean"].get(imt, None), dtype=float)
            sig_total_prior_grid = np.asarray(
                uv["unified_sigma_prior_total"].get(imt, self._uq_default_sigma_for_imt(imt, mu_linear)),
                dtype=float,
            )
    
            sig_total, sig_a, sig_ep = self._uq_decompose_sigma(
                sig_total_prior_grid, imt=imt, sigma_aleatory=sigma_aleatory, sigma_total_from_shakemap=sigma_total_from_shakemap
            )
            sig = sig_total if which == "total" else sig_ep
    
            # Work in correct space:
            # - lognormal: simulate in ln-space, then exp back to linear output units
            # - normal: simulate in linear space
            mu_work = self._uq_mu_to_working_space(mu_linear, imt)
    
            # accumulators in OUTPUT space (linear units users expect)
            acc = np.zeros_like(mu_linear, dtype=float)
            acc2 = np.zeros_like(mu_linear, dtype=float)
    
            for _ in range(int(nsim)):
                e = rng.standard_normal(mu_linear.shape)
                if sigma_cells > 0:
                    e = self._uq_fft_gaussian_filter(e, sigma_cells=sigma_cells)
                    es = np.nanstd(e)
                    if es > 0:
                        e = e / es
    
                if is_logn:
                    y = mu_work + sig * e               # ln-space
                    x = self._uq_mu_from_working_space(y, imt)  # -> linear output units
                else:
                    x = mu_work + sig * e               # linear
    
                acc += x
                acc2 += x * x
    
            mc_mean = acc / float(nsim)
            mc_var = np.maximum(0.0, acc2 / float(nsim) - mc_mean * mc_mean)
            mc_std = np.sqrt(mc_var)
    
            out["per_version"][int(v)] = {
                "sigma_aleatory": float(sig_a),
                "mean_of_mean": float(np.nanmean(mc_mean)),
                "mean_of_std": float(np.nanmean(mc_std)),
            }
    
            if export:
                vdir = self._uq_ensure_dir(base / f"v{int(v)}" / "uq_results")
                np.savez_compressed(
                    vdir / f"montecarlo_{imt}_{which}_nsim{int(nsim)}.npz",
                    mc_mean=mc_mean,
                    mc_std=mc_std,
                    mu=mu_linear,
                    sigma_used=sig,
        ...
        if export:
            with open(base / f"montecarlo_{imt}_{which}_summary.json", "w", encoding="utf-8") as f:
    ...
    def uq_monte_carlo(
        ...
        # UPDATED ONLY: base already points to .../uq
        base = Path(self.uq_state["base_folder"])
        ...
        dx_deg = abs(float(spec["dx"]))
        dy_deg = abs(float(spec["dy"]))
        # approximate km per degree at center
        lat0 = float(spec["lat_min"])
        latc = lat0 + float(spec["dy"]) * (int(spec["nlat"]) - 1) / 2.0
        km_per_deg_lon = 111.32 * np.cos(np.radians(latc))
        km_per_deg_lat = 111.32
        ...
        out = {"method": "monte_carlo", "imt": imt, "nsim": int(nsim), "which_sigma": which, "correlation_length_km": float(correlation_length_km), "per_version": {}}
        ...
        for v in versions:
            ...
            sig_total_prior_grid = np.asarray(uv["unified_sigma_prior_total"].get(imt, self._uq_default_sigma_for_imt(imt, mu)), dtype=float)
    
            sig_total, sig_a, sig_ep = self._uq_decompose_sigma(sig_total_prior_grid, imt=imt, sigma_aleatory=sigma_aleatory, sigma_total_from_shakemap=sigma_total_from_shakemap)
            sig = sig_total if which == "total" else sig_ep
    
            acc = np.zeros_like(mu, dtype=float)
            acc2 = np.zeros_like(mu, dtype=float)
    
            for _ in range(int(nsim)):
                e = rng.standard_normal(mu.shape)
                if sigma_cells > 0:
                    e = self._uq_fft_gaussian_filter(e, sigma_cells=sigma_cells)
                    es = np.nanstd(e)
                    if es > 0:
    ...
    # ---------------------------
    # Hierarchical pooling (pool across versions -> update on pooled field)
    # ---------------------------
    def uq_hierarchical(
        self,
        version_list=None,
        ...
        # pool priors across versions on unified grid (in WORKING space for mu if lognormal)
        mu_list = []
        sig_ep_list = []
        sig_a_val = None
        ...
        for v in versions:
            ...
            sig_a_val = sig_a if sig_a_val is None else sig_a_val
    
            mu_list.append(mu_work)
            sig_ep_list.append(sig_ep)
    
        if not mu_list:
            raise RuntimeError("[UQ] No versions available for hierarchical pooling.")
    
        MU = np.stack(mu_list, axis=0)       # (V,nlat,nlon) in working space (ln for lognormal)
        SIGE = np.stack(sig_ep_list, axis=0) # (V,nlat,nlon) in sigma-space
    
        mu_pool = np.nanmean(MU, axis=0)
        within = np.nanmean(SIGE ** 2, axis=0)
        between = np.nanvar(MU, axis=0)
        sig_ep_pool = np.sqrt(np.maximum(1e-12, within + between))
    
        # collect pooled observations across versions (union)
        obs_all = []
        for v in versions:
            obs_all.extend(self._uq_collect_observations(int(v), imt))
        ...
        mu_post = mu_pool.copy()
        sig_ep_post = sig_ep_pool.copy()
        obs_used = 0
    
        for o in obs_all:
            lat_o = float(o["lat"])
            lon_o = float(o["lon"])
            val_lin = o.get("value", None)
            if val_lin is None:
                continue
    
            val_o = self._uq_obs_to_working_space(val_lin, imt)
            if val_o is None or (not np.isfinite(val_o)):
                continue
            ...
            sig_obs2 = float(sig_a_val) ** 2
            prec_prior = 1.0 / np.maximum(1e-12, sig_ep_post[mask] ** 2)
            prec_like = w_eff / np.maximum(1e-12, sig_obs2)
            prec_post = prec_prior + prec_like
        ...
        sig_total_post = np.sqrt(np.maximum(0.0, (float(sig_a_val) ** 2) + sig_ep_post ** 2))
        sig_total_prior = np.sqrt(np.maximum(0.0, (float(sig_a_val) ** 2) + sig_ep_pool ** 2))
    
        audit = self._uq_bayes_impact_audit(sig_ep_pool, sig_ep_post, obs_used=obs_used)
    
        # Convert pooled/posterior mu back to linear output units
        mu_pool_lin = self._uq_mu_from_working_space(mu_pool, imt)
        mu_post_lin = self._uq_mu_from_working_space(mu_post, imt)
        ...
        out = {
            ...
            "versions": versions,
            "n_obs_used": int(obs_used),
            "sigma_aleatory": float(sig_a_val),
            "audit": audit,
        }
    
        if export:
            pdir = self._uq_ensure_dir(base / "uq_results")
            np.savez_compressed(
                pdir / f"hierarchical_{imt}_posterior.npz",
                mu_pool=np.asarray(mu_pool_lin, dtype=float),
                sigma_ep_pool=sig_ep_pool,
                sigma_total_pool=sig_total_prior,
                mu_post=np.asarray(mu_post_lin, dtype=float),
                sigma_ep_post=sig_ep_post,
                sigma_total_post=sig_total_post,
                sigma_aleatory=np.array([float(sig_a_val)], dtype=float),
                # optional working-space mu
                mu_pool_working=np.asarray(mu_pool, dtype=float),
                mu_post_working=np.asarray(mu_post, dtype=float),
                mu_unit=str(self._uq_get_imt_unit(imt)),
                sigma_base_unit=str(self._uq_base_unit_for_sigma(imt)),
            )
    
            with open(pdir / f"hierarchical_{imt}_audit.json", "w", encoding="utf-8") as f:
                json.dump(audit, f, indent=2)
    
            with open(pdir / f"hierarchical_{imt}_summary.json", "w", encoding="utf-8") as f:
                json.dump(out, f, indent=2)
    
        self.uq_last_hier = out
        return out
    ...
    def uq_hierarchical(
        ...
        # pool priors across versions on unified grid
        mu_list = []
        sig_ep_list = []
        sig_a_val = None
        ...
        MU = np.stack(mu_list, axis=0)          # (V,nlat,nlon)
        SIGE = np.stack(sig_ep_list, axis=0)    # (V,nlat,nlon)
    
        mu_pool = np.nanmean(MU, axis=0)
        # combine epistemic: within-version + between-version
        within = np.nanmean(SIGE ** 2, axis=0)
        between = np.nanvar(MU, axis=0)
        sig_ep_pool = np.sqrt(np.maximum(1e-12, within + between))
    ...
    # ---------------------------
    # X-axis helper for plots (version vs time-after-event)
    # ---------------------------
    def _uq_make_x(
        self,
        version_list,
        x_axis: str = "version",  # version|TaE
        tae_hours=None,           # optional list aligned with version_list
        tae_seconds=None,         # optional list aligned with version_list
    ):
        import numpy as np
        x_axis = str(x_axis).strip()
        versions = [int(v) for v in list(version_list)]
        if x_axis.lower() == "tae":
            if tae_seconds is not None:
                x = np.asarray(tae_seconds, dtype=float)
                xlabel = "TaE (s)"
            elif tae_hours is not None:
                x = np.asarray(tae_hours, dtype=float)
                xlabel = "TaE (h)"
            else:
                x = np.arange(len(versions), dtype=float)
                xlabel = "TaE (index)"
        else:
            x = np.asarray(versions, dtype=float)
            xlabel = "Version"
        return x, xlabel


    # ============================================================
    # LOCAL UQ PATCH (FIX OUTPUT ROOT + LOCAL METRICS + MAPS)
    # PASTE AFTER YOUR GLOBAL UQ PART INSIDE SHAKEtime CLASS
    # ============================================================
    
    def _uq_uqdir(self):
        """
        Canonical UQ root directory (NEVER creates uq/uq):
    
          export/<event_id>/uq
    
        Rules:
        - If uq_state["base_folder"] already ends with ".../uq", use it as-is.
        - Else append "/uq".
        """
        from pathlib import Path
        if not hasattr(self, "uq_state") or self.uq_state is None:
    ...
    def _uq_results_dir(self, version: int = None):
        ...
        if version is None:
            ...
            p = uq / f"v{int(version)}" / "uq_results"
        p.mkdir(parents=True, exist_ok=True)
        return p
    ...
    def _uq_load_bayes_posterior_npz(self, version: int, imt: str):
        """
        Robust loader for BayesUpdate posteriors.
        Looks in canonical folders (and legacy fallbacks).
        ...
        candidates = [
            ...
            # older “base/uq/...” mistake patterns
    ...
    def _uq_load_hierarchical_posterior_npz(self, imt: str):
        ...
        candidates = [
            uq / "uq_results" / f"hierarchical_{imt}_posterior.npz",  # canonical
            base / "uq_results" / f"hierarchical_{imt}_posterior.npz",  # legacy
            base / "uq" / "uq_results" / f"hierarchical_{imt}_posterior.npz",  # legacy mistake
        ]
        for p in candidates:
            try:
                if p.exists():
    ...
    def _uq_collect_obs_coords_all(self, version: int, imt: str):
        """
        Returns (lats, lons, weights) for observations used for local masks.
        Uses your existing _uq_collect_observations().
        """
        import numpy as np
    ...
    def _uq_mask_within_km_of_points(self, lat2d, lon2d, pts_lat, pts_lon, radius_km: float):
        """
        Boolean mask: True where ANY obs point is within radius_km.
        """
        import numpy as np
        if pts_lat.size == 0:
    ...
    def _uq_prior_sigma_fields(self, version: int, imt: str):
        """
        Returns:
          sig_ep_prior, sig_total_prior, sig_aleatory_scalar
        """
        import numpy as np
        ...
        if method == "bayesupdate":
            ...
            if dat is None:
                raise FileNotFoundError(
                    f"Missing Bayes posterior for v{v}, imt={imt}. "
                    f"Run uq_bayes_update(export=True)."
                )
    ...
    def uq_compute_local_metrics(
        ...
        # Set PoE context (signature unchanged)
        ...
        for v in versions:
            ...
            poe = self._uq_poe_exceed(mu, sig_post, float(poe_threshold))
    ...
    def uq_plot_shakemap_sigma_and_data_map(
        ...
        version: int,
        imt: str = "MMI",
        which_sigma: str = "total",     # "total" (USGS) or "epistemic" (your decomposition)
        station_radius_km: float = 0.0, # if >0, also draw an "influence mask" outline
        cmap: str = "viridis",
        vmin=None,
        vmax=None,
        show_obs: bool = True,
        ...
        """
        QA MAP (single version):
          - Background: ShakeMap prior uncertainty σ (from uncertainty.xml, optionally decomposed)
          - Overlay: observation points used in that version (instrumented or DYFI depending on IMT)
          - Text box: counts (n_instrumented, n_dyfi)
          - Optional: station_radius_km outline (cells within R km of any obs)
        """
        import numpy as np
        import matplotlib.pyplot as plt
        import cartopy.crs as ccrs
        ...
        # --- obs (for overlay + optional radius outline) ---
        pts_lat, pts_lon, _w = self._uq_collect_obs_coords_all(v, imt)
        ...
        # Optional: outline cells within R km of any obs (useful to see “footprint”)
        ...
        cbar.set_label(f"ShakeMap σ ({which})")
    ...
    def uq_plot_shakemap_sigma_change_map(
        self,
        imt: str = "MMI",
        which_sigma: str = "total",      # "total" (USGS) or "epistemic"
        v_from: int = None,              # first version
        v_to: int = None,                # second version
        mask_to_data_radius_km: float = 0.0,  # if >0, show change only within R km of obs (from v_to)
        cmap: str = "viridis",
        vmin=None,
        vmax=None,
        show_obs: bool = True,
        ...
        QA MAP (two versions):
          Δσ = σ(v_from) − σ(v_to)
        Positive means uncertainty DECREASED from v_from to v_to.
    
        Optional mask:
          Only display Δσ within R km of observations (from v_to), to focus on “data-relevant” areas.
        """
        import numpy as np
        import matplotlib.pyplot as plt
        import cartopy.crs as ccrs
        ...
        d = np.asarray(s1 - s2, dtype=float)  # positive => decreased
    
        axes = self.uq_state["unified_axes"]
        LAT2 = np.asarray(axes["lat2d"], dtype=float)
        LON2 = np.asarray(axes["lon2d"], dtype=float)
    
        # optional mask to data footprint (use obs from v_to by default)
        ...
        cbar = fig.colorbar(pm, ax=ax, shrink=0.85, pad=0.02)
        cbar.set_label(f"Δσ = σ(v{v_from}) − σ(v{v_to})  ({which})")
    ...
    def _uq_save_figure(
        ...
        """
        Save helper consistent with other SHAKEtime functions.
    
        Priority:
          1) if output_path provided -> output_path/SHAKEtime/<event_id>/uq/<subdir>/
          2) else -> export/<event_id>/uq/uq_plots/<subdir>/
        """
        from pathlib import Path
    
        if output_path:
            out_dir = Path(output_path) / "SHAKEtime" / self.event_id / "uq"
            if subdir:
                out_dir = out_dir / subdir
        else:
            out_dir = self._uq_plots_dir(subdir) if subdir else self._uq_plots_dir("")
    ...
    def _uq_cartopy_axes(
        ...
        """
        Create a Cartopy GeoAxes styled similarly to your other SHAKEmaps plots.
        """
        import matplotlib.pyplot as plt
        import cartopy.crs as ccrs
        import cartopy.feature as cfeature
        ...
        if add_gridlines:
            ...
            # match your other patterns: no top/right labels
            try:
                gl.top_labels = False
                gl.right_labels = False
    ...
    # ------------------------------------------------------------
    # 1) GLOBAL UNCERTAINTY DECAY (now supports multi-curves)
    # ------------------------------------------------------------
    def uq_plot_uncertainty_decay(
        ...
        show: bool = True,
        # new overlay options
        overlay_other_sigma: bool = True,   # if True, also plot the other sigma (total vs epistemic) on same axes
    ):
        import numpy as np
        import matplotlib.pyplot as plt
        from pathlib import Path
        ...
        for v in versions:
            uv = self.uq_state["versions_unified"].get(int(v), None)
            if uv is None:
                y_main.append(np.nan)
                y_other.append(np.nan)
                continue
    
            mu = np.asarray(uv["unified_mean"].get(imt, None), dtype=float)
            sig_total_prior_grid = np.asarray(
                uv["unified_sigma_prior_total"].get(imt, self._uq_default_sigma_for_imt(imt, mu)), dtype=float
            )
            sig_total_prior, sig_a, sig_ep_prior = self._uq_decompose_sigma(
                sig_total_prior_grid, imt=imt, sigma_aleatory=None, sigma_total_from_shakemap=True
            )
            ...
            if method == "bayesupdate":
                p = Path(self.uq_state["base_folder"]) / f"v{int(v)}" / "uq_results" / f"bayesupdate_{imt}_posterior.npz"
                if p.exists():
                    dat = np.load(p, allow_pickle=True)
                    sig_total = dat["sigma_total_post"]
                    sig_ep = dat["sigma_ep_post"]
    
            elif method == "hierarchical":
                p = Path(self.uq_state["base_folder"]) / "uq_results" / f"hierarchical_{imt}_posterior.npz"
                if p.exists():
                    dat = np.load(p, allow_pickle=True)
                    sig_total = dat["sigma_total_post"]
                    sig_ep = dat["sigma_ep_post"]
    ...
    # ------------------------------------------------------------
    # 2) AREA WHERE PoE >= cut (styling + multi-curve option)
    # ------------------------------------------------------------
    def uq_plot_metric_area_ge_threshold(
        ...
        for v in versions:
            uv = self.uq_state["versions_unified"].get(int(v), None)
            if uv is None:
                areas_main.append(np.nan)
                areas_other.append(np.nan)
                continue
            ...
            sig_total_prior, sig_a, sig_ep_prior = self._uq_decompose_sigma(sig_total_prior_grid, imt=imt, sigma_aleatory=None, sigma_total_from_shakemap=True)
    ...
    # ------------------------------------------------------------
    # 3) MEAN PoE (styling + multi-curve option)
    # ------------------------------------------------------------
    def uq_plot_metric_mean_poe(
        ...
        for v in versions:
            uv = self.uq_state["versions_unified"].get(int(v), None)
            if uv is None:
                mpoe_main.append(np.nan)
                mpoe_other.append(np.nan)
                continue
    ...
    # ------------------------------------------------------------
    # 4) LOCAL UNCERTAINTY DECAY (now supports multiple curves)
    # ------------------------------------------------------------
    def uq_plot_local_uncertainty_decay(
        ...
        if include_prior:
            ...
            ax.plot(x, y_prior_global, marker=None, linestyle=":", label="Prior: global mean σ")
    ...
    # ------------------------------------------------------------
    # 5) SIGMA REDUCTION MAP (Cartopy basemap + lon/lat coords)
    # ------------------------------------------------------------
    def uq_plot_sigma_reduction_map(
        self,
        version: int,
        imt: str = "MMI",
        which_sigma: str = "epistemic",   # epistemic|total
        ...
        show: bool = True,
        plot_colorbar: bool = True,
        cbar_label: str = "Δσ = σ_prior − σ_post",
    ):
        """
        Cartopy map of sigma reduction on the unified grid:
          Δσ = σ_prior - σ_post
        """
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        d = np.asarray(sig_prior - sig_post, dtype=float)
        ...
        if show_title:
            ax.set_title(f"{method} Δσ map — {imt} ({which}) — v{v}", fontsize=14, pad=10)
        ...
        if save:
            self._uq_save_figure(
                fig,
                fname_stem=f"{method}_sigma_reduction_{which}_{imt}_v{v}",
                subdir="uq_plots/sigma_reduction",
                output_path=output_path,
                save_formats=save_formats,
                dpi=dpi,
            )
    ...
    # ------------------------------------------------------------
    # 6) BATCH EXPORT SIGMA REDUCTION MAPS (supports save_formats)
    # ------------------------------------------------------------
    def uq_export_sigma_reduction_maps(
        ...
        for v in versions:
            self.uq_export_sigma_reduction_maps(
                version=int(v),
                imt=imt,
                which_sigma=which,
                method=method,
    ...
    #
    #
    # UQ VERSION-CHANGE + DECOMPOSITION PATCH
    # 
    # ============================================================
    
    def uq_compute_uncertainty_change_first_to_final(
        self,
        imt: str = "MMI",
        which_sigma: str = "epistemic",      # "epistemic"|"total"
        posterior_method: str = "bayesupdate",  # "bayesupdate"|"hierarchical"
        ...
        """
        Compute first->final uncertainty change maps and a decomposition:
    
        A) Posterior evolution:
            Δσ_post = σ_post(first) - σ_post(final)
    
        B) Prior evolution (model / procedure evolution):
            Δσ_prior = σ_prior(first) - σ_prior(final)
    
        C) Update-effect evolution (data impact evolution):
            reduction(v) = σ_prior(v) - σ_post(v)
            Δreduction = reduction(final) - reduction(first)
    
        Identity (by construction):
            Δσ_post = Δσ_prior - Δreduction
    
        Interpretation:
          - Positive Δσ_post  => uncertainty DECREASED from first to final
          - Positive Δσ_prior => prior uncertainty DECREASED over versions (model/procedure matured)
          - Positive Δreduction => observations reduce MORE in final than first (data impact grew)
        """
        import numpy as np
        ...
        imt = str(imt)
        which = str(which_sigma).strip().lower()
        method = str(posterior_method).strip().lower()
    
        versions = [int(v) for v in self.uq_state["version_list"]]
        v1 = int(versions[0] if first_version is None else first_version)
        vN = int(versions[-1] if final_version is None else final_version)
    
        # --- priors (per-version) ---
        sig_ep_prior_1, sig_total_prior_1, _ = self._uq_prior_sigma_fields(v1, imt)
        sig_ep_prior_N, sig_total_prior_N, _ = self._uq_prior_sigma_fields(vN, imt)
    
        sig_prior_1 = sig_total_prior_1 if which == "total" else sig_ep_prior_1
        sig_prior_N = sig_total_prior_N if which == "total" else sig_ep_prior_N
    
        # --- posteriors (per-version) ---
        # if hierarchical: same pooled posterior for all versions; still computable but Δσ_post will be ~0
        sig_ep_post_1, sig_total_post_1 = self._uq_posterior_sigma_fields(v1, imt, method=method)
        sig_ep_post_N, sig_total_post_N = self._uq_posterior_sigma_fields(vN, imt, method=method)
    
        sig_post_1 = sig_total_post_1 if which == "total" else sig_ep_post_1
        sig_post_N = sig_total_post_N if which == "total" else sig_ep_post_N
    
        # --- maps ---
        d_post = np.asarray(sig_post_1 - sig_post_N, dtype=float)     # posterior evolution
        d_prior = np.asarray(sig_prior_1 - sig_prior_N, dtype=float)  # prior evolution
    
        red_1 = np.asarray(sig_prior_1 - sig_post_1, dtype=float)     # update effect at first
        red_N = np.asarray(sig_prior_N - sig_post_N, dtype=float)     # update effect at final
        d_reduction = np.asarray(red_N - red_1, dtype=float)          # change in update effect
    
        # sanity identity: d_post ≈ d_prior - d_reduction (numerical tolerance)
        residual = np.asarray(d_post - (d_prior - d_reduction), dtype=float)
        ...
        out = {
            ...
            "first_version": v1,
            "final_version": vN,
            "delta_sigma_post": d_post,
            "delta_sigma_prior": d_prior,
            "delta_reduction": d_reduction,
            "identity_residual": residual,
            # optional reference fields
            "sigma_prior_first": np.asarray(sig_prior_1, dtype=float),
            "sigma_prior_final": np.asarray(sig_prior_N, dtype=float),
            "sigma_post_first": np.asarray(sig_post_1, dtype=float),
            "sigma_post_final": np.asarray(sig_post_N, dtype=float),
        }
    
        if export:
            # save into your standard UQ results root
            rdir = self._uq_results_dir(None)
            np.savez_compressed(
                rdir / f"version_change_{method}_{imt}_{which}_v{v1}_to_v{vN}.npz",
                delta_sigma_post=d_post,
                delta_sigma_prior=d_prior,
                delta_reduction=d_reduction,
                identity_residual=residual,
                sigma_prior_first=out["sigma_prior_first"],
                sigma_prior_final=out["sigma_prior_final"],
                sigma_post_first=out["sigma_post_first"],
                sigma_post_final=out["sigma_post_final"],
            )
    
        self.uq_last_version_change = out
        return out
    
    
    def uq_plot_uncertainty_change_first_to_final_map(
        self,
        imt: str = "MMI",
        which_sigma: str = "epistemic",
        posterior_method: str = "bayesupdate",
        kind: str = "post",  # "post"|"prior"|"reduction"|"residual"
        first_version: int = None,
        final_version: int = None,
        cmap: str = "viridis",
        vmin=None,
        ...
        """
        Cartopy map for first->final uncertainty changes.
    
        kind:
          - "post":     Δσ_post = σ_post(first) - σ_post(final)
          - "prior":    Δσ_prior = σ_prior(first) - σ_prior(final)
          - "reduction":Δreduction = (σ_prior-σ_post)(final) - (σ_prior-σ_post)(first)
          - "residual": identity residual: Δσ_post - (Δσ_prior - Δreduction)  (should be ~0)
        """
        import numpy as np
        import matplotlib.pyplot as plt
        import cartopy.crs as ccrs
        ...
        if kind == "post":
            Z = np.asarray(dat["delta_sigma_post"], dtype=float)
            cbar_label = "Δσ_post = σ_post(first) − σ_post(final)"
        elif kind == "prior":
            Z = np.asarray(dat["delta_sigma_prior"], dtype=float)
            cbar_label = "Δσ_prior = σ_prior(first) − σ_prior(final)"
        elif kind == "reduction":
            Z = np.asarray(dat["delta_reduction"], dtype=float)
            cbar_label = "Δreduction = (σ_prior−σ_post)(final) − (σ_prior−σ_post)(first)"
        elif kind == "residual":
            Z = np.asarray(dat["identity_residual"], dtype=float)
            cbar_label = "Residual: Δσ_post − (Δσ_prior − Δreduction)"
        else:
            raise ValueError("kind must be 'post', 'prior', 'reduction', or 'residual'")
        ...
        # obs overlay: use FINAL version obs locations by default
        if show_obs:
            vN = int(dat["final_version"])
            pts_lat, pts_lon, _w = self._uq_collect_obs_coords_all(vN, str(imt))
            if pts_lat.size > 0:
                ax.scatter(
                    pts_lon, pts_lat,
                    s=float(obs_size),
        ...
        if show_title:
            ax.set_title(
                f"{posterior_method} {kind} change — {imt} ({which_sigma}) — v{dat['first_version']}→v{dat['final_version']}",
                fontsize=14,
                pad=10,
            )
        ...
        if save:
            self._uq_save_figure(
                fig,
                fname_stem=f"{posterior_method}_uncertainty_change_{kind}_{which_sigma}_{imt}_v{dat['first_version']}_to_v{dat['final_version']}",
    ...
    def uq_plot_net_uncertainty_decomposition_first_to_final(
        ...
        One figure with 3 panels (Cartopy) summarizing the decomposition:
          ...
          Panel 1: Δσ_post       (net posterior evolution)
          Panel 2: Δσ_prior      (model/procedure evolution)
          Panel 3: Δreduction    (data/update-effect evolution)
    
        All are first->final quantities:
          Δσ_post = σ_post(first) - σ_post(final)
          Δσ_prior = σ_prior(first) - σ_prior(final)
          Δreduction = reduction(final) - reduction(first)
        """
        ...
        for ax in (ax1, ax2, ax3):
            # reuse your styling helper
            self._uq_cartopy_axes(fig=fig, ax=ax, figsize=None)
            self._uq_set_map_extent_from_axes(ax)
    ...
    # ============================================================
    # 12b) ONE FIGURE: COMBINED UNCERTAINTY DECAY (prior+bayes+hier+MC)
    # 
    # ============================================================
    
    def _uq_find_montecarlo_file(self, version: int, imt: str, which: str, nsim: int = None):
        """
        Find a MonteCarlo NPZ for a given version/imt/which.
    
        Expected canonical naming from uq_monte_carlo():
          v<version>/uq_results/montecarlo_<imt>_<which>_nsim<NSIM>.npz
    
        If nsim is None, picks the file with the largest NSIM (best quality),
        and if tie, the newest mtime.
        """
        import re
        from pathlib import Path
        ...
        uq = self._uq_uqdir() if hasattr(self, "_uq_uqdir") else Path(self.uq_state["base_folder"])
        vdir = uq / f"v{v}" / "uq_results"
        if not vdir.exists():
            return None
    
        pattern = re.compile(rf"^montecarlo_{re.escape(imt)}_{re.escape(which)}_nsim(\d+)\.npz$", re.IGNORECASE)
        candidates = []
        for p in vdir.glob(f"montecarlo_{imt}_{which}_nsim*.npz"):
        ...
        if nsim is not None:
            nsim = int(nsim)
            exact = [c for c in candidates if c[1] == nsim]
            if exact:
                # newest among exact
                exact.sort(key=lambda t: t[2], reverse=True)
                return exact[0][0]
            return None
    
        # pick largest nsim, then newest
        candidates.sort(key=lambda t: (t[1] if t[1] is not None else -1, t[2]), reverse=True)
        return candidates[0][0]
    
    
    def uq_plot_uncertainty_decay_compare(
        self,
        imt: str = "MMI",
        which: str = "epistemic",  # "epistemic"|"total"
        ...
        show: bool = True,
        # MC controls
        mc_nsim: int = None,  # if None, auto-pick best available per version
    ):
        """
        One publishing-ready figure overlaying multiple uncertainty-decay curves.
    
        Curves supported:
          - prior: mean σ from unified ShakeMap sigma (decomposed to epistemic/total)
          - bayesupdate: mean σ_post from exported bayesupdate_<imt>_posterior.npz
          - hierarchical: mean σ_post from exported hierarchical_<imt>_posterior.npz (pooled; same for all versions)
          - montecarlo: mean(mc_std) from exported montecarlo_<imt>_<which>_nsim*.npz per version
    
        Notes:
          - hierarchical produces a single pooled field; we plot the same mean for each x (so it’s a flat line).
          - montecarlo plots the *propagated* uncertainty (mc_std), not the analytic sigma field.
        """
        import numpy as np
        import matplotlib.pyplot as plt
        from pathlib import Path
        ...
        imt = str(imt)
        which = str(which).strip().lower()
        methods = [str(m).strip().lower() for m in list(methods)]
        ...
        # -------- helpers to get sigma fields ----------
        def _prior_mean_sigma(v: int):
            uv = self.uq_state["versions_unified"].get(int(v), None)
            if uv is None:
                return float("nan")
            mu = np.asarray(uv["unified_mean"].get(imt, None), dtype=float)
            ...
            field = sig_ep if which == "epistemic" else sig_total
            return float(np.nanmean(field))
    
        def _bayes_mean_sigma(v: int):
            # prefer robust loader if present
            dat = None
            if hasattr(self, "_uq_load_bayes_posterior_npz"):
                dat = self._uq_load_bayes_posterior_npz(int(v), imt)
            if dat is None:
            ...
            if dat is None:
                return float("nan")
            field = dat["sigma_ep_post"] if which == "epistemic" else dat["sigma_total_post"]
            return float(np.nanmean(np.asarray(field, dtype=float)))
    
        def _hier_mean_sigma():
            dat = None
            if hasattr(self, "_uq_load_hierarchical_posterior_npz"):
                dat = self._uq_load_hierarchical_posterior_npz(imt)
            if dat is None:
                p = Path(self.uq_state["base_folder"]) / "uq_results" / f"hierarchical_{imt}_posterior.npz"
                if p.exists():
        ...
        def _mc_mean_sigma(v: int):
            p = self._uq_find_montecarlo_file(int(v), imt, which, nsim=mc_nsim)
            if p is None:
                return float("nan")
            z = np.load(p, allow_pickle=True)
            # uq_monte_carlo saves: mc_std, mc_mean, mu, sigma_used
            if "mc_std" not in z.files:
                return float("nan")
            return float(np.nanmean(np.asarray(z["mc_std"], dtype=float)))
    
        # -------- compute curves ----------
        curves = {}  # name -> list y
        if "prior" in methods:
            curves["prior"] = [_prior_mean_sigma(v) for v in versions]
        if "bayesupdate" in methods:
            curves["bayesupdate"] = [_bayes_mean_sigma(v) for v in versions]
        if "hierarchical" in methods:
            yh = _hier_mean_sigma()
            curves["hierarchical"] = [yh for _ in versions]
        if "montecarlo" in methods:
            curves["montecarlo"] = [_mc_mean_sigma(v) for v in versions]
    
        # -------- plot ----------
        fig, ax = plt.subplots(figsize=figsize)
    
        # simple style mapping (kept minimal—users can tune later)
        style = {
            "prior": dict(marker="o", linestyle="-"),
            "bayesupdate": dict(marker="s", linestyle="--"),
            "hierarchical": dict(marker="^", linestyle="-."),
            "montecarlo": dict(marker="D", linestyle=":"),
        }
    
        for name, y in curves.items():
            kw = style.get(name, {})
            ax.plot(x, y, label=name, **kw)
    
        ax.set_xlabel(xlabel, fontsize=font_sizes["labels"])
        ax.set_ylabel(f"Mean σ ({which})", fontsize=font_sizes["labels"])
        ax.tick_params(axis="both", labelsize=font_sizes["ticks"])
        ax.tick_params(axis="x", labelrotation=xrotation)
        ...
        if show_title:
            extra = " (log y)" if ylog else ""
            ax.set_title(f"Uncertainty decay comparison — {imt} — {which}{extra}", fontsize=font_sizes["title"])
    
        fig.tight_layout()
    
        if save:
            meth_tag = "-".join([m for m in ["prior", "bayesupdate", "hierarchical", "montecarlo"] if m in curves])
            self._uq_save_figure(
                fig,
                fname_stem=f"compare_uncertainty_decay_{imt}_{which}_{meth_tag}_ylog{int(bool(ylog))}",
                subdir="uq_plots",
                output_path=output_path,
                save_formats=save_formats,
                dpi=dpi,
    ...
    #
    #
    # UQ v26.4 HELPER FUNCTIONS UPGRADE PATCH 
    # 
    # 
    # ============================================================


    def uq_set_imt_units(
        self,
        pga_unit: str = None,     # "%g" | "g" | "m/s^2"
        pgv_unit: str = None,     # "cm/s" | "m/s"
        psa_unit: str = None,     # "%g" | "g" | "m/s^2"
    ):
        """
        Optional: set preferred output/input units for IMTs in the UQ framework.
    
        - Affects:
            * Bayes/hierarchical exported mu_post units
            * Monte Carlo output units (mc_mean/mc_std)
            * PoE thresholds interpreted in these units
        - Does NOT change how station values are parsed/collected; conversions happen internally.
    ...
    def _uq_default_imt_unit(self, imt: str) -> str:
        """
        Default units as typically stored in ShakeMap grid.xml:
          - PGA/PSA: %g
          - PGV: cm/s
          - MMI: MMI
        """
    ...
    def _uq_get_imt_unit(self, imt: str) -> str:
        """
        Returns preferred unit if user set via uq_set_imt_units(), else default.
        """
        imt_u = str(imt).upper()
        if hasattr(self, "uq_units") and isinstance(self.uq_units, dict):
            # PSA inherits from PSA key if present, else PGA if present
    ...
    def _uq_base_unit_for_sigma(self, imt: str) -> str:
        """
        Base units matching ShakeMap uncertainty.xml sigma conventions:
          - PGA/PSA: ln(g)
          - PGV: ln(cm/s)
          - MMI: normal-space (MMI units)
        """
        imt_u = str(imt).upper()
        if imt_u == "PGV":
            return "cm/s"
    ...
    def _uq_convert_units(self, x, imt: str, from_unit: str, to_unit: str):
        """
        Convert scalar/array x between a small set of supported IMT units.
        Supported:
          - PGA/PSA: "%g" <-> "g" <-> "m/s^2"
          - PGV: "cm/s" <-> "m/s"
          - MMI: no conversion
    ...
    def _uq_mu_to_working_space(self, mu_linear, imt: str):
        ...
        # Convert from preferred/output unit -> base sigma unit, then ln
        from_unit = self._uq_get_imt_unit(imt_u)
    ...
    def _uq_mu_to_working_space(self, mu_linear, imt: str):
        """
        Convert mean field into the working space used for probabilistic math:
          - lognormal IMTs (PGA/PGV/PSA): ln( value_in_base_unit )
          - MMI: unchanged
    
        IMPORTANT:
          mu_linear is assumed to be in the *source/default* units from ShakeMap grid.xml,
          NOT in user-preferred output units.
        """
        import numpy as np
        ...
        # ShakeMap grid.xml typical units (source units)
        #from_unit = self._uq_default_imt_unit(imt_u)
        from_unit = self._uq_default_imt_unit(imt_u)
    
        # Base units that match ShakeMap sigma convention (g for PGA/PSA, cm/s for PGV)
        base_unit = self._uq_base_unit_for_sigma(imt_u)
    ...
    """
    v26.1
    def _uq_obs_to_working_space(self, obs_value_linear, imt: str):
        import numpy as np
    ...
    def _uq_obs_to_working_space(self, obs_value_linear, imt: str):
        """
        Convert a single observation value into working space (same rules as mu).
    
        IMPORTANT:
          stationlist values are assumed to be in the *source/default* units
          (same convention as ShakeMap outputs), not in user-preferred output units.
        """
        import numpy as np
    ...
    def _uq_threshold_to_working_space(self, threshold, imt: str):
        ...
        if tb <= 0:
            # if threshold <= 0, exceedance is trivially 1 for lognormal; handle upstream
            return float("-inf")
        return float(np.log(tb))
    """
    def _uq_threshold_to_working_space(self, threshold, imt: str):
        """
        Convert a threshold into working space (same rules as mu).
    
        IMPORTANT:
          threshold is interpreted in the *current output unit choice* only if the caller
          is explicitly treating thresholds as user-facing.
          For safety/consistency, we interpret thresholds in user-preferred units *only*
          at the moment of conversion here, but convert to base-unit ln-space correctly.
        """
        import numpy as np
        ...
        # Here it's OK to treat thresholds as user-facing numbers:
        from_unit = self._uq_get_imt_unit(imt_u)
        base_unit = self._uq_base_unit_for_sigma(imt_u)
    ...
    def _uq_mu_from_working_space(self, mu_working, imt: str):
        """
        Convert working-space mean back to linear in the user's chosen output unit.
        - lognormal: exp(mu_working in base unit) then convert base -> preferred
        - normal: unchanged
        """
        import numpy as np
    ...
    # ======================================================================
    # PATCH: Target-based time-evolving prediction + uncertainty decay curves
    #        (point / area; ShakeMap-published vs v1-prior-updated methods)
    #        + GLOBAL mean/median + delta(global) support
    #        + Robust helper-signature wrappers (mu + sigma + obs conversion)
    #        + Observation filtering aligned with SHAKEtime UQ conventions
    #        + Audit export (CSV + JSON) after extraction / plotting
    #        + NEW: ShakeMap sigma_total is RAW from uncertainty.xml (no decomposition)
    #        + NEW: Validation utilities: raw XML extraction vs unified grid values
    #
    # Drop this block INSIDE the SHAKEtime class (replace the whole section).
    # Relies on top-of-file imports already present in SHAKEtime.py (np/pd/plt/Path/os/ET/etc).
    # ======================================================================
    ...
    def _uq_mu_to_ws(self, imt, mu_linear):
        ...
        try:
            return fn(mu_linear, imt)   # current SHAKEtime
        except Exception:
            return fn(imt, mu_linear)   # fallback
    ...
    def _uq_obs_to_ws(self, imt, y_linear):
        ...
        try:
            return float(fn(y_linear, imt))   # common: (y, imt)
        except Exception:
            return float(fn(imt, y_linear))   # fallback: (imt, y)
    ...
    def _uq_decompose_sigma_safe(
        ...
        IMPORTANT:
        - Always returns arrays shaped like sigma_total (broadcasts scalars).
        - This is used ONLY for epistemic/aleatory bookkeeping for update methods.
        - ShakeMap's plotted sigma_total is RAW and does NOT use this.
    ...
    # 2) Target parsing
    # ----------------------------------------------------------------------
    def _uq_parse_targets(self, points=None, areas=None):
        """
    ...
    # ----------------------------------------------------------------------
    # 4) Observations (station/DYFI) collection (aligned with SHAKEtime UQ)
    # ----------------------------------------------------------------------
    def _uq_collect_obs_for_version(self, version, imt, measurement_sigma=0.30, include_weights=True):
        """
        Collect observations available for a given version/imt.
        Prefer SHAKEtime's existing _uq_collect_observations if present.
        Returns list of dicts: {"lat","lon","y_ws","w","type"}.
        """
        imtU = str(imt).upper()
        obs = []
    ...
    def _uq_save_figure_safe(self, fig, fname_stem, subdir=None, output_path=None, save_formats=("png", "pdf"), dpi=300):
        """
        Prefer SHAKEtime's _uq_save_figure if present, else do a safe save into output_path/subdir.
        """
        ...
        for ext in save_formats:
            ext = str(ext).lstrip(".")
            fig.savefig(root / f"{fname_stem}.{ext}", dpi=dpi, bbox_inches="tight")
    
    
    # ----------------------------------------------------------------------
    # 7) RAW ShakeMap XML extraction (truth baseline for validation)
    # ----------------------------------------------------------------------
    def _uq_find_shakemap_xml(self, version, which="grid", base_folder=None, shakemap_folder=None):
        """
        Find grid.xml / uncertainty.xml for a version.
        Tries:
          - uq_state["base_folder"]/v{version}/... (recursive)
          - base_folder argument (recursive)
          - shakemap_folder argument (recursive)
        ...
        patterns = []
        if which == "grid":
            patterns = ["*grid.xml", "*_grid.xml"]
        else:
            patterns = ["*uncertainty.xml", "*_uncertainty.xml"]
        ...
        if hasattr(self, "uq_state") and isinstance(self.uq_state, dict):
            ...
            if bf:
                # version folder often like .../uq/v8 or .../uq/v08
        ...
        for root in search_roots:
            try:
                if not root.exists():
                    continue
                for pat in patterns:
                    # filter by version in name if possible (loose)
                    hits = list(root.rglob(pat))
                    if not hits:
                        continue
                    # Prefer files with version-like token (v08/us_008 etc)
                    scored = []
                    for p in hits:
                        name = p.name.lower()
                        score = 0
                        if f"_{str(v).zfill(3)}_" in name or f"_{str(v).zfill(3)}" in name:
                            score += 3
    ...
    def _uq_parse_gridxml_to_arrays(self, grid_xml_path):
        """
        Parse ShakeMap grid.xml or uncertainty.xml that contains:
          - grid_spec (nlat, nlon, lat_min/max, lon_min/max)
          - grid_field mapping (index -> name)
          - grid_data text rows (nlat*nlon lines)
        Returns dict:
          {
            "nlat","nlon","lat_min","lat_max","lon_min","lon_max",
            "lat_vec","lon_vec","fields": {name: col_index},
        ...
        # grid_spec is usually an element with attributes
        ...
        # Use true spacing (not nominal) for consistent indexing
        ...
        for gf in root.findall(".//grid_field"):
            try:
                ...
                # In files, indices are often 1-based; grid_data columns are 0-based in arrays
                # We'll store 0-based column index.
                fields[name] = idx - 1
            except Exception:
                continue
        ...
        # Sanity: expected rows = nlat*nlon
        if arr.shape[0] != nlat * nlon:
            # still return but warn via exception-like behavior
            # (some products can include headers; try to trim to last nlat*nlon rows)
            if arr.shape[0] > nlat * nlon:
                arr = arr[-(nlat * nlon):, :]
            else:
                raise RuntimeError(
                    f"grid_data row count mismatch in {p.name}: got {arr.shape[0]} expected {nlat*nlon}"
                )
    ...
    def _uq_raw_nearest_from_xml(self, grid_xml_path, uncertainty_xml_path, lat, lon, want=("MMI", "PGA", "STDMMI", "STDPGA")):
        """
        Extract nearest-node values from raw XML files.
        Returns dict with:
          nearest_km, nearest_lat, nearest_lon,
          values: {MMI,PGA,PGV,PSA03,STDMMI,STDPGA,...} as available.
        """
        ...
        # Nearest indices using vectors
        # Lat vector is increasing from lat_min to lat_max; same for lon.
        j = int(np.argmin(np.abs(G["lon_vec"] - lon)))
        i = int(np.argmin(np.abs(G["lat_vec"] - lat)))
    
        nearest_lat = float(G["lat_vec"][i])
        nearest_lon = float(G["lon_vec"][j])
    
        # row-major: i*nlon + j
        idx = i * G["nlon"] + j
    
        # distance (km)
        nearest_km = float(self._uq_haversine_km(lat, lon, nearest_lat, nearest_lon))
        ...
        for k in want:
            kU = str(k).upper()
            if kU.startswith("STD"):
                out[kU] = _get_val(U, kU)
            else:
                out[kU] = _get_val(G, kU)
    
        return out
    
    
    # ----------------------------------------------------------------------
    # 8) Core extraction: per-target series (RAW ShakeMap sigma_total)
    # ----------------------------------------------------------------------
    def uq_extract_target_series(
        ...
        areas=None,
        agg="mean",
        global_stat=None,  # None|"mean"|"median"|"delta mean"|"delta median"
        # IMPORTANT: ShakeMap sigma_total is RAW from uncertainty.xml (no decomposition)
        shakemap_total_sigma_mode="raw",  # kept for future; currently "raw" is enforced
        ...
        """
        Build tidy DataFrame of per-target, per-version predictions + uncertainties.
    
        METHODS:
          - "ShakeMap": mean from ShakeMap grid, sigma_total = RAW uncertainty grid (STD*)
          - "bayes","hierarchical","kriging","montecarlo": use v0 prior (mu0 + sigma_ep0)
            and obs available in each version.
    
        GLOBAL MODE:
          If global_stat in {"mean","median","delta mean","delta median"}, a single global target is created:
            target_id="GLOBAL_<stat>", target_type="global"
          The agg used becomes that global_stat.
    
        Output columns (core):
          target_id, target_type, version, method,
          mean, sigma_total, sigma_total_raw, sigma_epistemic, sigma_aleatoric,
          n_obs, n_cells
    
        Notes:
          - sigma_total_raw is ALWAYS the raw uncertainty grid aggregation for ShakeMap and for the prior version at the target.
          - For update methods, sigma_total is reconstructed from (sigma_a0_t, sigma_ep_post_t).
        """
        if interp_kwargs is None:
            interp_kwargs = {}
        ...
        if global_stat_eff is not None:
            #targets = [{"id": f"GLOBAL_{global_stat_eff.replace(' ', '_')}".replace("__", "_"), "type": "global"}]
            gid = f"GLOBAL_{str(global_stat_eff).replace(' ', '_')}"
        ...
        # decompose prior sigma (ONLY for update methods: sigma_a0 + sigma_ep0)
        s_tot0_dummy, s_a0, s_ep0 = self._uq_decompose_sigma_safe(
            imtU,
            sig0_raw,
            sigma_aleatory=sigma_aleatory,
            sigma_total_from_shakemap=bool(sigma_total_from_shakemap),
        ...
        # helper: global mask
        def _global_mask(lat2d):
            return np.isfinite(lat2d)
        ...
        for t in targets:
            ...
            if n_cells <= 0:
                for v in versions:
                    for method in ("ShakeMap", "bayes", "hierarchical", "montecarlo", "kriging"):
                        rows.append({
                            ...
                            "sigma_total_raw": float("nan"),
                            "sigma_epistemic": float("nan"),
                            "sigma_aleatoric": float("nan"),
                            "n_obs": 0,
                            "n_cells": 0,
                        })
                continue
    
            # prior target scalars
            ...
            # baseline for delta modes (applied after row creation)
            base_sig_raw = float(sig0_raw_t)
            base_mu = float(mu0_lin_t)
    
            for v in versions:
                # ShakeMap published (mean + RAW sigma_total)
                ...
                mu_pub = self._uq_agg(mu_v_lin[mask], agg=agg_effective)
                sig_pub_raw = self._uq_agg(sig_v_raw[mask], agg=agg_effective)
    
                # (optional) decomposition for bookkeeping only (do not overwrite raw)
                _s_tot_v, s_a_v, s_ep_v = self._uq_decompose_sigma_safe(
                    imtU,
                    sig_v_raw,
                    sigma_aleatory=sigma_aleatory,
                    sigma_total_from_shakemap=bool(sigma_total_from_shakemap),
                )
                sig_a_pub = self._uq_agg(s_a_v[mask], agg=agg_effective)
                sig_ep_pub = self._uq_agg(s_ep_v[mask], agg=agg_effective)
                ...
                rows.append({
                    ...
                    "method": "ShakeMap",
                    "mean": float(mu_pub),
                    # IMPORTANT: RAW total sigma from uncertainty.xml
                    "sigma_total": float(sig_pub_raw),
                    "sigma_total_raw": float(sig_pub_raw),
                    "sigma_epistemic": float(sig_ep_pub),
                    "sigma_aleatoric": float(sig_a_pub),
                    "n_obs": 0,
                    "n_cells": int(n_cells),
                })
    
                # observations for this version
                obs = self._uq_collect_obs_for_version(int(v), imtU, measurement_sigma=measurement_sigma, include_weights=True)
                n_obs = len(obs)
    
                # If no obs: update methods = prior (v0-based)
                if n_obs == 0:
                    for method in ("bayes", "hierarchical", "montecarlo", "kriging"):
                        rows.append({
                            "target_id": t.get("id", "GLOBAL"),
                            "target_type": t.get("type", "global" if t.get("type") == "global" else "target"),
                            "version": int(v),
                            "method": method,
                            "mean": float(mu0_lin_t),
                            # keep RAW prior for verification consistency
                            "sigma_total": float(sig0_raw_t),
                            "sigma_total_raw": float(sig0_raw_t),
                            "sigma_epistemic": float(s_ep0_t),
                            "sigma_aleatoric": float(s_a0_t),
                            "n_obs": 0,
                            "n_cells": int(n_cells),
                        })
                    continue
                ...
                mu_b_t = self._uq_agg(mu_b_lin[mask], agg=agg_effective)
                s_ep_b_t = self._uq_agg(s_ep_b[mask], agg=agg_effective)
                s_tot_b_t = float(np.sqrt(max(0.0, s_a0_t**2 + s_ep_b_t**2)))
                ...
                rows.append({
                    ...
                    "method": "bayes",
                    "mean": float(mu_b_t),
                    "sigma_total": float(s_tot_b_t),
                    "sigma_total_raw": float(sig0_raw_t),
                    "sigma_epistemic": float(s_ep_b_t),
                    "sigma_aleatoric": float(s_a0_t),
                    "n_obs": int(n_obs),
                    "n_cells": int(n_cells),
                })
                ...
                mu_h_t = self._uq_agg(mu_h_lin[mask], agg=agg_effective)
                s_ep_h_t = self._uq_agg(s_ep_h[mask], agg=agg_effective)
                s_tot_h_t = float(np.sqrt(max(0.0, s_a0_t**2 + s_ep_h_t**2)))
                ...
                rows.append({
                    ...
                    "method": "hierarchical",
                    "mean": float(mu_h_t),
                    "sigma_total": float(s_tot_h_t),
                    "sigma_total_raw": float(sig0_raw_t),
                    "sigma_epistemic": float(s_ep_h_t),
                    "sigma_aleatoric": float(s_a0_t),
                    "n_obs": int(n_obs),
                    "n_cells": int(n_cells),
                })
                ...
                mu_k_t = self._uq_agg(mu_k_lin[mask], agg=agg_effective)
                s_ep_k_t = self._uq_agg(s_ep_k[mask], agg=agg_effective)
                s_tot_k_t = float(np.sqrt(max(0.0, s_a0_t**2 + s_ep_k_t**2)))
                ...
                rows.append({
                    ...
                    "method": "kriging",
                    "mean": float(mu_k_t),
                    "sigma_total": float(s_tot_k_t),
                    "sigma_total_raw": float(sig0_raw_t),
                    "sigma_epistemic": float(s_ep_k_t),
                    "sigma_aleatoric": float(s_a0_t),
                    "n_obs": int(n_obs),
                    "n_cells": int(n_cells),
                })
                ...
                mu_ref = float(mu_b_t)
                sig_ref = float(s_tot_b_t) if mc_include_aleatory else float(s_ep_b_t)
                ...
                rows.append({
                    ...
                    "method": "montecarlo",
                    "mean": float(mu_mc),
                    "sigma_total": float(sig_tot_mc),
                    "sigma_total_raw": float(sig0_raw_t),
                    "sigma_epistemic": float(sig_ep_mc),
                    "sigma_aleatoric": float(s_a0_t),
                    "n_obs": int(n_obs),
                    "n_cells": int(n_cells),
                })
    
                # Audit row (per target/version)
                if audit:
                    audit_rows.append({
                        "target_id": t.get("id", "GLOBAL"),
                        "target_type": t.get("type", "global" if t.get("type") == "global" else "target"),
                        ...
                        "mask_meta": str(meta)[:500],
                        "note": "ShakeMap sigma_total is RAW (uncertainty.xml). Update methods use decomposed prior for sigma_ep.",
                    })
    
            # If delta global: apply deltas against baseline v0 for that global target
            if (t.get("type") == "global") and (global_stat_eff in ("delta mean", "delta median")):
                # create extra delta columns by post-processing in plotter (simpler)
                pass
        ...
        # audit write
        if audit:
            pref = audit_prefix or f"UQ-TargetAudit-{imtU}"
    ...
    # ----------------------------------------------------------------------
    # 9) Plotting: target decay curves (now ShakeMap sigma_total is RAW)
    # ----------------------------------------------------------------------
    def uq_plot_targets_decay(
        ...
        points=None,
        areas=None,
        what="sigma_total",  # "mean"|"sigma_total"|"sigma_epistemic"|"sigma_aleatoric"|"sigma_total_raw"|"delta mean"|"delta median"
        ...
        """
        Plot per-target decay curves and optionally a combined figure.
    
        NOTE:
          - method="ShakeMap" -> sigma_total is RAW (uncertainty.xml) by design.
          - Use what="sigma_total_raw" if you want to explicitly plot the raw column.
          - For global_stat="delta mean"/"delta median", what can be "delta mean"/"delta median"
            to plot delta of sigma_total relative to baseline prior version.
        """
        if legend_kwargs is None:
            legend_kwargs = {}
        ...
        if wl in ("delta mean", "delta median"):
            # interpret as delta of sigma_total relative to baseline version (per target & method)
        ...
        for tid in targets:
            ...
            for m in methods:
                ...
                ax.plot(s["version"].values, s[w].values, marker="o", linewidth=1.8, label=m)
            ...
            if legend:
                if legend_size is not None:
                    ...
                    legend_kwargs.setdefault("fontsize", float(legend_size))
                ax.legend(**legend_kwargs)
        ...
        if plot_combined and len(targets) > 1:
            ...
            if show_title:
                ax.set_title(title if title is not None else f"Combined target decay ({str(imt).upper()} / {w})",
                             fontsize=title_size)
    ...
    # ----------------------------------------------------------------------
    # 10) Plotting: mean + sigma panels
    # ----------------------------------------------------------------------
    def uq_plot_targets_mean_and_sigma(
        self,
        version_list,
        imt="MMI",
        points=None,
        ...
        targets = sorted(df["target_id"].unique().tolist())
        panels = [
            ("mean", f"{str(imt).upper()}"),
            ("sigma_total", "sigma_total (ShakeMap RAW)"),
            ("sigma_epistemic", "sigma_epistemic"),
        ]
    
        for tid in targets:
            sub = df[(df["target_id"] == tid) & (df["method"].isin(methods))].copy().sort_values(["version", "method"])
            fig, axes = plt.subplots(3, 1, figsize=figsize, dpi=dpi, sharex=True)
    
            for ax, (col, yl) in zip(axes, panels):
                for m in methods:
                    s = sub[sub["method"] == m].sort_values("version")
                    ax.plot(s["version"].values, s[col].values, marker="o", linewidth=1.8, label=m)
                ax.set_ylabel(yl)
                ax.grid(True, which="both", alpha=0.3)
                if col.startswith("sigma") and ylog_sigma:
                    ax.set_yscale("log")
                if tick_size is not None:
                    ax.tick_params(axis="both", labelsize=float(tick_size))
                if label_size is not None:
            ...
            axes[-1].set_xlabel(str(xlabel))
            axes[-1].tick_params(axis="x", rotation=float(xrotation))
            if label_size is not None:
                axes[-1].xaxis.label.set_size(float(label_size))
            ...
            if show_title:
                if title is not None:
                    ...
                    fig.suptitle(f"Target series @ {tid} ({str(imt).upper()})", fontsize=title_size)
            ...
            if save and output_path is not None:
                self._uq_save_figure_safe(
                    fig,
                    fname_stem=f"UQ-TargetSeries-{str(imt).upper()}-{tid}",
                    subdir="uq_plots/target_series",
                    output_path=output_path,
                    save_formats=save_formats,
                    dpi=dpi,
                )
        ...
        # combined only if multiple targets
        if plot_combined and len(targets) > 1:
            ...
            for ax, (col, yl) in zip(axes, panels):
                for tid in targets:
                    ...
                    for m in methods:
                        s = sub[sub["method"] == m].sort_values("version")
                        ax.plot(s["version"].values, s[col].values, marker="o", linewidth=1.2, label=f"{tid} | {m}")
                ax.set_ylabel(yl)
                ax.grid(True, which="both", alpha=0.3)
                if col.startswith("sigma") and ylog_sigma:
                    ax.set_yscale("log")
            ...
            if legend_size is not None:
                lk.setdefault("fontsize", float(legend_size))
            axes[0].legend(ncol=2, **lk)
    
            if show_title:
                fig.suptitle(title if title is not None else f"Combined target series ({str(imt).upper()})",
                             fontsize=title_size)
            ...
            if save and output_path is not None:
                self._uq_save_figure_safe(
                    fig,
                    fname_stem=f"UQ-TargetSeries-COMBINED-{str(imt).upper()}",
                    subdir="uq_plots/target_series",
                    output_path=output_path,
                    save_formats=save_formats,
                    dpi=dpi,
    ...
    # ----------------------------------------------------------------------
    # 11) Validation tool: raw XML vs unified grid values (MMI/PGA + STDs)
    #     + SAFE plotting/export guards (prevents KeyError when no rows_err)
    # ----------------------------------------------------------------------
    def uq_validate_unified_vs_raw(
        self,
        version_list,
        points,
        imts=("MMI", "PGA"),
        ...
        """
        Robust cross-check:
          For each version and point:
            1) Extract raw nearest-node values from XML:
               - MMI, PGA
               - STDMMI, STDPGA
            2) Extract unified-grid values at nearest unified cell:
               - unified mean (MMI/PGA)
               - unified sigma_raw (STD*)
            3) Compute errors:
               err = unified - raw
               abs_err, rel_err (where possible)
    
        Returns:
          df_values, df_errors
    
        Notes:
          - If raw XML files cannot be found for ALL versions, df_errors will be empty.
            This is now handled safely (no KeyError); plotting will be skipped with a clear message.
        """
        if interp_kwargs is None:
            interp_kwargs = {}
        ...
        targets = self._uq_parse_targets(points=points, areas=None)
    
        # Build unified axes once (we only need lat/lon)
        _, lat2d, lon2d = self._uq_get_unified_for_versions(
            versions,
            imt=str(imts[0]).upper(),
            grid_res=grid_res,
            interp_method=interp_method,
            interp_kwargs=interp_kwargs,
        )
        ...
        for imt in imts:
            imtU = str(imt).upper()
            want.append(imtU)
            want.append(f"STD{imtU}")
    
        for v in versions:
            # locate raw xml files
            grid_p = self._uq_find_shakemap_xml(
                v, which="grid", base_folder=base_folder, shakemap_folder=shakemap_folder
            )
            unc_p = self._uq_find_shakemap_xml(
                v, which="uncertainty", base_folder=base_folder, shakemap_folder=shakemap_folder
            )
    
            # If missing raw XML: record and continue (no errors possible)
                    ...
                    rows_val.append(
                        {
                            ...
                            "lat": float(t["lat"]),
                            "lon": float(t["lon"]),
                            "nearest_km_raw": np.nan,
                            "nearest_lat_raw": np.nan,
                            "nearest_lon_raw": np.nan,
                            "nearest_ij_unified": None,
                            "imt": None,
                            "raw_mean": np.nan,
                            "raw_sigma": np.nan,
                            "unified_mean": np.nan,
                            "unified_sigma": np.nan,
                            "raw_grid": str(grid_p) if grid_p else None,
                            "raw_uncertainty": str(unc_p) if unc_p else None,
                            "note": "Missing raw XML file(s).",
                        }
                    )
                continue
    
            # unified values per IMT (compute per IMT for this version)
            unified = {}
            for imt in imts:
                imtU = str(imt).upper()
                ...
                unified[imtU] = (mu_u, sig_u)
    
            for t in targets:
                # raw nearest
                raw = self._uq_raw_nearest_from_xml(
                    grid_p, unc_p, t["lat"], t["lon"], want=want
                )
    
                # unified nearest mask (nearest unified cell)
                mask, meta = self._uq_target_mask(
                    {"id": t["id"], "type": "point", "lat": t["lat"], "lon": t["lon"]},
                    lat2d,
                    lon2d,
                )
                ij = meta.get("ij", None)
    
                # collect unified at that cell
                for imt in imts:
                    imtU = str(imt).upper()
                    mu_u, sig_u = unified[imtU]
    
                    mu_uni = float(np.asarray(mu_u, dtype=float)[mask][0])
                    sig_uni = float(np.asarray(sig_u, dtype=float)[mask][0])
    
                    # raw values
                    raw_mu = float(raw.get(imtU, np.nan))
                    raw_sig = float(raw.get(f"STD{imtU}", np.nan))
    
                    # errors
                    err_mu = (
                        mu_uni - raw_mu
                        if (np.isfinite(mu_uni) and np.isfinite(raw_mu))
                        else np.nan
                    )
                    err_sig = (
                        sig_uni - raw_sig
                        if (np.isfinite(sig_uni) and np.isfinite(raw_sig))
                        else np.nan
                    )
                    ...
                    rows_val.append(
                        {
                            ...
                            "lat": float(t["lat"]),
                            "lon": float(t["lon"]),
                            "nearest_km_raw": float(raw.get("nearest_km", np.nan)),
                            "nearest_lat_raw": float(raw.get("nearest_lat", np.nan)),
                            "nearest_lon_raw": float(raw.get("nearest_lon", np.nan)),
                            "nearest_ij_unified": str(ij),
                            "imt": imtU,
                            "raw_mean": raw_mu,
                            "raw_sigma": raw_sig,
                            "unified_mean": mu_uni,
                            "unified_sigma": sig_uni,
                            "raw_grid": str(grid_p),
                            "raw_uncertainty": str(unc_p),
                            "note": "",
                        }
                    )
                    ...
                    rows_err.append(
                        {
                            ...
                            "param": "mean",
                            "error": err_mu,
                            "abs_error": np.abs(err_mu) if np.isfinite(err_mu) else np.nan,
                            "rel_error": (
                                (err_mu / raw_mu)
                                if (np.isfinite(err_mu) and np.isfinite(raw_mu) and raw_mu != 0)
                                else np.nan
                            ),
                        }
                    )
                    rows_err.append(
                        {
                            ...
                            "param": "sigma",
                            "error": err_sig,
                            "abs_error": np.abs(err_sig) if np.isfinite(err_sig) else np.nan,
                            "rel_error": (
                                (err_sig / raw_sig)
                                if (np.isfinite(err_sig) and np.isfinite(raw_sig) and raw_sig != 0)
                                else np.nan
                            ),
                        }
                    )
    
        # ---- Build DataFrames (safe even if empty) ----
        df_values = pd.DataFrame(rows_val)
        df_errors = pd.DataFrame(rows_err)
        ...
        if export_tables and output_path is not None:
            outp = Path(output_path)
            outp.mkdir(parents=True, exist_ok=True)
            df_values.to_csv(outp / f"{export_prefix}-values.csv", index=False)
            df_errors.to_csv(outp / f"{export_prefix}-errors.csv", index=False)
    
        # Plots: error vs version (guard against empty df_errors)
        if make_plots:
            if df_errors.empty or ("target_id" not in df_errors.columns):
                # Count missing-raw flags for quick diagnosis
                n_missing = 0
                if (not df_values.empty) and ("note" in df_values.columns):
                    n_missing = int(
                        df_values["note"]
                        .fillna("")
                        .str.contains("Missing raw XML", case=False)
                        .sum()
                    )
                print(
                    "[uq_validate_unified_vs_raw] No error rows produced; skipping plots.\n"
                    "Likely cause: raw XML files were not found for the requested versions.\n"
                    f"Rows flagged as missing raw XML in df_values: {n_missing}\n"
                    "Fix: pass base_folder=... or shakemap_folder=... so _uq_find_shakemap_xml can locate grid.xml/uncertainty.xml."
                )
            else:
                for tid in df_errors["target_id"].dropna().unique():
                    sub_tid = df_errors[df_errors["target_id"] == tid]
                    for imtU in sub_tid["imt"].dropna().unique():
                        sub_imt = sub_tid[sub_tid["imt"] == imtU]
                        for param in ("mean", "sigma"):
                            s = sub_imt[sub_imt["param"] == param].sort_values("version")
                            if s.empty:
                                continue
                            fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
                            ax.plot(
                            ...
                            ax.set_ylabel(f"unified - raw ({imtU} {param})")
                            ax.tick_params(axis="x", rotation=float(xrotation))
                            ax.set_title(f"Raw vs unified error @ {tid} — {imtU} {param}")
                            fig.tight_layout()
    
                            if save and output_path is not None:
                                self._uq_save_figure_safe(
                                    fig,
                                    fname_stem=f"{export_prefix}-Err-{tid}-{imtU}-{param}",
                                    subdir="uq_plots/validation_raw_vs_unified",
                                    output_path=output_path,
                                    save_formats=save_formats,
                                    dpi=dpi,
                                )
                            plt.show() if True else plt.close(fig)
    
        return df_values, df_errors
    
    
    # ============================================================
    # UQ VALIDATION: RAW GRID vs UNIFIED GRID (targets/points)
    # ============================================================
    
    def uq_validate_raw_vs_unified(
        self,
        version_list: list,
        points: list,
        imts: list = ("MMI", "PGA"),
        *,
        # Sampling behavior
        raw_sample_method: str = "nearest",      # "nearest" | "bilinear"
        unified_sample_method: str = "nearest",  # "nearest" | "bilinear"
        # Units / naming
        sigma_prefix: str = "STD",               # STDMMI, STDPGA, ...
        # Outputs
        output_path: str = None,
        ...
        """
        Compare point-sampled RAW ShakeMap grids vs corresponding UNIFIED-grid values for:
          - mean IMT (e.g., MMI, PGA)
          - sigma_total (from uncertainty.xml STD* fields)
    
        Outputs:
          - pandas.DataFrame with raw/unified values + abs/rel errors
          - optional CSV export
          - optional diagnostic plots
    
        Invariants:
          - sigma_total MUST come from raw ShakeMap uncertainty grid (STD* fields).
          - No decomposition/recombination for sigma_total.
        """
        import os
        import numpy as np
        import pandas as pd
        ...
        if points is None or len(points) == 0:
            raise ValueError("points must be a non-empty list of dicts with at least {'id','lat','lon'}.")
    
        version_list = [int(v) for v in (version_list or [])]
        if not version_list:
        ...
        for i, p in enumerate(points):
            if not isinstance(p, dict):
                raise TypeError(f"points[{i}] must be a dict.")
            pid = p.get("id", f"P{i+1}")
            lat = p.get("lat", None)
            lon = p.get("lon", None)
            if lat is None or lon is None:
                raise ValueError(f"points[{i}] missing 'lat' or 'lon'.")
            norm_points.append({"id": str(pid), "lat": float(lat), "lon": float(lon)})
    
        # Unified dataset: try to use existing uq_state if present; else attempt build
        unified = getattr(self, "uq_state", None)
        if unified is None:
            try:
                unified = self.uq_build_dataset(
                    version_list=version_list,
                    imts=list(imts),
                    build_uncertainty=True,
                    interp_method=getattr(self, "uq_interp_method", "nearest"),
                    use_cache=True,
                )
            except TypeError:
                # fallback if uq_build_dataset signature differs
                unified = self.uq_build_dataset(
                    version_list=version_list,
                    imts=list(imts),
                    export=False,
        ...
        for v in version_list:
            # RAW parse (grid.xml + uncertainty.xml) via wrappers you added
            raw_grid = self._uq__get_raw_grid_for_version(v)
            raw_unc = self._uq__get_raw_unc_for_version(v)
    
            for p in norm_points:
                for imt in imts:
                    imtU = str(imt).upper()
                    sigma_field = f"{sigma_prefix}{imtU}"
    
                    raw_mean = self._uq__sample_raw_field(
                        raw_grid, field=imtU, lat=p["lat"], lon=p["lon"], method=raw_sample_method
                    )
                    raw_sigma = self._uq__sample_raw_field(
                        raw_unc, field=sigma_field, lat=p["lat"], lon=p["lon"], method=raw_sample_method
                    )
    
                    uni_mean = self._uq__sample_unified_field(
                        unified, version=v, field=imtU, lat=p["lat"], lon=p["lon"], method=unified_sample_method
                    )
                    uni_sigma = self._uq__sample_unified_field(
                        unified, version=v, field=sigma_field, lat=p["lat"], lon=p["lon"], method=unified_sample_method
                    )
    
                    # Errors
                    mean_abs = np.nan if (raw_mean is None or uni_mean is None) else float(abs(uni_mean - raw_mean))
                    sig_abs = np.nan if (raw_sigma is None or uni_sigma is None) else float(abs(uni_sigma - raw_sigma))
    
                    mean_rel = np.nan
                    if raw_mean not in (None, 0, 0.0) and uni_mean is not None and np.isfinite(raw_mean):
                        mean_rel = float((uni_mean - raw_mean) / raw_mean)
    
                    sig_rel = np.nan
                    if raw_sigma not in (None, 0, 0.0) and uni_sigma is not None and np.isfinite(raw_sigma):
                        sig_rel = float((uni_sigma - raw_sigma) / raw_sigma)
                    ...
                    rows.append(
                        {
                            ...
                            "abs_err_mean": mean_abs,
                            "rel_err_mean": mean_rel,
                            "raw_sigma_total": raw_sigma,
                            "unified_sigma_total": uni_sigma,
                            "abs_err_sigma_total": sig_abs,
                            "rel_err_sigma_total": sig_rel,
                            "raw_sample_method": str(raw_sample_method),
                            "unified_sample_method": str(unified_sample_method),
                        }
                    )
        ...
        if export_csv:
            if output_path is None:
                base = getattr(self, "export_folder", None) or getattr(self, "output_folder", None) or "export"
                output_path = os.path.join(base, "uq_validation_raw_vs_unified.csv")
        ...
        if make_plots:
            for imt in imts:
                imtU = str(imt).upper()
                sub = df[df["imt"] == imtU].copy()
                if len(sub) == 0:
                    continue
    
                plt.figure(dpi=dpi)
                plt.scatter(sub["raw_mean"], sub["unified_mean"])
                plt.xlabel(f"RAW {imtU}")
                plt.ylabel(f"UNIFIED {imtU}")
                plt.title(f"RAW vs UNIFIED mean — {imtU}")
                ...
                plt.figure(dpi=dpi)
                plt.scatter(sub["raw_sigma_total"], sub["unified_sigma_total"])
                plt.xlabel(f"RAW {sigma_prefix}{imtU}")
                plt.ylabel(f"UNIFIED {sigma_prefix}{imtU}")
                plt.title(f"RAW vs UNIFIED sigma_total — {imtU}")
                plt.grid(True, alpha=0.3)
                if show_plots:
                    plt.show()
                plt.close()
            ...
            for imt in imts:
                imtU = str(imt).upper()
                sub = df[df["imt"] == imtU].copy()
                for pid in sorted(sub["point_id"].dropna().unique()):
                    sp = sub[sub["point_id"] == pid]
                    if sp.empty:
                        continue
                    plt.figure(dpi=dpi)
                    plt.plot(sp["version"], sp["abs_err_sigma_total"], marker="o")
                    plt.xlabel("version")
                    plt.ylabel(f"|Δ sigma_total| ({sigma_prefix}{imtU})")
                    plt.title(f"Abs sigma_total mismatch over versions — {pid} — {imtU}")
                    plt.grid(True, alpha=0.3)
    ...
    # -----------------------------
    # Helpers: RAW access + sampling
    # -----------------------------
    
    def _uq__get_raw_grid_for_version(self, version):
        """
        Return parsed RAW grid container for a version.
        Must expose lat/lon arrays + fields for IMTs.
        This wrapper tries common parser entry points used in SHAKEtime.
        """
        if hasattr(self, "parse_grid_xml"):
            return self.parse_grid_xml(version=version)
        if hasattr(self, "shakemap_parse_grid"):
            return self.shakemap_parse_grid(version=version)
        if hasattr(self, "SHAKEparser") and hasattr(self.SHAKEparser, "parse_grid_xml"):
            return self.SHAKEparser.parse_grid_xml(version=version)
        raise AttributeError(
            "No grid.xml parser found (expected parse_grid_xml/shakemap_parse_grid/SHAKEparser.parse_grid_xml)."
        )
    
    
    def _uq__get_raw_unc_for_version(self, version):
        """
        Return parsed RAW uncertainty container for a version.
        Must expose lat/lon arrays + fields STD*.
        """
        if hasattr(self, "parse_uncertainty_xml"):
            return self.parse_uncertainty_xml(version=version)
        if hasattr(self, "shakemap_parse_uncertainty"):
            return self.shakemap_parse_uncertainty(version=version)
        if hasattr(self, "SHAKEparser") and hasattr(self.SHAKEparser, "parse_uncertainty_xml"):
            return self.SHAKEparser.parse_uncertainty_xml(version=version)
        raise AttributeError(
            "No uncertainty.xml parser found (expected parse_uncertainty_xml/shakemap_parse_uncertainty/SHAKEparser.parse_uncertainty_xml)."
        )
    
    
    def _uq__sample_raw_field(self, raw_container, field: str, lat: float, lon: float, method: str = "nearest"):
        """
        Sample a field from a RAW parsed container at lat/lon.
        raw_container is expected to allow:
          - raw_container['lats'], raw_container['lons'], raw_container['fields'][field]
            OR attribute equivalents.
        """
        import numpy as np
        ...
        if isinstance(raw_container, dict):
            lats = raw_container.get("lats", raw_container.get("lat", None))
            lons = raw_container.get("lons", raw_container.get("lon", None))
            fields = raw_container.get("fields", raw_container.get("data", raw_container))
        else:
            lats = getattr(raw_container, "lats", getattr(raw_container, "lat", None))
            lons = getattr(raw_container, "lons", getattr(raw_container, "lon", None))
            fields = getattr(raw_container, "fields", getattr(raw_container, "data", raw_container))
        ...
        if isinstance(fields, dict):
            grid = fields.get(field, None)
            if grid is None:
                grid = fields.get(str(field).upper(), None)
        else:
            try:
                grid = raw_container[field]
            except Exception:
                try:
                    grid = raw_container[str(field).upper()]
                except Exception:
                    grid = None
        ...
        return self._uq__sample_on_grid(lats, lons, grid, lat, lon, method=method)
    
    
    def _uq__sample_unified_field(self, unified, version, field: str, lat: float, lon: float, method: str = "nearest"):
        """
        Sample a field from the UNIFIED dataset at lat/lon.
    
        IMPORTANT: This remains a *generic* accessor. If your uq_build_dataset stores unified
        arrays differently, adjust this accessor only (keep validation logic intact).
        """
        import numpy as np
        ...
        # (A) If you passed self.uq_state from uq_build_dataset (recommended structure from your patch)
        if isinstance(unified, dict) and "versions_unified" in unified:
            lat2d = unified.get("lat2d", unified.get("unified_axes", {}).get("lat2d", None))
            lon2d = unified.get("lon2d", unified.get("unified_axes", {}).get("lon2d", None))
            if lat2d is not None and lon2d is not None:
                # Use nearest cell on the 2D meshes
                lat2d = np.asarray(lat2d, dtype=float)
                lon2d = np.asarray(lon2d, dtype=float)
                d2 = (lat2d - float(lat)) ** 2 + (lon2d - float(lon)) ** 2
                iy, ix = np.unravel_index(np.nanargmin(d2), d2.shape)
    
                vblock = unified["versions_unified"].get(int(version), None)
                if isinstance(vblock, dict):
                    # Accept common names used in your uq_state: unified_mean / unified_sigma_prior_total
                    fU = str(field).upper()
                    if fU.startswith("STD"):
                        sigs = vblock.get("unified_sigma_prior_total", {})
                        if isinstance(sigs, dict):
                            g = sigs.get(fU.replace("STD", ""), None)  # STDMMI -> MMI
                            if g is not None:
                                return float(np.asarray(g, dtype=float)[iy, ix])
                    else:
                        mus = vblock.get("unified_mean", {})
        ...
        # (B) Generic dict structure: unified["lats"], unified["lons"], unified["versions"][v][field]
        if isinstance(unified, dict):
            lats = unified.get("lats", unified.get("lat", None))
            lons = unified.get("lons", unified.get("lon", None))
    
            vblock = None
            if "versions" in unified and isinstance(unified["versions"], dict):
                vblock = unified["versions"].get(version, None)
            if vblock is None and version in unified:
                vblock = unified.get(version, None)
    
            if isinstance(vblock, dict):
                grid = vblock.get(field, vblock.get(str(field).upper(), None))
            else:
                try:
                    grid = vblock[field]
                except Exception:
                    try:
                        grid = vblock[str(field).upper()]
                    except Exception:
                        grid = None
    
        # (C) Object-like
        if (grid is None) and (not isinstance(unified, dict)):
            lats = getattr(unified, "lats", getattr(unified, "lat", None))
            lons = getattr(unified, "lons", getattr(unified, "lon", None))
            vblock = getattr(unified, "versions", None)
            if isinstance(vblock, dict):
                vblock = vblock.get(version, None)
            if isinstance(vblock, dict):
                grid = vblock.get(field, vblock.get(str(field).upper(), None))
    ...
    def _uq__sample_on_grid(self, lats, lons, grid, lat: float, lon: float, method: str = "nearest"):
        """
        Sample value from a regular grid. Supports:
          - lats/lons as 1D vectors (rectilinear grid)
          - lats/lons as 2D meshes (curvilinear; treated approximately via nearest)
        """
        import numpy as np
        ...
        lats_arr = np.asarray(lats) if lats is not None else None
        lons_arr = np.asarray(lons) if lons is not None else None
        if lats_arr is None or lons_arr is None:
            return None
    
        # 2D lat/lon: nearest in 2D space
        ...
        if lats_arr.ndim == 1 and lons_arr.ndim == 1:
            iy = int(np.nanargmin(np.abs(lats_arr - lat)))
            ix = int(np.nanargmin(np.abs(lons_arr - lon)))
            ...
            if method.lower() == "bilinear":
                iy0 = max(0, min(iy, len(lats_arr) - 2))
                ix0 = max(0, min(ix, len(lons_arr) - 2))
                iy1 = iy0 + 1
                ix1 = ix0 + 1
    
                y0, y1 = lats_arr[iy0], lats_arr[iy1]
                x0, x1 = lons_arr[ix0], lons_arr[ix1]
        ...
        # fallback nearest in flattened space
        lat_flat = lats_arr.reshape(-1)
        lon_flat = lons_arr.reshape(-1)
        g_flat = g.reshape(-1)
        d2 = (lat_flat - lat) ** 2 + (lon_flat - lon) ** 2

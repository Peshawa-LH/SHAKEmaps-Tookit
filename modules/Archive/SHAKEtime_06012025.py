...
############3333
    ...
    # PASTE AT THE END OF SHAKEtime CLASS
    # ============================================================
    ...
    def _uq_base_dir(self):
        """
        Central UQ output directory:
          export/<event_id>/uq
        """
        from pathlib import Path
        if not hasattr(self, "uq_state") or self.uq_state is None:
            raise RuntimeError("UQ state not initialized yet.")
        base = Path(self.uq_state["base_folder"]) / "uq"
        #base = self._uq_base_dir()
        uq = base / "uq"
    ...
    def uq_bayes_update(
        ...
        for v in versions:
            ...
            if export:
                ...
                np.savez_compressed(vdir / f"bayesupdate_{imt}_posterior.npz",
                                    mu_post=mu_post,
    ...
    def uq_monte_carlo(
        ...
        for v in versions:
            ...
            # approach: filter white noise then standardize to unit variance approximately
            ...
            for _ in range(int(nsim)):
                ...
                if sigma_cells > 0:
                    ...
                    # standardize to unit variance (avoid division by 0)
                    es = np.nanstd(e)
                    if es > 0:
                        e = e / es
                x = mu + sig * e
            ...
            if export:
                vdir = self._uq_ensure_dir(base / f"v{int(v)}" / "uq_results")
                np.savez_compressed(vdir / f"montecarlo_{imt}_{which}_nsim{int(nsim)}.npz",
                                    mc_mean=mc_mean, mc_std=mc_std, mu=mu, sigma_used=sig, sigma_aleatory=np.array([sig_a], dtype=float))
    ...
    def uq_hierarchical(
        ...
        if export:
            ...
            np.savez_compressed(pdir / f"hierarchical_{imt}_posterior.npz",
                                mu_pool=mu_pool,
    ...
    # ---------------------------
    # Plot helpers (matplotlib-only; filenames include method)
    # ---------------------------
    def uq_plot_uncertainty_decay(
        ...
        for v in versions:
            ...
            if method == "prior":
                ...
                sig_ep = sig_ep_prior
            elif method == "bayesupdate":
                # load if exists; otherwise fallback to prior
            ...
            val = float(np.nanmean(sig_total if which == "total" else sig_ep))
            y.append(val)

        fig, ax = plt.subplots()
        ax.plot(x, y, marker="o")
        ax.set_xlabel(xlabel)
        ax.set_ylabel(f"Mean sigma ({which})")
        ax.set_title(f"{method}_uncertainty_decay_{which}_{imt}")

        if custom_xticks is not None:
            ax.set_xticks(custom_xticks)
        if custom_xticklabels is not None:
            ax.set_xticklabels(custom_xticklabels, rotation=rotation)
        else:
        ...
        if save:
            od = Path(out_dir) if out_dir is not None else Path(self.uq_state["base_folder"]) / "uq_plots"
            od = self._uq_ensure_dir(od)
            fname = od / f"{method}_uncertainty_decay_{which}_{imt}.png"
            fig.savefig(fname, dpi=int(dpi))
    ...
    def uq_plot_metric_area_ge_threshold(
        ...
        for v in versions:
            ...
            sig_use = sig_total if which_sigma == "total" else sig_ep
            poe = self._uq_poe_exceed(mu_use, sig_use, float(threshold))
            area = float(np.nansum(poe >= 0.5) * cell_area)  # binary PoE>=0.5 area
            areas.append(area)

        fig, ax = plt.subplots()
        ax.plot(x, areas, marker="o")
        ax.set_xlabel(xlabel)
        ax.set_ylabel("Area (km²) where PoE>=0.5")
        ax.set_title(f"{method}_area_ge_threshold_{imt}_thr{threshold}")
        for label in ax.get_xticklabels():
            label.set_rotation(rotation)
        fig.tight_layout()
        ...
        if save:
            od = Path(out_dir) if out_dir is not None else Path(self.uq_state["base_folder"]) / "uq_plots"
            od = self._uq_ensure_dir(od)
            fname = od / f"{method}_area_ge_threshold_{imt}_thr{threshold}.png"
            fig.savefig(fname, dpi=int(dpi))
    ...
    def uq_plot_metric_mean_poe(
        ...
        for v in versions:
            ...
            if method == "prior":
                ...
                p = _Path(self.uq_state["base_folder"]) / f"v{int(v)}" / "uq_results" / f"bayesupdate_{imt}_posterior.npz"
                if p.exists():
                    dat = np.load(p, allow_pickle=True)
                    mu_use = dat["mu_post"]
                    sig_total = dat["sigma_total_post"]
                ...
                p = _Path(self.uq_state["base_folder"]) / "uq_results" / f"hierarchical_{imt}_posterior.npz"
                if p.exists():
                    dat = np.load(p, allow_pickle=True)
                    mu_use = dat["mu_post"]
                    sig_total = dat["sigma_total_post"]
            ...
            sig_use = sig_total if which_sigma == "total" else sig_ep
            poe = self._uq_poe_exceed(mu_use, sig_use, float(threshold))
            mpoe.append(float(np.nanmean(poe)))

        fig, ax = plt.subplots()
        ax.plot(x, mpoe, marker="o")
        ax.set_xlabel(xlabel)
        ax.set_ylabel("Mean PoE")
        ax.set_title(f"{method}_mean_poe_{imt}_thr{threshold}")
        for label in ax.get_xticklabels():
            label.set_rotation(rotation)
        fig.tight_layout()
        ...
        if save:
            od = Path(out_dir) if out_dir is not None else Path(self.uq_state["base_folder"]) / "uq_plots"
            od = self._uq_ensure_dir(od)
            fname = od / f"{method}_mean_poe_{imt}_thr{threshold}.png"
            fig.savefig(fname, dpi=int(dpi))

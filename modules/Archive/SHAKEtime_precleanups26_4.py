...
############################
    ...
    def _uq_area_raw_values_from_xml(
        ...
        Key fix:
          Correctly maps grid_field indices to grid_data columns (absolute vs +2 offset),
          preventing "RAW missing" and ensuring RAW mean/bands appear.
        """
        import numpy as np
        ...
        # Parse once (your parser may now be cached)
        try:
            G = self._uq_parse_gridxml_to_arrays(grid_p)
            U = self._uq_parse_gridxml_to_arrays(unc_p)
        except Exception as e:
        ...
        if max_cells is not None and n > int(max_cells):
            if method == "stride":
                ...
                # auto: prefer stride unless user gave stride, otherwise random for irregular masks
        ...
        # Flattened row index into RAW grid_data
        nlon = int(G["nlon"])
    ...
    def uq_compare_raw_vs_unified_curves(
        ...
        # -----------------------------
        # FIX: export CSV to export/SHAKEtime/<event_id>/uq/
        # -----------------------------
        if export_table and output_path is not None:
            out_root = Path(output_path)
    
            # determine event id robustly

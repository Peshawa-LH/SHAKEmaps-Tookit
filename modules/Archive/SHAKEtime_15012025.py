...
############################
    ...
    def uq_plot_uq_sigma_map(
        ...
        *,
        ...
        axes = self.uq_state["unified_axes"]
        lat2d = np.asarray(axes["lat2d"], float)
        lon2d = np.asarray(axes["lon2d"], float)
        tr = self._uq_get_cartopy_transform()
        ...
        def _ensure_posteriors(method_name: str, v_need: int):
            ...
            # already have standardized?
            dat = self._uq_load_posterior_npz(method=method_name, imt=imtU, version=int(v_need), output_path=None)
            if dat is not None and ("sigma_ep_post" in dat) and ("sigma_total_post" in dat):
                return
            ...
            if method_name == "bayesupdate":
                # run the existing engine (may export legacy; we then bridge to standardized)
                ...
                # try to load from standardized (maybe you already write it somewhere)
                dat2 = self._uq_load_posterior_npz(method="bayesupdate", imt=imtU, version=int(v_need), output_path=None)
                if dat2 is not None and ("sigma_ep_post" in dat2) and ("sigma_total_post" in dat2):
                    return
    
                # otherwise: bridge legacy->standard using legacy loader
                ...
                self._uq_export_standard_posterior_from_legacy(
                    ...
                    version=int(v_need),
                    legacy=legacy,
                    output_path=posterior_output_path or output_path,
                    meta=meta,
                )
                return
        ...
        def _post_sigma(v):
            ...
            if ep is None or tot is None:
                # try standardized direct load (in case accessor not updated elsewhere)
                dat = self._uq_load_posterior_npz(method=m, imt=imtU, version=int(v), output_path=None)
                if dat is None:
        ...
        if not bool(panel_all):
            ...
            # pcolormesh with transform if cartopy is active
            if tr is not None and self._uq_is_cartopy_geoaxes(ax):
                pm = ax.pcolormesh(lon2d, lat2d, fld, transform=tr, cmap=cmap, vmin=vmin, vmax=vmax,
                                   shading="auto", zorder=5)
            else:
                pm = ax.pcolormesh(lon2d, lat2d, fld, cmap=cmap, vmin=vmin, vmax=vmax,
                                   shading="auto", zorder=5)
            ...
            if plot_colorbar:
                cb = fig.colorbar(pm, ax=ax, shrink=0.85, orientation=str(cbar_orientation))
                cb.set_label(cbar_label or lbl)
        ...
        # Use cartopy subplots if cartopy is available
        if tr is not None:
            try:
                import cartopy.crs as ccrs
        ...
        for i, (a, b, tag) in enumerate(pairs):
            ...
            if cartopy_panels:
                pm = ax.pcolormesh(lon2d, lat2d, fld, transform=tr, cmap=cmap, vmin=vmin, vmax=vmax, shading="auto")
            else:
                pm = ax.pcolormesh(lon2d, lat2d, fld, cmap=cmap, vmin=vmin, vmax=vmax, shading="auto")
        ...
        if plot_colorbar and (mappable is not None):
            cb = fig.colorbar(mappable, ax=axes[:n], shrink=0.85, orientation=str(cbar_orientation))
            cb.set_label(cbar_label or (last_lbl or ""))
